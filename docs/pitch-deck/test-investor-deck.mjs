...
test('both editions have 16 readable native frames with source links',async()=>{
 ...
 for(const lang of ['fr','en']){await page.goto(`${url}/pitch-deck.html?lang=${lang}`);await page.evaluate(()=>document.fonts.ready);assert.equal(await page.locator('.thumb').count(),16);
  for(let n=1;n<=16;n++){await page.locator(`.thumb[data-slide="${n}"]`).click();assert.equal(await page.locator('#slide>svg').count(),1);const bad=await page.locator('#slide svg').evaluate(svg=>{const b=[...svg.querySelectorAll('text')].map(t=>({text:t.textContent,...Object.fromEntries(['x','y','width','height'].map(k=>[k,t.getBBox()[k]]))}));const issues=b.filter(t=>t.x<65||t.x+t.width>1535||t.y<0||t.y+t.height>895).map(t=>t.text);for(let i=0;i<b.length;i++)for(let j=i+1;j<b.length;j++)if(Math.min(b[i].x+b[i].width,b[j].x+b[j].width)-Math.max(b[i].x,b[j].x)>3&&Math.min(b[i].y+b[i].height,b[j].y+b[j].height)-Math.max(b[i].y,b[j].y)>3)issues.push(b[i].text+' / '+b[j].text);return issues});assert.deepEqual(bad,[],`${lang} slide ${n}`);assert.ok((await page.locator('#slideNote').textContent()).length>20);if([4,6,9,10].includes(n))assert.ok(await page.locator('#sourceLinks a').count()>0)}
 }assert.deepEqual(errors,[]);await page.close();
});
test('fullscreen owns the only in-stage logo; replay, pause and keyboard work',async()=>{
 const page=await browser.newPage({viewport:{width:1440,height:1000}});await page.goto(`${url}/pitch-deck.html?lang=fr#slide-7`);
 ...
 await page.keyboard.press('ArrowRight');assert.match(await page.locator('#stage').getAttribute('aria-label'),/8:/);await page.locator('[data-fs=exit]').click();await page.waitForFunction(()=>!document.fullscreenElement);assert.equal(await page.locator('.fullscreen-brand').isVisible(),false);
 await page.goto(`${url}/pitch-deck.html?lang=en#slide-16`);assert.equal(await page.locator('#nextControl').isDisabled(),true);assert.match(await page.locator('#summaryLink').getAttribute('href'),/nota-pitch-deck\.pdf/);await page.close();
});

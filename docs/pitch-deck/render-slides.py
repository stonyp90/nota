...
Palette and type follow the brand contract (ADR 0048 tokens, ADR 0050 scale):
the dark canvas set from apps/web/public/styles.css, Sora 800 for titles and
Inter for everything else. The fonts must be installed locally (fontconfig):
ImageMagick substitutes a generic sans silently when a family is missing, so
the build refuses to run rather than bake a fallback face into 32 PNGs.

Run:  python3 docs/pitch-deck/render-slides.py
"""
...
ROOT = OUT.parents[1]
W, H = 1600, 900
# ADR 0048 dark canvas set (apps/web/public/styles.css :root[data-theme="dark"]).
BG = "#101820"        # --bg
PANEL = "#0f2030"     # --surface
PANEL_2 = "#132b3f"   # --surface-inset
WHITE = "#f4f8fa"     # --ink
MUTED = "#afc2cf"     # --ink-muted
BLUE = "#78a9bf"      # --nota-blue-400, the accent on dark
ORANGE = "#f79009"    # --nota-orange-400, the warm accent on dark
GREEN = "#407598"     # --nota-blue-500, the signal dot (no green in the brand)
LINE = "#294353"      # --border
MARK = "#264961"      # --nota-blue-900, the mark square
...
def rect(x: float, y: float, w: float, h: float, fill: str = PANEL,
         stroke: str = "none", radius: int = 18, opacity: float = 1) -> str:
...
def circle(cx: float, cy: float, r: float, fill: str = "none",
    ...
    return f'<circle cx="{cx}" cy="{cy}" r="{r}" fill="{fill}" stroke="{stroke}" stroke-width="{width}" opacity="{opacity}" />'
...
def base(num: int, title: str, subtitle: str, kicker: str, body: list[str]) -> list[str]:
    out = [
        ...
        rect(72, 58, 56, 56, MARK, "none", 8),
        rect(88, 72, 8, 30, WHITE, "none", 2), rect(108, 72, 8, 30, WHITE, "none", 2),
        path("M88 72 L96 72 L116 102 L108 102 Z", WHITE, "none", 0),
        circle(116, 72, 7, GREEN, MARK, 3),
        display(148, 92, "NOTA", 18, WHITE, "start", 2),
        t(238, 92, "QUÉBEC", 15, ORANGE, 700, "start", 2),
        t(1528, 91, f"{num:02d} / 16", 17, ORANGE, 700, "end", 1),
        t(80, 177, kicker.upper(), 18, ORANGE, 700, "start", 3),
    ]
    title_lines = title.split("\n")
    for index, title_line in enumerate(title_lines):
        # ImageMagick's SVG renderer ghosts glyphs with negative tracking.
        # Keep titles clean and let the display font carry the hierarchy.
        out.append(display(80, 249 + index * 62, title_line, 54 if len(title_lines) == 1 else 49))
    subtitle_y = 320 + (len(title_lines) - 1) * 62
    out.append(t(80, subtitle_y, subtitle, 25, MUTED, 450))
    out.extend(body)
    out.extend([line(80, 838, 1520, 838, LINE, 2), t(80, 870, "info@gonota.ca", 17, MUTED, 500), t(1520, 870, "Nota · capacity, clarity, trust", 17, MUTED, 500, "end")])
    out.append("</svg>")
    return out
...
def slide_1(lang: str) -> list[str]:
    ...
    body = [
        circle(1180, 475, 210, "none", BLUE, 2, .65), circle(1180, 475, 144, "none", ORANGE, 2, .85),
        circle(1180, 475, 68, PANEL_2, ORANGE, 3),
        display(1180, 492, "N", 68, WHITE, "middle"),
        circle(958, 475, 9, BLUE, "none", 0), circle(1402, 475, 9, ORANGE, "none", 0),
        t(1180, 735, "CAPACITY INFRASTRUCTURE", 18, BLUE, 700, "middle", 3),
...
def slide_2(lang: str) -> list[str]:
    ...
    body = [
        ...
        circle(240, 505, 16, ORANGE, "none", 0), circle(800, 505, 16, BLUE, "none", 0), circle(1370, 505, 16, ORANGE, "none", 0),
        ...
        t(1370, 575, "PHONE", 42, ORANGE, 700, "middle"),
...
def slide_3(lang: str) -> list[str]:
    ...
    body = [
        rect(110, 425, 525, 245, PANEL, BLUE, 22), rect(965, 425, 525, 245, PANEL, ORANGE, 22),
        ...
        t(1005, 480, "NOTAIRE" if fr else "NOTARY", 18, ORANGE, 700, letter=3),
        ...
        line(640, 548, 955, 548, BLUE, 4, "10 14"), circle(800, 548, 22, BG, ORANGE, 4),
...
def slide_5(lang: str) -> list[str]:
    ...
    cards = [("4", "services au catalogue" if fr else "services in code", BLUE), ("0", "dépendance runtime" if fr else "runtime dependencies", ORANGE), ("1", "flux de preuve" if fr else "evidence loop", GREEN)]
    body: list[str] = []
    ...
    body.extend([line(180, 720, 1400, 720, BLUE, 4), circle(260, 720, 12, ORANGE, "none", 0), circle(800, 720, 12, BLUE, "none", 0), circle(1340, 720, 12, GREEN, "none", 0), t(260, 770, "REQUEST", 16, MUTED, 700, "middle", 2), t(800, 770, "ACT", 16, MUTED, 700, "middle", 2), t(1340, 770, "EVIDENCE", 16, MUTED, 700, "middle", 2)])
...
def slide_6(lang: str) -> list[str]:
    ...
    body = [
        ...
        circle(1320, 530, 105, "none", GREEN, 4), t(1320, 520, "CNQ", 33, GREEN, 700, "middle"), t(1320, 560, "gate", 23, WHITE, 600, "middle"),
...
def slide_7(lang: str) -> list[str]:
    ...
    body = [
        ...
        circle(535, 575, 14, ORANGE, "none", 0), t(535, 625, "Québec", 23, WHITE, 700, "middle"),
        line(745, 590, 920, 590, ORANGE, 4, "8 12"),
    ]
    for step, (x, label, _, color, verb) in enumerate([(980, "QUÉBEC", "Prove", BLUE, "Prouver" if fr else "Prove"), (1220, "CANADA", "Expand", ORANGE, "Étendre" if fr else "Expand"), (1450, "GLOBAL", "Option", GREEN, "Option")], 1):
        body.extend([circle(x, 590, 45, BG, color, 4), t(x, 584, str(step), 28, color, 700, "middle"), t(x, 690, label, 17, color, 700, "middle", 2), t(x, 726, verb, 20, WHITE, 600, "middle")])
    body.extend([
...
def slide_9(lang: str) -> list[str]:
    ...
    body = []
    for i, (num, label, color) in enumerate([("30", "notaires recrutés" if fr else "notaries recruited", BLUE), ("25", "vérifiés et configurés" if fr else "verified and configured", ORANGE), ("244", "actes ciblés en Y1" if fr else "target Y1 completions", GREEN)]):
        x = 110 + i * 490
    ...
    body.extend([line(140, 710, 1450, 710, BLUE, 4), circle(140, 710, 9, BLUE, "none", 0), circle(800, 710, 9, ORANGE, "none", 0), circle(1450, 710, 9, GREEN, "none", 0), t(140, 760, "MOIS 1", 16, MUTED, 700, "middle", 2), t(800, 760, "MOIS 6", 16, MUTED, 700, "middle", 2), t(1450, 760, "MOIS 12", 16, MUTED, 700, "middle", 2)])
...
def slide_10(lang: str) -> list[str]:
    ...
    nodes = [
        ...
        (910, "REVUE NOTARIALE" if fr else "NOTARY REVIEW", "accepte · corrige · rejette" if fr else "accept · correct · reject", GREEN),
    ...
    body.extend([
        ...
        t(850, 805, "S’ABONNER" if fr else "SUBSCRIBE", 17, GREEN, 750, letter=2),
...
def slide_11(lang: str) -> list[str]:
    ...
    body = [
        rect(120, 430, 400, 190, PANEL, BLUE, 18), rect(600, 430, 400, 190, PANEL, ORANGE, 18), rect(1080, 430, 400, 190, PANEL, GREEN, 18),
        ...
        t(1280, 495, "CAPACITÉ", 18, GREEN, 700, "middle", 2), t(1280, 550, "Rendre utile" if fr else "Put to work", 31, WHITE, 700, "middle"), t(1280, 590, "offre additionnelle" if fr else "incremental supply", 20, MUTED, 500, "middle"),
...
def slide_12(lang: str) -> list[str]:
    ...
    body = [
        rect(110, 430, 420, 210, PANEL, BLUE, 20), rect(590, 430, 420, 210, PANEL, ORANGE, 20), rect(1070, 430, 420, 210, PANEL, GREEN, 20),
        ...
        t(1105, 500, "−204 623 $", 48, GREEN, 700), t(1105, 552, "résultat opérationnel" if fr else "operating result", 22, WHITE, 650), t(1105, 596, "avant preuve commerciale" if fr else "before commercial proof", 17, MUTED, 700, letter=1),
        line(150, 730, 1450, 730, BLUE, 4),
        t(150, 780, "Y1", 18, BLUE, 700), t(800, 780, "Y2", 18, ORANGE, 700, "middle"), t(1450, 780, "Y3", 18, GREEN, 700, "end"),
...
def slide_13(lang: str) -> list[str]:
    ...
    rows = [("Acquisition", 55000, BLUE, "notaires + clients" if fr else "notaries + clients"), ("Juridique + conseil", 45000, ORANGE, "avis + workflow" if fr else "opinion + workflow"), ("Produit + opérations", 150000, GREEN, "focus + résilience" if fr else "focus + resilience")]
    y = 485
...
def slide_14(lang: str) -> list[str]:
    ...
    body: list[str] = []
    gates = [("01", "30", "recrutés" if fr else "recruited", BLUE), ("02", "25", "vérifiés" if fr else "verified", ORANGE), ("03", "244", "complétés" if fr else "completed", GREEN)]
    for i, (step, num, label, color) in enumerate(gates):
        x = 110 + i * 490
        body.extend([circle(x + 42, 455, 31, BG, color, 4), t(x + 42, 464, step, 18, color, 700, "middle"), t(x + 40, 555, num, 72, color, 700), t(x + 40, 610, label, 24, WHITE, 650), line(x + 40, 660, x + 395, 660, color, 4)])
...
def slide_15(lang: str) -> list[str]:
    ...
    body = [
        ...
        rect(570, 425, 870, 78, PANEL, BLUE, 14), rect(570, 535, 870, 78, PANEL, ORANGE, 14), rect(570, 645, 870, 78, PANEL, GREEN, 14),
        ...
        t(610, 696, "CAPACITÉ", 18, GREEN, 700, letter=2), t(900, 696, "preuve avant expansion", 26, WHITE, 650),
...
def slide_16(lang: str) -> list[str]:
    ...
    body = [
        circle(300, 555, 136, "none", ORANGE, 5), path("M235 555 L280 600 L375 490", "none", ORANGE, 10), t(300, 730, "GATE", 18, ORANGE, 700, "middle", 3),
        rect(650, 420, 770, 78, PANEL, BLUE, 14), rect(650, 535, 770, 78, PANEL, ORANGE, 14), rect(650, 650, 770, 78, PANEL, GREEN, 14),
        ...
        t(690, 700, "ACTE", 20, GREEN, 700, letter=2), t(850, 700, "consentement, conseil, signature", 25, WHITE, 650),

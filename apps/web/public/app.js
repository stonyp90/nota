...
(function () {
    ...
    async listMonth(month) {
      try {
        ...
        if (r.ok) {
          this.online = true;
          var j = await r.json();
          if (j.experience && (j.experience.mode === 'standard' || j.experience.mode === 'guided')) {
            state.experience = j.experience;
            window.NotaExperience = j.experience;
          }
  ...
  // The marketplace doors and Beta are in the header; the remaining panes
  // are inner destinations reached from the account menu and product flows.
  var PANES = ['carnet', 'dossier', 'notaires', 'partenaires', 'beta', 'profil', 'confidentialite', 'conditions', 'charte'];
  ...
  var state = {
    ...
    focusDate: todayISO(),
    tab: 'carnet',
    // The server may turn on a bounded guidance mode after the daily customer
    // improvement check. The mode never changes questions or legal controls.
    experience: D.publicCustomerExperience ? D.publicCustomerExperience(null) : { mode: 'standard' },
    offer: { serviceId: '', dateISO: '', montant: 0, anonyme: true, pricing: {}, touched: false, stale: false },
  };
  // The in-app history rail is intentionally a thin layer over browser history.
  // It gives customers an obvious way out without inventing a second navigation
  // model, and the browser Back/Forward buttons remain authoritative.
  var navHistory = [{ tab: state.tab, url: location.href }];
  var navHistoryIndex = 0;
  window.NotaExperience = state.experience;
  ...
  var DEMO_REGIONS = [
    ...
    // La vitrine de l'arrivée : ses prix sont calibrés sur state.monthBids
    // comme ceux du calendrier, donc inventés quand le mois l'est.
    { region: 'ig-carnet', slot: '.ig-cal-head' },
  ];
  function demoMark() {
  ...
  function showOfferSuccess() {
    ...
    // L'offre est publiée : plus aucun écran, ni rail, ni barre d'action — la
    // confirmation est seule (ADR 0040). openDay() repose data-at à 1.
    var bf = bookForm(); if (bf) bf.dataset.at = 'done';
  }
  ...
  var SVC_ICONS = {
    ...
    // Les deux actes annoncés (D.ACTES_A_VENIR) : ils ne sont pas de la
    // famille du financement, alors ils quittent la maison — une feuille
    // signée pour la procuration, une feuille scellée pour le testament.
    procuration: '<path d="M7 3h7l4 4v14H7z"/><path d="M14 3v4h4"/><path d="M10 16.2c.9-1.3 1.8-1.3 2.7 0s1.8 1.3 2.7 0"/>',
    testament: '<path d="M6 3h8l4 4v8H6z"/><path d="M14 3v4h4"/><circle cx="12" cy="18" r="2.6"/><path d="m10.3 20.2-.5 2.6 2.2-1.1 2.2 1.1-.5-2.6"/>',
  };
  // Un glyphe au trait, construit UNE fois : même viewBox, même graisse, même
  // invisibilité pour les lecteurs d'écran. `cls` dit à quel registre il
  // appartient (l'acte, la section de questions…).
  function strokeGlyph(paths, size, cls) {
  ...
  function svcIcon(id, size) {
    ...
    return strokeGlyph(SVC_ICONS[id], size, 'svc-ic');
  }

  // Un glyphe par SECTION de questions, clé pour clé sur les groupes du domaine
  // (D.CRITERIA_GROUPS). Le sujet se reconnaît AVANT que son titre se lise —
  // « ceci parle du prêt », « ceci parle de l'immeuble », « ceci parle de la
  // signature » — ce qui est tout ce qu'on demande à un en-tête de groupe.
  // Une section sans glyphe rend son titre seul : jamais un trou.
  var CRIT_GROUP_ICONS = {
    // Le prêt : un billet. Pas la maison de l'acte — ce qu'on emprunte, pas
    // ce sur quoi on l'emprunte.
    pret: '<rect x="2" y="6" width="20" height="12" rx="2"/><circle cx="12" cy="12" r="2.6"/><path d="M6 12h.01M18 12h.01"/>',
    // L'immeuble : la maison POSÉE SUR SON SOL — le trait au sol la distingue
    // du glyphe de l'acte, qui vit deux cartes plus haut sur le même écran.
    immeuble: '<path d="M3 21h18"/><path d="M5 21V9l7-5 7 5v12"/><path d="M10 21v-6h4v6"/>',
    // La signature : la plume sur la ligne.
    signature: '<path d="M3 20h18"/><path d="M15.6 4.4a2.1 2.1 0 0 1 3 3L9 17l-4 1 1-4z"/>',
    // L'écran 3 tient ses sections en dur dans index.html (elles ne sortent
    // d'aucun catalogue) : elles nomment leur glyphe par `data-sec-ic` et sont
    // décorées au démarrage. Même registre, même bande, même lecture.
    montant: '<path d="M12 2v20"/><path d="M17 6.5H9.8a3 3 0 0 0 0 6h4.4a3 3 0 0 1 0 6H6"/>',
    marche: '<path d="M3 21h18"/><path d="M6 21v-6M11 21V9M16 21v-9M21 21V5"/>',
    devis: '<path d="M7 3h10v18l-3-2-2 2-2-2-3 2z"/><path d="M10 8h4M10 12h4"/>',
  };
  function critGroupIcon(id, size) {
    if (!CRIT_GROUP_ICONS[id]) return null;
    var box = el('span', 'crit-sec-ic');
    box.appendChild(strokeGlyph(CRIT_GROUP_ICONS[id], size || 15, 'crit-sec-glyph'));
    return box;
  }
  ...
  // ---------------------------------------------------------------------------
  // Info tip — the house pattern for a SITUATIONAL note.
  //
  // « Sans les instructions du prêteur… », « Peu de notaires se déplacent
  // jusqu'à 50 km », « Vos réponses précédentes — vérifiez-les » : sentences
  // that appear because of an answer and leave with the next one. Printed as a
  // line under their question they GREW the card that held them and pushed
  // every card below it down the page, then let it snap back — the form moved
  // under the client's hand while they were reading it.
  //
  // The sentence now rides a small « i » pinned to the question's own line and
  // opens in a panel that floats OVER the page. The form's height stops
  // depending on what its notes have to say. The tip keeps its box whether or
  // not it has something to say (CSS: visibility, never display), so even the
  // label line does not jitter — and a visibility:hidden control is neither
  // focusable nor announced, so a tip with nothing to say is silent too.
  //
  // Three ways in, because a tooltip that only answers the mouse answers
  // nobody on a phone or a keyboard: hover, keyboard focus, and a click that
  // PINS the panel open until Escape, an outside click, or another tip.
  // ---------------------------------------------------------------------------
  var ITIP_SEQ = 0;
  var itipOpen = null;   // the one PINNED panel — two never stand open at once
  var itipBound = false;

  function itipBindGlobals() {
    if (itipBound || typeof document === 'undefined') return;
    itipBound = true;
    document.addEventListener('keydown', function (e) {
      if (e.key !== 'Escape' || !itipOpen) return;
      var btn = itipOpen.querySelector('.itip-btn');
      itipClose();
      if (btn && btn.focus) btn.focus();
    });
    // Capture: a click anywhere else dismisses the pinned panel before that
    // click does its own work.
    document.addEventListener('pointerdown', function (e) {
      if (itipOpen && !itipOpen.contains(e.target)) itipClose();
    }, true);
  }

  // While a panel is PINNED it has to follow its « i » — a dialog that scrolls
  // under it would leave it stranded mid-page. The listeners exist only for as
  // long as a panel is pinned: an idle form listens to nothing.
  function itipTrack() { if (itipOpen) itipPlace(itipOpen); }
  ...
  // Pick the side and the alignment that keep the panel on screen — measured
  // ONLY when a panel is about to be seen (hover, focus, click), never on
  // render and never on scroll, so an idle form measures nothing.
  function itipPlace(tip) {
    var pop = tip.querySelector('.itip-pop');
  ...
  // Le glyphe dit le TON avant qu'on lise le panneau : « i » pour un
  // renseignement, « ! » pour une situation qui demande attention. Deux « i »
  // identiques côte à côte — l'aide de la question et sa note de situation —
  // ne se distinguaient que par la couleur (2026-09-05).
  function itipGlyph(tone) {
    var svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
    svg.setAttribute('viewBox', '0 0 24 24');
    ...
    svg.setAttribute('stroke-linecap', 'round'); svg.setAttribute('stroke-linejoin', 'round');
    svg.setAttribute('aria-hidden', 'true');
    svg.innerHTML = tone === 'warn'
      ? '<circle cx="12" cy="12" r="9"/><path d="M12 7.6v5"/><path d="M12 16v.6"/>'
      : '<circle cx="12" cy="12" r="9"/><path d="M12 11.2v5"/><path d="M12 7.6v.6"/>';
    return svg;
  }

  // opts: { id, tone: 'warn' | 'info', cls }. The returned node carries
  // setNote(content) — a string, a node, or an array of nodes (the caveats are
  // composed of separate text nodes so i18n can match each sentence whole).
  // setNote('') stands the tip down: the box stays, the signal leaves.
  function infoTip(opts) {
    opts = opts || {};
    var tip = el('span', 'itip' + (opts.cls ? ' ' + opts.cls : ''));
    tip.dataset.on = 'false';
    tip.dataset.tone = opts.tone === 'warn' ? 'warn' : 'info';
    if (opts.id) tip.id = opts.id;
    var popId = (opts.id || 'itip-' + (++ITIP_SEQ)) + '__pop';
    var btn = el('button', 'itip-btn');
    btn.type = 'button';
    // The icon is never the label: « Information » names the control, and the
    // panel it points at is what a screen reader reads out after it.
    btn.setAttribute('aria-label', 'Information');
    btn.setAttribute('aria-expanded', 'false');
    btn.setAttribute('aria-controls', popId);
    btn.setAttribute('aria-describedby', popId);
    btn.appendChild(itipGlyph(tip.dataset.tone));
    var pop = el('span', 'itip-pop');
    pop.id = popId;
    ...
    btn.addEventListener('click', function () {
      ...
      itipOpen = tip;
      itipPlace(tip);
      window.addEventListener('resize', itipTrack);
      document.addEventListener('scroll', itipTrack, true);
    });
    ...
    tip.setNote = function (content) {
      clear(pop);
      var has = false;
      if (typeof content === 'string') {
        if (content) { pop.textContent = content; has = true; }
      } else if (content) {
        [].concat(content).forEach(function (n) { if (n) { pop.appendChild(n); has = true; } });
      }
      var was = tip.dataset.on === 'true';
      tip.dataset.on = has ? 'true' : 'false';
      // Empty contextual tips are intentionally kept as reusable nodes across
      // re-renders, but they must leave both the layout and the tab order. A
      // visibility-only rule still leaves an invisible information button
      // focusable and can make multiple tips occupy the same top-right rail.
      tip.hidden = !has;
      btn.tabIndex = has ? 0 : -1;
      if (!has) {
        if (itipOpen === tip) itipClose();
        tip.removeAttribute('data-fresh');
        return tip;
      }
      // A note that has just arrived wakes its « i » once — a signal that no
      // longer takes a line of its own still has to be noticed.
      if (!was) {
        tip.removeAttribute('data-fresh');
        void tip.offsetWidth;
        tip.dataset.fresh = 'true';
      }
      return tip;
    };
    return tip;
  ...
  // The question's own line: its label, then whatever used to print itself
  // underneath — the awaited-answer mark, the situational tips.
  function critHead(labelEl) {
  ...
  // Toujours un TABLEAU : une valeur corrompue ou d'une ancienne forme faisait
  // lever `.forEach` en plein rendu du calendrier, donc écran blanc au boot.
  // Une clé illisible doit coûter l'historique local, jamais l'application.
  function myOffers() { var a = lsLoad(LS_MYOFFERS); return Array.isArray(a) ? a : []; }
  ...
  function applyOfferStatus(o, j) {
    try {
      ...
      var wasRetained = !!(prev && prev.bid && prev.bid.status === D.STATUS.RETENUE);
      ...
      // Le DOSSIER que le serveur détient, replié dans l'appareil. Sans ça, la
      // reprise n'était faite qu'à moitié : un client sur un téléphone neuf
      // retrouvait sa demande et une liste de documents vide, alors que ses
      // réponses étaient là. On FUSIONNE, on ne remplace pas — le serveur ne
      ...
      if (st.bid && st.bid.status === D.STATUS.RETENUE) {
        markMyOfferRetained(o.id, { etude: st.bid.etude, montant: st.bid.montant });
        // A live client session learns about an acceptance through this status
        // poll. Keep the same durable bell entry used at boot and confirm it
        // immediately when the customer is already looking at Mes offres.
        addRetainedNotif(o, st.bid);
        if (!wasRetained && prev && prev.bid && prev.bid.status !== D.STATUS.RETENUE) {
          toast('Votre demande est retenue à ' + D.money(Number(st.bid.montant) || Number(o.montant) || 0));
        }
      }
  ...
  function clientPollTick() {
    ...
    clientPollBusy = true;
    // Poll the status and the server journal together. Status updates repaint
    // the offer; the journal supplies the durable acceptance notice (and any
    // message/document notice) even when the customer never reloads the page.
    Promise.all([
  ...
  // ADR 0041 — l'indemnité d'une annulation tardive, telle que le client la
  // lit : rien n'est prélevé à l'annulation ; le notaire peut RÉCLAMER, sur
  // justification et dans un délai, jusqu'à un plafond. Chaque statut a sa
  // phrase, et une phrase composée a sa règle de traduction.
  function keptPhrase(kept) {
    if (!kept) return null;
    var st = kept.statut;
    if (st === 'en_attente') {
      return 'Votre notaire peut réclamer, sur justification et dans les ' + (Number(kept.delaiJours) || 7) + ' jours, une indemnité allant jusqu’à '
        + D.money(kept.plafond) + ' (' + pctLabel(kept.taux) + ' du montant convenu). Rien n’est retenu pour l’instant.';
    }
    if (st === 'renoncee') return 'Votre notaire n’a réclamé aucune indemnité : rien n’est retenu.';
    if (st === 'expiree') return 'Aucune indemnité n’a été réclamée dans le délai : rien n’est retenu.';
    if (!(Number(kept.frais) > 0)) return null;
    var amt = D.money(kept.frais);
    if (kept.percu === false) {
      return 'Votre notaire a réclamé une indemnité de ' + amt + ', mais votre carte a refusé le prélèvement : rien n’a été débité.';
    }
    if (kept.mecanisme === 'hors_session') {
      return 'Une indemnité de ' + amt + ', justifiée par le notaire, a été portée à la carte que vous avez enregistrée et lui est versée en dédommagement.';
    }
    return 'Une indemnité de ' + amt + ', justifiée par le notaire, a été retenue sur la somme réservée pour cet acte et lui est versée en dédommagement.';
  }
  ...
  // Le serveur nomme ses genres (domain.NOTIF_KINDS) ; la carte
  // « Notifications » nomme ses interrupteurs. Les deux vocabulaires sont
  // volontairement distincts — l'un décrit un événement, l'autre une
  // préférence — mais ils doivent se rejoindre, sans quoi `notifAllowed`
  // interroge une clé absente, la lit comme « pas false », et un interrupteur
  // fermé ne ferme rien.
  var SERVER_NOTIF_PREF = {
    message: 'message',
    document: 'documents',
    retenue: 'retained',
  ...
  function notifPrefKey(kind) {
    if (!kind) return null;
    return SERVER_NOTIF_PREF[kind] || kind;
  }
  ...
  function roleGet() { var r = flagGet(LS_ROLE); return r === 'notary' || r === 'client' || r === 'partner' ? r : ''; }
  function roleSet(role) { if (role === 'notary' || role === 'client' || role === 'partner') flagSet(LS_ROLE, role); }
  ...
  // ---------------------------------------------------------------------------
  // Doors that need an account
  //
  // The owner, 2026-09-12: « les sections qui doivent nécessiter une
  // authentification ne sont pas disponibles. Donc la signature ne doit pas
  // être disponible tant et aussi longtemps que l'usager ne soit pas signé.
  // Faire de même pour tous les autres. »
  //
  // So a signed-out visitor is not shown a door they cannot walk through: the
  // tab goes from the header and the drawer, and a deep link to the pane lands
  // on the carnet with the sign-in sheet open instead of a dead end.
  //
  // `profil` and `dossier` are NOT on this list, and that is deliberate: they
  // are where a visitor BECOMES authenticated. The profile is where a client
  // enters the address that signs them in on this device, and a client who
  // published without an account still holds that offer's bearer here — their
  // dossier is reachable from the post-publish card, authenticated by token
  // rather than by profile. Closing either would lock people out of their own
  // sign-in surface. The doors that need an account, and have nothing to offer
  // without one, are the list below.
  // ---------------------------------------------------------------------------
  var GATED_PANES = ['beta'];

  function paneNeedsAccount(tab) {
    return accountRole() === 'anon' && GATED_PANES.indexOf(tab) !== -1;
  }

  // The header tab and the drawer link for every closed door.
  function syncGatedDoors() {
    GATED_PANES.forEach(function (tab) {
      var closed = paneNeedsAccount(tab);
      document.querySelectorAll('.nav-tab[data-tab="' + tab + '"], .mnav-link[data-tab="' + tab + '"]').forEach(function (el) {
        el.hidden = closed;
      });
    });
  }
  ...
  function oauthLoadProviders() {
    if (!oauthLoading) oauthLoading = fetch('/api/auth/oauth/providers', { credentials: 'same-origin' }).then(function (r) {
      ...
      if (Object.keys(oauthConfigured).length && $('auth-social-note')) $('auth-social-note').textContent = T('À la première connexion, confirmez votre courriel pour lier votre compte. Microsoft accepte les comptes Outlook et Microsoft 365.');
  ...
  function authRenderProviders() {
    var list = $('auth-soc-list'); if (!list) return;
    clear(list);
    var live = $('auth-soc-live'); if (live) live.textContent = ''; // pas de message d'un autre fournisseur
    (AUTH_PROVIDER_ORDER[authRole] || AUTH_PROVIDER_ORDER.client).forEach(function (id) {
      var p = AUTH_PROVIDERS[id]; if (!p) return;
      var b = el('button', 'auth-soc-btn'); b.type = 'button';
      ...
      b.addEventListener('click', function () {
        ...
        var live = $('auth-soc-live');
        if (live) {
          live.textContent = T('Connexion indisponible. Réessayez ou utilisez votre courriel.');
          if (live.scrollIntoView) { try { live.scrollIntoView({ block: 'nearest' }); } catch (e) {} }
        }
        var em = $('auth-email'); if (em) { try { em.focus({ preventScroll: true }); } catch (e) { try { em.focus(); } catch (e2) {} } }
      });
      list.appendChild(b);
    });
  }
  ...
  function authSetRole(role) {
    ...
    authRenderProviders();   // l'ordre des portes suit le métier
    authRenderSwitch();      // …et la bascule nomme la destination
  ...
  var authMode = 'signup';
  // La bascule nomme la DESTINATION, jamais l'état qu'on regarde déjà : « vous
  // avez déjà publié ? Se connecter » depuis l'inscription, et l'inverse. Sans
  // elle il fallait fermer la modale et viser l'autre bouton de l'en-tête.
  function authRenderSwitch() {
    var q = $('auth-switch-q'), b = $('auth-switch-btn');
    if (!q || !b) return;
    if (authMode === 'signin') {
      q.textContent = authRole === 'notary' ? 'Pas encore inscrit comme notaire ?' : 'Première visite sur Nota ?';
      b.textContent = authRole === 'notary' ? 'Créer mon compte' : 'S’inscrire';
    } else {
      q.textContent = authRole === 'notary' ? 'Vous avez déjà un compte notaire ?' : 'Vous avez déjà publié une demande ?';
  ...
  function openAuthModal(role, mode) {
    ...
    if (dlg && dlg.showModal) {
      ...
      setTab('profil', { focus: false, gate: false });
  ...
  // Basculer sans rien perdre : ni l'adresse déjà tapée, ni le rôle choisi.
  // Rouvrir la modale les écraserait tous les deux.
  function authToggleMode() {
    authMode = authMode === 'signin' ? 'signup' : 'signin';
    var errs = $('auth-errors'); if (errs) errs.hidden = true;
    authSetRole(authRole);   // repeint titre, CTA, mentions, portes et bascule
  ...
  function authSubmitEmail(e) {
    ...
    if (authRole === 'notary') {
      ...
      // The mode decides, because the API cannot. /notary/session/request is
      // enumeration-safe: for an address that is NOT yet a notary it answers
      // 200 and sends nothing. Routing a sign-UP through it therefore ended on
      // « Nous venons d'envoyer un lien » for a mail that would never arrive.
      // Signing up opens the free-signup step instead; signing in still asks
      // for the magic link (which is the security boundary, unchanged).
      if (authMode === 'signup') ncShowSignup(val);
      else ncSignIn(val);
      return;
    }
    // « Se connecter » veut dire « j'ai déjà des demandes, rends-les-moi » —
    // et ça, seul le serveur peut le faire. Il demande donc le lien à usage
    // unique (même geste que le notaire) plutôt que d'écrire une adresse dans
    // le localStorage et de se déclarer connecté sans preuve de boîte.
    // « S'inscrire » reste le geste léger : l'adresse vit sur l'appareil.
    if (authMode === 'signin') {
      clientLinkRequest(val);
      if (dlg && dlg.close) { try { dlg.close(); } catch (e) {} }
      return;
    }
  ...
  // --- L'espace client : retrouver SES demandes ------------------------------
  // Étape 1. La réponse est volontairement la MÊME pour une adresse connue et
  // pour une inconnue (anti-énumération), donc l'écran ne peut rien affirmer
  // d'autre que « si cette adresse a des demandes, le lien part ». Dire
  // « courriel envoyé » serait déjà en dire trop.
  async function clientLinkRequest(courriel) {
    var r;
    try {
      r = await fetch(API_BASE + '/client/session/request', {
        method: 'POST', headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ courriel: courriel }),
      });
    } catch (e) {
      toast('Hors ligne. Réessayez une fois en ligne.');
      return { ok: false };
    }
    var j = {}; try { j = await r.json(); } catch (e) {}
    if (r.status === 422) { toast('Entrez un courriel valide.'); return { ok: false }; }
    // La fenêtre est celle de l'API (Retry-After), jamais un « quelques
    // minutes » inventé : elle dure un quart d'heure.
    if (r.status === 429) { toast(throttledMessage(r)); return { ok: false }; }
    if (!r.ok) { toast('Impossible d’envoyer le lien pour l’instant.'); return { ok: false }; }
    // Chemin de développement / pile locale : l'écho boucle la poignée de main
    // sans boîte aux lettres, exactement comme pour le notaire.
    if (j.devToken) return clientLinkVerify(j.devToken);
    toast('Si des demandes existent pour ' + courriel + ', un lien vient d’être envoyé. Il est valide 15 minutes.');
    return { ok: true, pending: true };
  }

  // Étape 2. Le serveur rend les offres ET un jeton frais par offre. On FUSIONNE
  // plutôt qu'on écrase : le jeton du serveur gagne (il est neuf), mais tout ce
  // que l'appareil savait en plus — « retenue », l'étude, un montant renégocié —
  // survit. Un remplacement pur ferait régresser un appareil déjà à jour.
  async function clientLinkVerify(token) {
    var r;
    try {
      r = await fetch(API_BASE + '/client/session/verify', {
        method: 'POST', headers: { 'content-type': 'application/json' },
    ...
    var j = {}; try { j = await r.json(); } catch (e) {}
    if (!r.ok) {
      // Un lien mort ne DÉTRUIT rien : l'appareil garde ce qu'il avait.
      toast(r.status === 401
  ...
  function clientApplySession(j) {
    ...
    offres.forEach(function (o) {
      if (!o || !o.id) return;
      known[o.id] = Object.assign({}, known[o.id] || {}, o);
    });
    var merged = Object.keys(known).map(function (k) { return known[k]; });
    merged.sort(function (a, b) { return String(a.dateISO).localeCompare(String(b.dateISO)); });
    lsSave(LS_MYOFFERS, merged.slice(-50));

    // L'adresse vient d'être PROUVÉE : elle devient l'identité de l'appareil.
    if (j.courriel) profileSet({ courriel: j.courriel });
    renderAccountMenu();
    computeNotifications();
    setTab('profil', { focus: false });
    if (state.tab === 'profil') renderProfil();
  ...
  // `#cauth=<jeton>` — le lien du courriel. Le jeton est retiré de l'URL AVANT
  // le réseau : une barre d'adresse se copie, se partage et entre dans
  // l'historique. Même hygiène que `#nauth=` et `#offre=…&cle=`.
  function clientConsumeEspaceHash() {
    var params;
    try { params = new URLSearchParams(String(location.hash || '').replace(/^#/, '')); } catch (e) { return false; }
    var token = params.get('cauth');
    if (!token) return false;
    params.delete('cauth');
    var rest = params.toString();
    try { history.replaceState(null, '', location.pathname + location.search + (rest ? '#' + rest : '')); } catch (e) {}
    clientLinkVerify(token);
    return true;
  }
  ...
  function openOfferFlow() {
    ...
    openDay(state.selectedDate || firstStandardDate(), { offer: true });
  }
  ...
  function onbOpen() {
    onbCount('shown');
    var known = roleGet();
    if (known === 'partner') { partnerStart(); return; }
  ...
  // After the product explanation, coach the client in the actual carnet.
  function onbComplete(explore) {
    ...
    if (role === 'notary') {
      ...
      setTab('carnet');
      clientWalkStart(true);
    }
  }

  // Optional, contextual coaching. The real form owns its values, validation
  // and publication; this guide only points to controls or invokes Continue.
  // Put the card INSIDE an open dialog so native modal inertness never traps it.
  var clientWalk = null;
  var LS_CLIENT_WALK = 'nota.clientWalkthrough.v1';
  var CLIENT_WALK_STEPS = [
    ['Votre date de signature', 'Prenons un refinancement comme exemple. Cliquez sur la date qui vous convient, même si elle contient déjà des offres.'],
    ['Votre acte', 'Pour notre exemple, choisissez « Refinancement ». Vous pouvez aussi choisir l’acte dont vous avez besoin.'],
    ['Votre situation', 'Répondez aux questions avec les renseignements de votre dossier. Le notaire pourra ainsi évaluer votre demande.'],
    ['Votre prix', 'Vérifiez votre offre, le prix du service Nota et le total. Vous pouvez ajuster votre offre avant de continuer.'],
    ['Vos coordonnées', 'Indiquez le lieu de signature et vos coordonnées pour qu’un notaire puisse vous joindre.'],
    ['À vous de publier', 'Vérifiez votre demande, puis cliquez sur « Publier mon offre » lorsque vous êtes prêt. Un notaire devra la retenir pour confirmer la suite.']
  ];
  function clientWalkUnmark() {
    if (!clientWalk || !clientWalk.target) return;
    var target = clientWalk.target;
    target.classList.remove('client-walk-target');
    var ids = (target.getAttribute('aria-describedby') || '').split(/\s+/).filter(function (id) { return id && id !== 'client-walk-copy'; });
    if (ids.length) target.setAttribute('aria-describedby', ids.join(' '));
    else target.removeAttribute('aria-describedby');
    clientWalk.target = null;
  }
  function clientWalkStop(restoreFocus) {
    if (!clientWalk) return;
    var walk = clientWalk, restore = restoreFocus === true || walk.card.contains(document.activeElement) ? walk.target : null;
    clientWalkUnmark();
    walk.observer.disconnect();
    document.removeEventListener('keydown', clientWalkKey, true);
    walk.card.remove();
    clientWalk = null;
    flagSet(LS_CLIENT_WALK, '1');
    flagSet(LS_INTRO, '1');
    if (restore && restore.isConnected) clientWalkFocus(restore);
  }
  function clientWalkKey(event) {
    if (event.key !== 'Escape' || !clientWalk) return;
    // Nested selects retain their own Escape behavior. Otherwise Escape skips
    // only the guide, keeping the client's entered answers in the open form.
    if ($('day-dialog').open && document.querySelector('#day-dialog .nselect-btn[aria-expanded="true"]')) return;
    event.preventDefault();
    event.stopImmediatePropagation();
    clientWalkStop();
  }
  function clientWalkPoint() {
    if (!clientWalk || !clientWalk.target) return;
    var target = clientWalk.target;
    if (target.scrollIntoView) target.scrollIntoView({ block: $('day-dialog').open ? 'center' : 'start', behavior: 'instant' });
    clientWalkFocus(target);
  }
  function clientWalkFocus(target) {
    var control = target.matches('button, input, [tabindex]') ? target
      : target.querySelector('button:not([disabled]), input:not([type="hidden"]):not([disabled])');
    if (control && control.matches('#cal-grid .cal-cell')) {
      // Keyboard arrows use the ISO date in state, not document.activeElement.
      // Keep that anchor and the grid's single tab stop with the shown date.
      state.focusDate = control.dataset.date;
      Array.prototype.forEach.call($('cal-grid').querySelectorAll('.cal-cell'), function (cell) {
        cell.tabIndex = cell === control ? 0 : -1;
      });
    }
    if (control && control.focus) control.focus({ preventScroll: true });
  }
  function clientWalkSync() {
    if (!clientWalk) return;
    if (state.tab !== 'carnet') { clientWalkStop(); return; }
    var dlg = $('day-dialog'), form = $('offer-form');
    if (form && form.dataset.at === 'done') { clientWalkStop(); return; }
    var open = !!(dlg && dlg.open), preview = open && dlg.dataset.interaction === 'preview';
    var step = !open || preview ? 0 : bookAt;
    if (step === 4 && !bookScreenDue(4)) step = 5;
    var target = step === 0 ? (preview ? document.querySelector('#day-preview > button')
      : document.querySelector('#cal-grid .cal-cell[data-date="' + firstStandardDate() + '"]')
        || document.querySelector('#cal-grid .cal-cell:not(.is-out):not(.is-past)'))
      : step === 1 ? $('o-service-chips') : step === 2 ? $('o-criteria-step')
      : step === 3 ? document.querySelector('.book-step-offer') : step === 4 ? $('identity-rows') : $('offer-submit');
    var card = clientWalk.card, host = open ? dlg : document.body;
    if (card.parentNode !== host) {
      if (open) dlg.insertBefore(card, form);
      else host.appendChild(card);
    }
    if (clientWalk.target !== target) {
      clientWalkUnmark();
      clientWalk.target = target;
      if (target) {
        target.classList.add('client-walk-target');
        target.setAttribute('aria-describedby', ((target.getAttribute('aria-describedby') || '') + ' client-walk-copy').trim());
      }
    }
    var key = String(step) + (preview ? '-preview' : '');
    if (card.dataset.step === key) return;
    card.dataset.step = key;
    clientWalk.step = step;
    $('client-walk-progress').textContent = T('Guide interactif') + ' · ' + (step + 1) + ' / ' + CLIENT_WALK_STEPS.length;
    $('client-walk-title').textContent = T(CLIENT_WALK_STEPS[step][0]);
    $('client-walk-copy').textContent = T(preview
      ? 'Touchez « Préparer mon offre » pour ajouter votre projet et votre prix à cette date.'
      : CLIENT_WALK_STEPS[step][1]);
    $('client-walk-next').textContent = T(step === 5 ? 'Terminer le guide' : step === 0
      ? (preview ? 'Préparer mon offre' : 'Me montrer') : step === 4 ? 'Me montrer' : 'Continuer');
  }
  function clientWalkStart(replay) {
    if (clientWalk || state.tab !== 'carnet' || (!replay && flagGet(LS_CLIENT_WALK))) return;
    // The product explanation is already complete. A reload must not replace
    // this optional guide with the older first-visit product dialog.
    onbMarkSeen();
    roleSet('client');
    var card = el('aside', 'client-walk'); card.id = 'client-walkthrough';
    card.setAttribute('aria-labelledby', 'client-walk-title');
    var top = el('div', 'client-walk-top');
    var progress = el('span', 'client-walk-progress'); progress.id = 'client-walk-progress';
    var skip = el('button', 'client-walk-skip', T('Passer')); skip.type = 'button'; skip.id = 'client-walk-skip';
    skip.addEventListener('click', function () { clientWalkStop(true); });
    top.appendChild(progress); top.appendChild(skip); card.appendChild(top);
    var live = el('div'); live.setAttribute('aria-live', 'polite'); live.setAttribute('aria-atomic', 'true');
    var title = el('h2'); title.id = 'client-walk-title';
    var copy = el('p'); copy.id = 'client-walk-copy';
    live.appendChild(title); live.appendChild(copy); card.appendChild(live);
    var next = el('button', 'btn btn-primary'); next.type = 'button'; next.id = 'client-walk-next';
    next.addEventListener('click', function () {
      if (!clientWalk) return;
      var step = clientWalk.step;
      if (step === 5) { clientWalkStop(true); return; }
      if (step === 0 && $('day-dialog').open) { clientWalk.target.click(); return; }
      if (step === 0 || step === 4) { clientWalkPoint(); return; }
      bookForward();
      clientWalkSync();
    });
    card.appendChild(next);
    var observer = new MutationObserver(clientWalkSync);
    clientWalk = { card: card, observer: observer, target: null, step: 0 };
    observer.observe($('day-dialog'), { attributes: true, attributeFilter: ['open', 'data-interaction'] });
    observer.observe($('offer-form'), { attributes: true, attributeFilter: ['data-at'] });
    observer.observe($('offer-submit'), { attributes: true, attributeFilter: ['disabled'] });
    observer.observe($('cal-grid'), { childList: true });
    document.addEventListener('keydown', clientWalkKey, true);
    clientWalkSync();
    clientWalkPoint();
  }
  ...
  function clientSignOut() {
    ...
    // et le courriel du notaire retenu, le CORPS des messages, l'état de la
    // caution et les chiffres d'annulation ; `LS_DOCITEMS` nomme les documents.
    // Sur un poste partagé, les laisser derrière faisait mentir le mot
    // « déconnexion ».
    //
    // Ne SONT PAS touchées : la session notaire (elle a sa propre déconnexion)
    // et `nota.partner.v1` (une identité distincte, avec sa reprise par
    // courriel) — les effacer surprendrait quelqu'un qui n'a rien demandé.
    try {
      [
        LS_PROFILE, LS_MYOFFERS, LS_DOSSIER, LS_NOTIF,
        LS_OFFERSTATUS, LS_SEEN, LS_DOCITEMS, LS_CHECKOUT, LS_SUPPORT, LS_ROLE,
      ].forEach(function (k) { if (k) localStorage.removeItem(k); });
    } catch (e) {}
    toggleNotifPanel(false);
    renderAccountMenu();
    if (state.tab === 'profil') renderProfil();
  ...
  function renderAccountMenu() {
    ...
    // Signing in or out opens and closes the gated doors in the same breath.
    syncGatedDoors();
    if (role === 'anon') return;
    var p = profileGet();
  ...
  // Le journal d'avis que l'API tient POUR LE CLIENT. `notifIn` l'écrit à
  // chaque événement d'audience « client » ; `GET /notifications?id=…` le sert
  // contre le jeton de l'offre. Rien ne le lisait : la cloche se dérivait
  // localement, donc un cache vidé effaçait l'historique et un document déposé
  // par le notaire ne sonnait rien. Meilleur effort, comme la dérive locale :
  // un échec laisse la cloche telle quelle.
  async function clientSyncNotifs() {
    var offers = myOffers().filter(function (o) { return o && o.id && o.clientToken; });
    for (var i = 0; i < offers.length; i++) {
      var o = offers[i];
      var r;
      try {
        r = await fetch(API_BASE + '/notifications?id=' + encodeURIComponent(o.id), { headers: clientHeaders(o) });
      } catch (e) { continue; }
      if (!r || !r.ok) continue;
      var j = {}; try { j = await r.json(); } catch (e) { continue; }
      // Le serveur rend le plus récent d'abord ; addNotif empile, donc on
      // remonte la liste pour retrouver l'ordre à l'écran. Idempotent par clé.
      (j.avis || []).slice().reverse().forEach(function (a) {
        if (!a || !a.id) return;
        // Retention also has a local fallback (the public month feed and the
  ...
  function renderCalendar() {
      ...
      if (iso < today) {
        ...
        b.addEventListener('click', function () {
          ...
          reloadAndRender().then(function () {
            openDay(iso, { preview: compactCalendarInteraction() });
          });
        });
      }
      week.appendChild(b);
    ...
    function liveCell(iso) {
      ...
      if (!isPast) cell.addEventListener('click', function () {
        openDay(this.dataset.date, { preview: compactCalendarInteraction() });
      });
  ...
  function pulseRow(s, active, busiest, showRepere) {
    ...
    var hasRepere = showRepere && s.median != null && s.total >= PULSE_REPERE_MIN;
    row.setAttribute('aria-label',
      short + ', à partir de ' + D.money(prixAnnonceDollars(s.id))
    ...
    dep.appendChild(el('span', 'pulse-fig-v', D.money(prixAnnonceDollars(s.id))));
    figs.appendChild(dep);

    // Month-level, never row-level: a column on two rows and not the other two
    // would slide the amounts out of line. It goes only when the month holds no
    // offer at all — four em dashes under a heading promise a figure that
    // nothing in the month can produce.
    if (showRepere) {
    ...
    // A month with no offer at all gets no track: an empty gauge is a shape
    // that promises data. And a row with no offer in a month that has some gets
    // the track but no fill — the fill's 4px min-width (there so a 1-of-40
    // share stays visible) painted a sliver of market at 0 %.
    if (busiest > 0) {
      var bar = el('span', 'pulse-bar');
      if (s.total > 0) {
        var fill = el('span');
        fill.style.width = Math.round((s.total / busiest) * 100) + '%';
    ...
    // « Réserver une procuration » — the article comes from the catalogue (no
    // rule deduces a gender from a noun), and the name goes through T() like
    // every other composed sentence, so an English reader is not offered a
    // « Book a refinancement ».
    var svcC = D.serviceById(s.id);
    var booking = miniBtn('agenda', 'Réserver ' + ((svcC && svcC.article) || 'un') + ' ' + T(short).toLowerCase(), function () { bookService(s.id); });
    booking.classList.add('mini-reserver');
    item.appendChild(booking);
    return item;
  }
  ...
  function renderPulse() {
    ...
    // One month-level question, asked once for the whole strip: is there a
    // market at all? A month with offers keeps its reference column on every
    // row (« — » where an act has too few offers still reads as « not yet »,
    // because its neighbours carry figures). A month with NONE has nothing for
    // the column to mean — and nothing for a gauge to measure.
    p.services.forEach(function (s) {
      rows.appendChild(pulseRow(s, state.filters.service === s.id, busiest, busiest > 0));
    });
  }
  ...
  // Le premier geste de la boîte du jour — pas sa sortie de secours.
  // showModal() donne le focus au premier élément atteignable, et toute boîte
  // du site commence par la coquille .dlg-x-form : c'était donc le ✕, auquel
  // Chrome peint son anneau. « Fermer » devenait l'élément le plus fort d'une
  // boîte dont toute la question est « quel acte ? », et Entrée y répondait en
  // fermant. Le focus va à l'acte déjà retenu ; l'anneau ne se peint que si la
  // date a été ouverte au clavier, ce qui est exactement quand il sert.
  function focusBookingStart() {
    var dlg = $('day-dialog');
    if (dlg && dlg.dataset.interaction === 'preview') {
      var reserve = dlg.querySelector('#day-preview > button');
      if (reserve) reserve.focus();
      return;
    }
    var wrap = $('o-service-chips');
    if (!wrap) return;
    var t = wrap.querySelector('.chip.is-on') || wrap.querySelector('.chip:not(.chip-soon)');
    if (t) { try { t.focus(); } catch (e) {} }
  }

  // The autonomous controller can add one piece of orientation copy when the
  // aggregate funnel shows that clients are getting stuck. It leaves every
  // question, price and notary control intact, and is safe to hide on rollback.
  function renderExperienceGuidance() {
    var el = $('journey-guidance');
    if (!el) return;
    var guided = state.experience && state.experience.mode === 'guided';
    el.hidden = !guided;
    if (guided) el.textContent = T('Répondez à ce que vous savez; si un point reste à confirmer, le notaire le reprendra avec vous.');
  }
  ...
  // Touch users need a deliberate first step: a date tap explores the day's
  // market, while an explicit reserve action enters the offer form. Desktop
  // keeps the established one-click booking flow. The width fallback matters
  // for mobile emulation, where browsers do not always expose touch points.
  function compactCalendarInteraction() {
    var coarse = false;
    try {
      coarse = !!(window.matchMedia && window.matchMedia('(hover: none) and (pointer: coarse)').matches);
    } catch (e) {}
    return coarse || (window.innerWidth && window.innerWidth <= 680)
      || Number(navigator.maxTouchPoints || 0) > 0;
  }

  function setDayInteractionMode(preview) {
    var dlg = $('day-dialog'), form = $('offer-form'), bids = $('day-bids');
    if (!dlg || !form || !bids) return;
    var market = form.querySelector('.day-market');
    var host = $('day-preview');
    if (preview) {
      if (!host) {
        host = el('section', 'day-preview'); host.id = 'day-preview';
        var intro = el('p', 'day-preview-intro', T('Ajoutez votre projet et votre prix. Vous vérifierez le tout avant de publier.'));
        intro.id = 'day-preview-intro'; host.appendChild(intro);
        var action = el('button', 'btn btn-primary btn-lg', T('Préparer mon offre'));
        action.setAttribute('aria-describedby', intro.id);
        action.type = 'button'; action.addEventListener('click', function () {
          setDayInteractionMode(false);
          bookGoTo(1);
          focusBookingStart();
        });
        host.appendChild(action);
        var offers = el('details', 'day-preview-offers'); offers.id = 'day-preview-offers';
        var summary = el('summary');
        summary.appendChild(el('span', '', T('Offres déjà publiées')));
        var count = el('span', 'day-preview-count'); count.id = 'day-preview-count';
        summary.appendChild(count); offers.appendChild(summary); host.appendChild(offers);
        var empty = el('p', 'day-preview-empty', T('Aucune offre publiée pour cette date.'));
        empty.id = 'day-preview-empty'; host.appendChild(empty);
        dlg.insertBefore(host, form);
      }
      var offersHost = $('day-preview-offers');
      if (bids.parentNode !== offersHost) offersHost.appendChild(bids);
      offersHost.open = false;
      var total = state.monthBids.filter(function (b) { return b.dateISO === state.selectedDate; }).length;
      offersHost.hidden = total === 0;
      $('day-preview-count').textContent = bids.dataset.count || '0';
      $('day-preview-empty').hidden = total > 0;
      // One disclosure is enough: within the preview, show the full visible day.
      var rest = bids.querySelector('.day-bids-rest'); if (rest) rest.hidden = false;
      var toggle = bids.querySelector('.day-bids-toggle'); if (toggle) toggle.hidden = true;
      host.hidden = false;
      form.hidden = true;
      dlg.dataset.interaction = 'preview';
    } else {
      if (market && bids.parentNode !== market) market.appendChild(bids);
      // Restore the compact, act-specific market context in the real form.
      if (host && !host.hidden && state.selectedDate) renderDayBids(state.selectedDate);
      if (host) host.hidden = true;
      form.hidden = false;
      dlg.dataset.interaction = 'offer';
    }
  }

  async function openDay(iso, opts) {
    var preview = opts ? opts.preview === true : compactCalendarInteraction();
    ...
    $('day-sub').textContent = !preview && all.length
    ...
    if (!wasOpen && $('day-date-edit')) $('day-date-edit').open = false;
    if (dlg.showModal && !dlg.open) dlg.showModal();
    setDayInteractionMode(preview);
  ...
  function renderDayBids(iso) {
    ...
    list.dataset.count = String(dayAll.length);
    ...
    // La ligne du chiffre n'est RÉSERVÉE que si le jour en porte un. Elle
    // existe pour qu'une puce ne change pas de hauteur quand on passe d'un
    // acte à l'autre (ADR 0039) — pas pour poser une bande vide dans les deux
    // cartes d'un jour sans offre.
    if (chipWrap) chipWrap.dataset.sub = chipWrap.querySelector('.chip-svc-sub:not(:empty)') ? 'true' : 'false';
  ...
  function updateBeatUI() {
    ...
    // The label is written whether or not the button is on show, so its box
    // holds its place when it steps back (ADR 0039 — une ligne tenue).
    if (t != null) btn.textContent = 'Offrir autant · ' + D.money(t);
  ...
  function writeHash(opts) {
    ...
    if (opts && opts.push) {
      ...
      navHistory.push({ tab: state.tab, url: url });
      navHistoryIndex = navHistory.length - 1;
      history.pushState({ nota: true, notaIndex: navHistoryIndex, pane: state.tab }, '', url);
    } else {
      history.replaceState(Object.assign({}, history.state || {}, {
        nota: true, notaIndex: navHistoryIndex, pane: state.tab,
      }), '', url);
    }
  }
  ...
  function setGroupActive(group, btn) {
    group.querySelectorAll('.chip, .seg-btn').forEach(function (x) {
      // Un bouton désactivé (un acte annoncé) n'est pas un choix du groupe : il
      // ne prend ni « is-on » ni aria-pressed.
  ...
  function populateServiceSelects() {
    ['o-service', 'd-service'].forEach(function (id) {
      ...
      D.SERVICES.forEach(function (s) {
        ...
        o.value = s.id; o.textContent = s.nom + ', à partir de ' + D.money(prixAnnonceDollars(s.id));
        sel.appendChild(o);
      });
    });
    buildDossierChips();
  ...
  // La pastille du glyphe, la même pour un acte en vente et pour un acte
  // annoncé : c'est elle qui donne aux quatre cartes leur gabarit commun.
  function bookingChipBadge(id) {
    var badge = el('span', 'chip-svc-badge');
    var ic = svcIcon(id, 20); if (ic) badge.appendChild(ic);
    return badge;
  }
  function buildBookingChips() {
    ...
    D.SERVICES.forEach(function (s) {
      ...
      // Le glyphe monte dans une pastille : à l'écran 1 la carte est la
      // réponse à la seule question posée, alors elle se vise de loin — une
      // marque, un nom, et le chiffre du jour dessous.
      b.appendChild(bookingChipBadge(s.id));
      b.appendChild(el('span', 'chip-svc-main', s.nomCourt));
      // La marque du choix, au coin de la carte : la teinte et le cadre disent
      // déjà l'acte retenu, le crochet le dit sans dépendre de la couleur.
      var check = el('span', 'chip-svc-check', '✓'); check.setAttribute('aria-hidden', 'true');
      b.appendChild(check);
    ...
    // Le catalogue annoncé, à la même place et dans le même registre, mais en
    // gris et sans porte : le client voit où va Nota sans jamais croire qu'il
    // peut réserver l'acte aujourd'hui. `aria-disabled` plutôt que `disabled` —
    // la carte reste atteignable au clavier et lisible par un lecteur d'écran,
    // et le clic est refusé une seule fois, dans le gestionnaire du groupe.
    (D.ACTES_A_VENIR || []).forEach(function (a) {
      // Ce n'est pas un bouton : rien ne s'y clique, alors rien ne s'y focus et
      // rien ne s'y survole. La carte garde la taille et la place des autres —
      // même ligne, même hauteur — et dit « Bientôt » à côté du nom.
      var b = el('div', 'chip chip-svc chip-soon');
      b.dataset.soon = a.id;
      b.setAttribute('aria-disabled', 'true');
      b.title = a.description;
      b.appendChild(bookingChipBadge(a.id));
      var main = el('span', 'chip-svc-main');
      main.appendChild(document.createTextNode(a.nomCourt));
      b.appendChild(main);
      // La même ligne réservée que les actes réservables, pour que les quatre
      // cartes gardent exactement la même hauteur quand le jour porte un prix.
      // C'est elle qui porte « Bientôt » : sur la ligne du nom, la pastille
      // poussait « Procuration » hors de l'axe de son propre glyphe — deux
      // cartes centrées et deux de travers sur la même grille. Ici le nom
      // reste centré, et `grid-auto-rows: 1fr` garde les hauteurs égales.
      var sub = el('span', 'chip-svc-sub');
      sub.appendChild(el('span', 'chip-soon-tag', 'Bientôt'));
      b.appendChild(sub);
      wrap.appendChild(b);
    });
  }
  ...
  function buildCriterionRow(c, current, onChange, idPrefix, other, svc) {
    ...
    // Une réponse à TROIS choix ou plus ne tient pas dans une demi-largeur : à
    // côté d'un champ d'une seule ligne elle s'y empile en trois, et la paire
    // se lit de travers (retour du propriétaire, 2026-09-06). Le rang le
    // déclare, la grille lui donne la ligne entière, et les choix s'y rangent
    // enfin côte à côte. Le seuil se lit sur la DONNÉE — jamais sur une liste
    // d'ids tenue à la main.
    if (c.type === 'choice' && c.ui !== 'select' && (c.options || []).length >= 3) row.dataset.span = 'row';
    var helpId = idPrefix + c.id + '__help';
    var helpEl = c.aide ? el('span', 'help', c.aide) : null;
    if (helpEl) helpEl.id = helpId;
    // L'aide rejoint le panneau d'un « i » à la toute fin (voir plus bas) ; ce
    // porteur laisse les branches — l'échange de la liste, notamment — parler
    // au « i » sans connaître l'ordre de construction.
    var tipRef = {};
    ...
    if (c.type === 'flag') {
      ...
      var depHead = critHead(depLbl);
      row.appendChild(depHead);
      ...
      var farNote = infoTip({ tone: 'warn', cls: 'crit-caveat' });
      farNote.dataset.caveat = 'notaire';
      var farText = function () {
        return [el('span', null, 'Peu de notaires se déplacent jusqu’à'),
          document.createTextNode(' ' + farKm + ' km. '),
          el('span', null, 'Votre offre ne sera visible que pour eux.')];
      };
      var onlineNote = infoTip({ tone: 'warn', cls: 'crit-caveat' });
      onlineNote.dataset.caveat = 'urgence';
      var onlineText = function () {
        return [el('span', null, 'La signature 100 % en ligne n’est offerte que par les notaires qui l’acceptent.'),
          document.createTextNode(' '),
          el('span', null, 'Votre offre ne sera visible que pour eux.')];
      };
      depHead.appendChild(farNote);
      depHead.appendChild(onlineNote);
      ...
      var paintDep = function () {
        ...
        kmRow.hidden = !quiId;
        kmLbl.hidden = !quiId;
        farNote.setNote(quiId === NOTAIRE_QUI_ID ? farText() : '');
        onlineNote.setNote(q && q.urgence ? onlineText() : '');
        if (!quiId) return;
        var qq = quiOf(quiId);
      ...
      var selHead = critHead(selLbl);
      row.appendChild(selHead);
      ...
      if ((c.options || []).some(function (o) { return o.aide; })) {
        // What the CHOSEN option implies — a sentence that comes and goes with
        // the answer, so it rides the question's line, not the card's height.
        var selOptHelp = infoTip({ id: idPrefix + c.id + '__opt_help' });
        var paintSelHelp = function () {
          var o = (c.options || []).filter(function (x) { return x.id === sel.value; })[0];
          selOptHelp.setNote(o && o.aide ? o.aide : '');
        };
        paintSelHelp();
        sel.addEventListener('change', paintSelHelp);
        selHead.appendChild(selOptHelp);
      }
      ...
      if (c.autre) {
        ...
        // Le libellé du compagnon n'a plus sa ligne à lui : il nomme le champ
        // pour un lecteur d'écran, et le champ le dit en clair par son
        // placeholder. Sans ça, choisir « Autre prêteur » dans la liste
        // faisait grandir le bloc de 101 px — un composant qui bouge parce
        // qu'on y a répondu (retour du propriétaire, 2026-09-05).
        var olbl = el('label', 'crit-label visually-hidden', c.autre.label);
        ...
        oinp.placeholder = c.autre.label;
        ...
        if (c.autre.aide) {
          // Lue, jamais imprimée : le placeholder dit déjà quoi écrire, et une
          // ligne de plus ici rendrait l'échange inégal — répondre à la
          // question déplacerait ce qui suit. `visually-hidden` reste dans
          // l'arbre d'accessibilité (contrairement à visibility:hidden), donc
          // le champ garde sa description.
          var oh = el('span', 'help visually-hidden', c.autre.aide); oh.id = idPrefix + c.autre.champ + '__help';
          oinp.setAttribute('aria-describedby', oh.id);
          obox.appendChild(oh);
        }
        var syncOther = function () {
          var on = sel.value === c.autre.option;
          obox.hidden = !on;
          // L'échange, pas l'empilement : l'aide cède sa place au champ — et son
          // « i » se retire avec elle, sinon il ouvrirait un panneau vide.
          if (helpEl) helpEl.hidden = on;
          if (tipRef.tip) tipRef.tip.dataset.on = on ? 'false' : 'true';
        };
        syncOther();
      ...
      var chHead = critHead(chLbl);
      row.appendChild(chHead);
      ...
      var optHelp = infoTip({ id: idPrefix + c.id + '__opt_help' });
      var paintOptHelp = function (id) {
        var o = (c.options || []).filter(function (x) { return x.id === id; })[0];
        optHelp.setNote(o && o.aide ? o.aide : '');
      };
      if ((c.options || []).some(function (o) { return o.aide; })) {
    ...
    // L'AIDE PASSE DERRIÈRE UN « i » (2026-09-05, retour du propriétaire :
    // « keep it really straight forward — (i) for more information with info on
    // mouse hover etc, keep the same pattern »). Trois à cinq lignes de prose
    // sous chaque question doublaient la hauteur de la carte et noyaient les
    // réponses ; deux questions côte à côte ne tenaient plus dans un écran.
    // Le texte ne disparaît pas — il rejoint le panneau du MÊME composant que
    // les notes de situation (infoTip, ADR 0039) : survol, focus clavier ou
    // clic. Le contrôle continue de le nommer en `aria-describedby` : un nœud
    // RÉFÉRENCÉ est lu par un lecteur d'écran même quand il n'est pas affiché,
    // donc la description ne se perd pas en route.
    var tipHost = row.querySelector(':scope > .crit-head')
      || row.querySelector(':scope > .crit-flag > .crit-text');
    if (helpEl && tipHost) {
      var tip = infoTip({ id: idPrefix + c.id + '__tip' });
      tip.setNote(helpEl);
      // Contre le libellé, JAMAIS après la ligne réservée du message d'attente :
      // celle-ci occupe toute la largeur, et un « i » posé derrière elle tombait
      // sur une ligne à lui, entre la question et sa réponse.
      var req = tipHost.querySelector(':scope > .crit-req');
      if (req) tipHost.insertBefore(tip, req); else tipHost.appendChild(tip);
      tipRef.tip = tip;
      // Dans une case à cocher, le « i » vit DANS le <label> : sans ce garde,
      // ouvrir l'aide cocherait la case.
      if (tipHost.classList.contains('crit-text')) {
        tip.addEventListener('click', function (e) { e.preventDefault(); e.stopPropagation(); });
      }
    }
    // La CONSÉQUENCE de la réponse, sous la réponse (2026-09-05). Une question
    // ne fait pas que bouger le prix : elle décide de ce que le notaire
    // demandera. La ligne est réservée dès la première peinture et masquée par
    // `visibility` (registre ADR 0039) — répondre, se raviser, répondre à
    // nouveau ne déplace jamais la question suivante.
    var effet = critEffet(svc, c);
    if (effet) { row.appendChild(effet.node); effet.paint(current); row.critEffet = effet; }
    return row;
  }

  // La ligne « Ajoute / Retire un document : … » d'une question, ou null quand
  // AUCUNE réponse possible ne change la liste (la plupart des questions).
  // Ce que la réponse change vient du domaine (documentEffect) ; l'écran ne lit
  // jamais les prédicats des documents lui-même.
  function critEffet(svc, c) {
    if (!svc || !c) return null;
    var answers = c.type === 'choice' ? (c.options || []).map(function (o) { return o.id; })
      : c.type === 'flag' ? [true] : [];
    var moves = function (a) {
      var e = D.documentEffect(svc, c.id, a);
      return e.ajoutes.length || e.retires.length ? e : null;
    };
    if (!answers.some(moves)) return null;
    var node = el('span', 'crit-effet');
    node.dataset.on = 'false';
    return {
      node: node,
      paint: function (value) {
        var e = value == null ? null : moves(value);
        clear(node);
        node.dataset.on = e ? 'true' : 'false';
        if (!e) return;
        // Deux nœuds, jamais une phrase : le cadre est une entrée du
        // dictionnaire et chaque nom de document en est une autre (ils viennent
        // du domaine) — un changement de langue en vol les traduit tous les
        // deux, ce qu'une phrase composée ne permettrait pas.
        var docs = e.ajoutes.length ? e.ajoutes : e.retires;
        node.appendChild(el('span', 'crit-effet-k', e.ajoutes.length ? 'Ajoute un document :' : 'Retire un document :'));
        node.appendChild(document.createTextNode(' ' + docs.map(function (d) { return T(d.nom); }).join(' · ')));
      },
    };
  }
  ...
  function renderOfferCriteria(serviceId) {
    ...
    // Answers older than a month (audit 2.11): shown for checking, said on the
    // step's OWN line — the amounts were already cleared by
    // onOfferServiceChange. A tip rather than a banner above the grid: a
    // warning that arrives on a re-render must not push the questions down the
    // dialog while the client is reading them.
    var staleNote = $('o-criteria-stale');
    if (!staleNote && step) {
      staleNote = infoTip({ id: 'o-criteria-stale', tone: 'warn' });
      var stepLbl = step.querySelector('.book-step-lbl');
      var stepCount = $('o-criteria-count');
      if (stepLbl && stepCount && stepCount.parentNode === stepLbl) stepLbl.insertBefore(staleNote, stepCount);
      else if (stepLbl) stepLbl.appendChild(staleNote);
    }
    if (staleNote) staleNote.setNote(state.offer.stale && criteria.length ? 'Vos réponses précédentes — vérifiez-les.' : '');
    var mkRow = function (c) {
      ...
      var row = buildCriterionRow(c, current, function (val) {
        setCriterion(c.id, val);
        if (row.critEffet) row.critEffet.paint(val);
      }, 'crit-',
    ...
    renderCriteriaSections(box, svc, mkRow, { tally: true });
    settleSegTracks(box);
  }

  // ---------------------------------------------------------------------------
  // Les questions du notaire, EN SECTIONS (2026-09-05).
  //
  // Retour du propriétaire : « on peut grouper en sections et rendre ça le plus
  // intuitif possible pour chaque cas ». Dix questions à la file sur une grille
  // plate ne disaient pas ce qui les reliait — le montant du prêt, la
  // succession et le déplacement se suivaient sans transition — et les quatre
  // précisions facultatives tombaient toutes dans un seul tiroir anonyme au bas
  // de l'écran, loin de la question qu'elles précisent.
  //
  // Trois temps, déclarés par le DOMAINE (D.criteriaGroups) : le prêt,
  // l'immeuble, la signature. Chaque section dit à quoi elle sert, où elle en
  // est, et REPLIE SES PROPRES précisions — « Préciser (facultatif) » vit
  // désormais à côté du cas qu'il précise. Le chemin court ne s'allonge pas :
  // ce qui était replié le reste.
  //
  // Un rang qui prend la ligne entière : une réponse à trois choix ou plus, et
  // la bande du déplacement, qui en porte déjà deux.
  function wideRow(r) { return r.dataset.span === 'row' || r.classList.contains('crit-row--wide'); }

  // Le bouton qui replie une section sur son titre. Il n'a pas de mot à lui :
  // le titre juste à côté le nomme (aria-label), et `aria-expanded` dit l'état
  // — un chevron seul, dans la bande, à droite du compte.
  function sectionFoldBtn(bodyId, nom) {
    var b = el('button', 'crit-sec-fold');
    b.type = 'button';
    b.setAttribute('aria-expanded', 'true');
    b.setAttribute('aria-controls', bodyId);
    b.setAttribute('aria-label', nom);
    b.appendChild(strokeGlyph('<path d="m6 9 6 6 6-6"/>', 14, 'crit-sec-chev'));
    b.addEventListener('click', function () {
      var sec = b.closest ? b.closest('.crit-sec') : null;
      if (!sec) return;
      sec.dataset.held = 'true';   // un pli posé à la main se respecte
      setSectionFold(sec, sec.dataset.fold !== 'true');
    });
    return b;
  }

  function setSectionFold(sec, folded) {
    var was = sec.dataset.fold === 'true';
    sec.dataset.fold = folded ? 'true' : 'false';
    var b = sec.querySelector('.crit-sec-fold');
    if (b) b.setAttribute('aria-expanded', folded ? 'false' : 'true');
    // Une piste segmentée n'a de géométrie qu'une fois visible : au dépli, on
    // la remesure, comme le volet « Préciser » le fait déjà.
    if (was && !folded) settleSegTracks(sec);
  }

  // Les sections de l'écran 3 vivent en dur dans index.html (elles ne sortent
  // d'aucun catalogue) : elles NOMMENT leur glyphe, on le leur pose une fois.
  // Même carte, même bande, même lecture que les questions de l'écran 2.
  function paintStaticSectionIcons() {
    Array.prototype.forEach.call(document.querySelectorAll('.crit-sec-h[data-sec-ic]'), function (h) {
      if (h.querySelector('.crit-sec-ic')) return;
      var ic = critGroupIcon(h.dataset.secIc);
      if (ic) h.insertBefore(ic, h.firstChild);
    });
  }

  // La même fonction sert la feuille de réservation et le carnet du dossier, si
  // bien que les deux surfaces ne peuvent pas diverger.
  // ---------------------------------------------------------------------------
  function renderCriteriaSections(box, svc, mkRow, opts) {
    opts = opts || {};
    D.criteriaGroups(svc).forEach(function (g) {
      var sec = el('section', 'crit-sec');
      sec.dataset.groupe = g.id;
      // Nommée pour un lecteur d'écran par le titre qu'on lit à l'œil.
      sec.setAttribute('role', 'group');
      var head = el('div', 'crit-sec-h');
      // Le sujet AVANT son titre : dix questions à la file se lisaient toutes
      // dans la même graisse, et « ceci parle du prêt » demandait de lire pour
      // le savoir. Le glyphe le dit d'un coup d'œil ; il vient du même registre
      // que la pastille des actes, deux cartes plus haut.
      var mark = critGroupIcon(g.id);
      if (mark) head.appendChild(mark);
      var title = el('span', 'crit-sec-t', g.nom);
      ...
      if (opts.tally) {
        var n = el('span', 'crit-sec-n');
        n.setAttribute('aria-live', 'polite');
        head.appendChild(n);
      }
      var grid = el('div', 'crit-sec-grid');
      grid.id = (opts.idPrefix || 'crit-') + 'sec-' + g.id + '__body';
      // Une section complète se replie sur son titre — seulement là où la
      // feuille se REMPLIT dans l'ordre (opts.tally). Le dossier, lui, se
      ...
      // précédait. C'est le DOM qu'on réordonne, jamais un `order` CSS —
      // l'ordre de lecture reste l'ordre du DOM, à l'œil comme au clavier
      // (audit 2.15).
      var rows = g.requis.map(function (c) { return mkRow(c); });
      rows.forEach(function (r) { if (!wideRow(r)) grid.appendChild(r); });
      rows.forEach(function (r) { if (wideRow(r)) grid.appendChild(r); });
      sec.appendChild(head);
      sec.appendChild(grid);
      // Les précisions de CETTE section, repliées avec leur compte : le volet
      // dit ce qu'il cache au lieu de promettre un vague « Affiner ».
      if (g.facultatifs.length) {
        var det = document.createElement('details'); det.className = 'disclosure crit-more';
        var sum = document.createElement('summary');
        sum.appendChild(document.createTextNode('Préciser (facultatif)'));
        sum.appendChild(el('span', 'crit-more-n', String(g.facultatifs.length)));
        det.appendChild(sum);
        var inner = el('div', 'crit-sec-grid');
        g.facultatifs.forEach(function (c) { inner.appendChild(mkRow(c)); });
  ...
  // Le compte de chaque section : les mêmes mots que le compte de l'étape,
  // comptés là où les questions vivent. Appelé par validateOfferUI après que
  // les lignes en attente ont été marquées.
  function paintSectionTallies() {
    Array.prototype.forEach.call(document.querySelectorAll('#o-criteria > .crit-sec'), function (sec) {
      var n = sec.querySelector('.crit-sec-n');
      if (!n) return;
      var due = sec.querySelectorAll('.crit-row.crit-missing').length;
      n.dataset.state = due ? 'due' : 'done';
      // COMPACT à l'œil, ENTIER à l'oreille : l'étape dit déjà la phrase
      // (« 3 réponses attendues ») et la répéter sur chaque section ferait lire
      // deux fois le même compte au même endroit. La section n'en montre que le
      // chiffre — et le porte en entier dans son aria-label, que la passe i18n
      // traduit comme n'importe quel attribut.
      n.textContent = due === 0 ? '✓' : String(due);
      n.setAttribute('aria-label', due === 0 ? '✓ complet' : due === 1 ? '1 réponse attendue' : due + ' réponses attendues');
      // Une section RÉGLÉE SOUS LES YEUX DU CLIENT se replie sur son titre : à
      // l'écran il ne reste que le sujet en cours, et les trois temps se
      // comptent d'un regard au lieu de se dérouler sur deux écrans.
      //
      // Deux garde-fous, chacun payé par un essai raté (2026-09-06) :
      //  · une question ENCORE ATTENDUE ne se cache jamais — la section se
      //    rouvre d'elle-même dès qu'un compte repart, et le pli posé à la
      //    main repart avec lui ;
      //  · une section déjà complète À L'ARRIVÉE reste OUVERTE. Un dossier
      //    déjà rempli repliait les trois sections d'un coup et l'écran 2
      //    s'ouvrait sur trois bandes et zéro question — tout replier, ce
      //    n'est plus ranger, c'est cacher. On ne replie qu'un passage de
      //    « attendue » à « réglée », donc seulement ce que le client vient
      //    lui-même de finir.
      if (due) { sec.dataset.held = 'false'; sec.dataset.wasDue = 'true'; setSectionFold(sec, false); }
      else if (sec.dataset.wasDue === 'true' && sec.dataset.held !== 'true') setSectionFold(sec, true);
    });
  }
  ...
  // LPC art. 224 c) et Loi sur la concurrence art. 74.01 (1.1) : un prix
  // annoncé comprend le total que le client paiera, taxes en sus. Le « à
  // partir de » d'un acte est donc les honoraires de départ PLUS le prix de
  // Nota au palier standard, jamais le plancher des honoraires seul. Lu sur la
  // grille servie par l'API quand elle est là, sur le catalogue sinon.
  function prixAnnonceDollars(serviceId) {
    var t = store.tarif;
    var p = D.prixAnnonce(serviceId, t && t.grille ? t.grille : null);
    return Math.round(p.totalCents) / 100;
  }
  ...
  function renderDevis() {
    ...
    $('devis-nota').textContent = lignes ? D.money(lignes.service) : 'à confirmer';
    ...
    $('devis-total').textContent = prix != null ? D.money(honoraires + prix) : 'à confirmer';
    ...
    // ADR 0041 / art. 12 LPC — ce qu'une annulation peut coûter se dit ICI,
    // avant l'engagement, et non au moment d'annuler : les plafonds par
    // palier et le délai de réclamation. Jamais un montant fixé d'avance.
    devisAnnulationNote(box2, tarif && tarif.annulation);
  }

  // Les plafonds d'indemnité d'annulation, phrase par phrase (chacune a sa
  // règle de traduction) : « 30 % à 3 jours ou moins de la signature »,
  // « 10 % de 4 à 14 jours ».
  function devisAnnulationNote(box, ann) {
    var liste = ann && (Array.isArray(ann.plafonds) ? ann.plafonds : ann.paliers);
    var paliers = Array.isArray(liste)
      ? liste.map(function (p) { return p && { maxJours: p.maxJours, plafond: p.plafond != null ? p.plafond : p.taux }; })
        .filter(function (p) { return p && p.maxJours != null && p.plafond != null; })
        .sort(function (x, y) { return Number(x.maxJours) - Number(y.maxJours); })
      : [];
    if (!paliers.length) return;
    var p = el('p', 'devis-annulation');
    p.appendChild(el('span', '', 'Annulation sans frais tant qu’aucun notaire n’a retenu votre demande.'));
    p.appendChild(document.createTextNode(' '));
    p.appendChild(el('span', '', 'Ensuite, sur justification et dans les ' + (Number(ann.delaiJours) || 7)
      + ' jours, le notaire peut réclamer ses frais réels et la valeur du travail accompli, jusqu’à :'));
    var from = 0;
    paliers.forEach(function (pal, i) {
      var max = Number(pal.maxJours);
      p.appendChild(document.createTextNode(' '));
      p.appendChild(el('span', '', from === 0
        ? pctLabel(pal.plafond) + ' du montant à ' + max + ' jours ou moins de la signature'
        : pctLabel(pal.plafond) + ' du montant de ' + from + ' à ' + max + ' jours'));
      p.appendChild(document.createTextNode(i < paliers.length - 1 ? ',' : '.'));
      from = max + 1;
    });
    p.appendChild(document.createTextNode(' '));
    p.appendChild(el('span', '', 'Rien n’est retenu sans réclamation, et l’indemnité va au notaire, jamais à Nota.'));
  ...
  function validateOfferUI() {
    ...
    if (hint) {
      ...
      // Le rail dit la même chose que le garde, en plus court.
      bookPaint();
      ...
      // Le même état, dit deux fois : sur l'étape, et sur chaque section.
      paintSectionTallies();
  ...
  //
  // Un écran à la fois : 1 l'acte · 2 les réponses · 3 le prix · 4 les
  // coordonnées. La STRUCTURE DU FORMULAIRE NE CHANGE PAS — chaque bloc porte
  // le numéro de son écran (`data-screen`), le formulaire porte celui où l'on
  // est (`data-at`), et c'est le CSS seul qui n'en montre qu'un. Rien n'est
  // détaché du DOM : ce qui a été répondu à l'écran 2 reste répondu, la porte
  // « Répondez à : … » ramène à la bonne question, et le garde de publication
  // continue de juger le formulaire ENTIER, pas l'écran courant.
  // ---------------------------------------------------------------------------
  var BOOK_LAST = 4;
  var bookAt = 1;      // l'écran affiché
  var bookSeen = 1;    // le plus loin qu'on ait atteint — le rail n'ouvre pas au-delà

  function bookForm() { return $('offer-form'); }
  // L'écran 2 n'existe que si l'acte pose des questions ; sinon on l'enjambe
  // dans les deux sens plutôt que de montrer une carte vide.
  function bookSkips(n) {
    if (n !== 2) return false;
    var step = $('o-criteria-step');
    return !step || step.hidden;
  }
  function bookNextUsable(n, dir) {
    while (n >= 1 && n <= BOOK_LAST && bookSkips(n)) n += dir;
    return Math.min(BOOK_LAST, Math.max(1, n));
  }

  function bookGoTo(n, opts) {
    var form = bookForm(); if (!form) return;
    n = bookNextUsable(Math.min(BOOK_LAST, Math.max(1, n || 1)), n > bookAt ? 1 : -1);
    bookAt = n;
    ...
    var head = form.querySelector('[data-screen="' + n + '"] .book-step-lbl');
    if (head && head.focus) { head.tabIndex = -1; try { head.focus({ preventScroll: true }); } catch (e) { head.focus(); } }
  }

  // Le rail et la barre d'action disent la même vérité que le garde : ce qui
  // est fait, où l'on est, et ce qui fait avancer.
  function bookPaint() {
    var rail = $('book-rail');
    if (rail) {
      Array.prototype.forEach.call(rail.querySelectorAll('.book-rail-b'), function (b) {
        var n = Number(b.dataset.goto);
        var li = b.parentNode;
        // « Fait » veut dire fait, pas visité : une étape traversée sans y
        // répondre garde une marque d'attente, sinon le rail dirait vert sur
        // un formulaire qu'on ne peut pas publier.
        li.dataset.state = bookSkips(n) ? 'skip'
          : n === bookAt ? 'at'
          : n > bookSeen ? 'todo'
          : bookScreenDue(n) ? 'open' : 'done';
        // On ne saute pas par-dessus une étape qu'on n'a pas encore atteinte.
        b.disabled = n > bookSeen;
        if (n === bookAt) b.setAttribute('aria-current', 'step'); else b.removeAttribute('aria-current');
      });
    }
    var back = $('book-back'), next = $('book-next'), sub = $('offer-submit');
    if (back) back.hidden = bookAt === 1;
    if (next) next.hidden = bookAt === BOOK_LAST;
    if (sub) sub.hidden = bookAt !== BOOK_LAST;
  }

  // Ce qui manque À CET ÉCRAN, et rien d'autre. « Continuer » ne bloque pas en
  // silence : il marque ce qui est attendu et pose le curseur dessus.
  function bookScreenDue(n) {
    if (n === 1) return !state.offer.serviceId;
    if (n === 2) return !!document.querySelector('#o-criteria .crit-row.crit-missing');
    if (n === 4) { var s = $('offer-submit'); return !!(s && s.disabled); }
    return false; // le prix est toujours pré-rempli
  }
  function bookForward() {
    ...
    state.offer.touched = true;
    validateOfferUI();
    var row = document.querySelector('#o-criteria .crit-row.crit-missing');
    if (row && row.dataset.crit) focusCriterionRow(row.dataset.crit);
    else if (bookAt === 1) {
      var chip = document.querySelector('#o-service-chips .chip');
      if (chip && chip.focus) chip.focus();
    }
  }

  // Une porte (« Répondez à : … », un refus du serveur) doit d'abord ouvrir
  // l'écran où la question habite, sinon elle vise un champ que le CSS cache.
  function bookShow(node) {
    if (!node || !node.closest) return;
    var owner = node.closest('[data-screen]');
    var n = owner ? Number(owner.dataset.screen) : 0;
    if (n && n !== bookAt) {
      if (n > bookSeen) bookSeen = n;
      bookGoTo(n, { silent: true });
    }
  }
  ...
  function markRowMissing(row, on) {
    ...
    var head = row.querySelector(':scope > .crit-head');
    var note = row.querySelector('.crit-req');
    if (!note) {
      // The slot is opened by the FIRST pass that finds the question awaited —
      // which runs while the sheet is being painted — and is never taken back.
      // The message can then arrive and leave without moving a pixel: only its
      // visibility changes, so the question keeps the same height whether it
      // is awaited, answered, or answered then cleared again.
      if (!on) return;
      note = el('span', 'crit-req', 'Réponse requise');
      // En FIN de ligne de libellé : les « i » de la question gardent leur place
      // contre le mot qu'ils expliquent, et le message se range après eux.
      if (head) head.appendChild(note);
      else row.appendChild(note);
    }
    // visibility, not `hidden`: a reserved line that is neither read aloud nor
    // reachable, and a row whose height never depends on the answer.
    note.dataset.on = on && state.offer.touched ? 'true' : 'false';
  }
  ...
  function renderApprobationNote() {
    ...
    if (!note) {
      note = infoTip({ id: 'o-approbation-note', tone: 'warn' });
      var head = row.querySelector(':scope > .crit-head') || row;
    ...
    note.setNote(show ? 'Sans les instructions du prêteur en main, une signature dans moins de deux semaines est rarement tenable. Choisissez une date plus éloignée, ou confirmez l’approbation avant de publier.' : '');
  }
  ...
  function buildMyOffersCard() {
    ...
    if (!offers.length) {
      ...
      empty.appendChild(el('p', 'profil-empty-text', 'Vous n’avez pas encore publié d’offre. Choisissez une date au carnet pour envoyer votre première demande.'));
  ...
  function evaluationBlock(o, status) {
    ...
    if (status.evaluation) {
      ...
      done.appendChild(el('span', 'help', ' Merci — elle est transmise à votre notaire. Elle n’est publiée nulle part.'));
  ...
  async function answerProposition(o, p, verb, block) {
    ...
    if (verb === 'accept') {
      ...
      addRetainedNotif(o, Object.assign({}, bid, { etude: bid.etude || p.etude }));
  ...
  async function openCancelDialog(o, st) {
    ...
    if (feeEl) {
      if (prev && Number(prev.plafond) > 0) {
        feeEl.hidden = false;
        // ADR 0041 — RIEN n'est prélevé à l'annulation. Le client lit ce que
        // le notaire POURRA réclamer (frais réels et travail accompli,
        // art. 2129 C.c.Q.), jusqu'à quel plafond, dans quel délai, et ce qu'il
        // advient de la somme réservée entre-temps. Chaque phrase est son
        // propre nœud : la couche i18n traduit par phrase.
        var surCautionNow = !!(status && status.caution && status.caution.etat === 'posee');
        clear(feeEl);
        feeEl.appendChild(el('span', '', 'Annuler maintenant permet au notaire de réclamer, sur justification et dans les '
          + (Number(prev.delaiJours) || 7) + ' jours, ses frais réels et la valeur du travail accompli, jusqu’à '
          + D.money(prev.plafond) + ' (' + pctLabel(prev.taux) + ' du montant convenu).'));
        feeEl.appendChild(document.createTextNode(' '));
        feeEl.appendChild(el('span', '', 'Rien n’est retenu automatiquement.'));
        feeEl.appendChild(document.createTextNode(' '));
        feeEl.appendChild(el('span', '', surCautionNow
        ...
        feeEl.appendChild(document.createTextNode(' '));
        feeEl.appendChild(el('span', '', 'Une indemnité réclamée est versée au notaire en dédommagement de la journée réservée, jamais à Nota.'));
      } else if (false) {
  ...
  var contactMessageStarted = false;
  var contactSubjectSyncing = false;
  function openContactDialog(context) {
    track('contact_ouvert');
    var input = $('chat-text'), offer = context && context.offer;
    // Offer help stays in the same inbox. Never send or overwrite a draft.
    if (offer && input && !input.value.trim()) {
      input.value = T('Aide avec une offre') + ' — ' + offer.id + '\n';
      chatAutoGrow(input); chatCount();
    }
    chatToggle(true);
  }
  // Kept for existing contact integrations; the site's contact doors open chat.
  function openEmailContactDialog(context) {
    var o = context && context.offer;
    contactBidId = o ? o.id : null;
    contactMessageStarted = false;
    track('contact_ouvert');
    var p = profileGet();
    ...
    contactSubjectSyncing = true;
    $('ct-sujet').dispatchEvent(new Event('change', { bubbles: true }));
    contactSubjectSyncing = false;
    var ctx = $('ct-context');
    ctx.hidden = !o;
  ...
  async function submitContact(e) {
    ...
    track('contact_soumis');
    if (!v.ok) {
      track('contact_echec');
      var emailInvalid = !D.isEmail(input.courriel);
      var messageInvalid = !String(input.message || '').trim();
      $('ct-courriel').setAttribute('aria-invalid', emailInvalid ? 'true' : 'false');
      $('ct-message').setAttribute('aria-invalid', messageInvalid ? 'true' : 'false');
      clear(eb); eb.hidden = false;
      v.errors.forEach(function (er) { eb.appendChild(el('li', null, er.message)); });
      try { (emailInvalid ? $('ct-courriel') : $('ct-message')).focus(); } catch (e5) {}
      return;
    }
    $('ct-courriel').setAttribute('aria-invalid', 'false');
    $('ct-message').setAttribute('aria-invalid', 'false');
    eb.hidden = true;
    var btn = $('ct-submit');
    btn.disabled = true; btn.setAttribute('aria-busy', 'true'); btn.textContent = T('Envoi…');
    var ok = false;
    try {
    ...
    btn.disabled = false; btn.removeAttribute('aria-busy'); btn.textContent = T('Envoyer');
    if (!ok) {
      track('contact_echec');
      toast('Impossible d’envoyer pour le moment. Réessayez, ou écrivez-nous par courriel.');
      return;
    }
    track('contact_envoye');
  ...
  function buildReferralCard() {
    var card = el('div', 'profil-card profil-referral'); card.id = 'profil-referral';
    var rec = partnerGet();
    card.appendChild(profilHead(IC_PARR, 'Parrainage', 'Un lien à partager. Une récompense quand la référence aboutit.'));
    var rewards = el('div', 'profil-rewards');
    [
      { amount: D.REFERRAL.client, title: 'Client référé', detail: 'Quand sa demande est retenue par un notaire.' },
      { amount: D.REFERRAL.notaire, title: 'Notaire référé', detail: 'Une seule fois, à son premier acte retenu.' },
    ].forEach(function (reward) {
      var item = el('div', 'profil-reward');
      item.appendChild(el('span', 'profil-reward-label', reward.title));
      item.appendChild(el('strong', 'profil-reward-amount', D.money(reward.amount)));
      item.appendChild(el('p', 'help', reward.detail)); rewards.appendChild(item);
    });
    card.appendChild(rewards);
    card.appendChild(el('p', 'help', 'Le versement suit la signature de l’acte. Une demande annulée avant la signature ne donne lieu à aucun versement.'));
    if (!rec) {
      var empty = el('div', 'profil-empty');
      var copy = el('div', 'profil-empty-copy');
      copy.appendChild(el('p', 'profil-empty-text', 'Votre prochain bon contact commence ici.'));
      copy.appendChild(el('p', 'help', 'Choisissez votre code, confirmez votre courriel, puis partagez votre lien.'));
      empty.appendChild(copy);
      var cta = el('button', 'btn btn-primary btn-sm', 'Devenir partenaire'); cta.type = 'button';
      cta.addEventListener('click', function () {
        toggleNotifPanel(false); setTab('partenaires');
        var form = $('partner-details'); if (form) form.open = true;
        var email = $('partner-courriel'); if (email) email.focus();
      });
      empty.appendChild(cta);
      card.appendChild(empty);
      card.appendChild(el('p', 'help', 'Programme réservé aux professionnels qui ne sont pas notaires.'));
      return card;
    }
    var codeRow = el('div', 'parr-code-row');
    codeRow.appendChild(el('span', 'parr-code-lbl', 'Votre code'));
    var codeValue = el('strong', 'parr-code', rec.code); codeValue.setAttribute('data-i18n-skip', ''); codeRow.appendChild(codeValue);
    codeRow.appendChild(el('span', 'profil-status', 'Code actif'));
    card.appendChild(codeRow);
    var linkRow = el('div', 'partner-link-row');
    var link = el('code', null, partnerShareLink(rec.code));
    link.setAttribute('data-i18n-skip', ''); link.tabIndex = 0;
    linkRow.appendChild(link);
    var copy = el('button', 'btn btn-primary btn-sm', 'Copier le lien'); copy.type = 'button';
    copy.addEventListener('click', function () { copyLinkText(link.textContent, function () { flashCopied(copy); }); });
    ...
    var message = el('details', 'profil-message');
    message.appendChild(el('summary', null, 'Un message prêt à envoyer'));
    var messageText = el('p', 'partner-msg-text', partnerMessageText(rec.code)); messageText.setAttribute('data-i18n-skip', '');
    message.appendChild(messageText);
    var copyMessage = el('button', 'btn btn-sm', 'Copier le message'); copyMessage.type = 'button';
    copyMessage.addEventListener('click', function () {
      copyText(messageText.textContent, 'Message copié.', 'Copie impossible — sélectionnez le message.', function () { flashCopied(copyMessage); });
    });
    message.appendChild(copyMessage); card.appendChild(message);
    var note = el('div', 'profil-referral-followup');
    note.appendChild(el('p', 'help', 'Vos récompenses et leur suivi vous sont envoyés par courriel.'));
    if (rec.courriel) { var email = el('span', 'profil-referral-email', rec.courriel); email.setAttribute('data-i18n-skip', ''); note.appendChild(email); }
    var program = el('button', 'btn btn-sm btn-ghost', 'Voir le programme'); program.type = 'button';
    program.addEventListener('click', function () { setTab('partenaires'); });
    note.appendChild(program); card.appendChild(note);
    return card;
  }

  function renderProfilNav() {
    var nav = $('profil-nav'); if (!nav) return; clear(nav);
    nav.appendChild(el('span', 'eyebrow', 'Dans votre espace'));
    [
      ['profil-offers', 'Mes offres', IC_OFFERS], ['profil-contact', 'Coordonnées', IC_COORD],
      ['profil-notifications', 'Notifications', IC_NOTIF], ['profil-documents', 'Mes documents', IC_DOCS],
      ['profil-referral', 'Parrainage', IC_PARR],
    ].forEach(function (section) {
      var button = el('button', 'profil-nav-item'); button.type = 'button'; button.dataset.profileTarget = section[0]; button.setAttribute('aria-controls', section[0]);
      var icon = el('span', 'profil-card-ic');
      icon.innerHTML = '<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">' + section[2] + '</svg>';
      button.appendChild(icon); button.appendChild(el('span', null, section[1]));
      button.addEventListener('click', function () {
        var target = $(section[0]); if (!target) return;
  ...
  function renderProfil() {
    ...
    [
      ...
      if (f.key === 'courriel' || f.key === 'prefixe') {
        var feedback = el('p', 'help profil-field-error'); feedback.id = 'p-' + f.key + '-error'; feedback.hidden = true;
        feedback.setAttribute('aria-live', 'polite'); row.appendChild(feedback);
        var description = inp.getAttribute('aria-describedby');
        inp.setAttribute('aria-describedby', (description ? description + ' ' : '') + feedback.id);
        var validate = function () {
          var value = inp.value.trim();
          var error = f.key === 'prefixe' ? (value && !D.isPostalPrefix(value) ? { message: 'Vérifiez votre secteur postal (ex. : G1R).' } : null)
            : (value && !D.isEmail(value) ? { message: 'Entrez un courriel valide.' } : null);
          inp.setAttribute('aria-invalid', error ? 'true' : 'false');
          feedback.textContent = error ? T(error.message) : ''; feedback.hidden = !error;
        };
        inp.addEventListener('blur', validate);
        inp.addEventListener('input', function () { if (inp.getAttribute('aria-invalid') === 'true') validate(); });
      }
      idFields.appendChild(row);
    });
    idCard.appendChild(idFields);
    var savedStatus = el('p', 'profil-save-status help', 'Enregistrement automatique sur cet appareil.');
    savedStatus.id = 'profil-save-status'; savedStatus.setAttribute('role', 'status');
    idFields.addEventListener('input', function () { savedStatus.textContent = T('Modifications enregistrées sur cet appareil.'); });
    idCard.appendChild(savedStatus);
    ...
    PROFILE_NOTIF_KINDS.forEach(function (t) {
      ...
      var txt = el('label', 'switch-title', t.label); txt.htmlFor = cb.id;
    ...
    var emailRow = el('div', 'profil-email-prefs');
    emailRow.appendChild(el('p', 'help', 'Les courriels se règlent séparément des notifications dans l’application.'));
    emailRow.appendChild(emailChoice); nCard.appendChild(emailRow);
    ...
    var dCard = el('div', 'profil-card profil-docs'); dCard.id = 'profil-documents';
    ...
    D.SERVICES.forEach(function (s) {
      var on = s.id === docsPre;
      var c = el('button', 'chip' + (on ? ' is-on' : ''), s.nom);
    ...
    accountTourReplay('client', $('profil-nav') || body);
  }
  ...
  // Replie les réponses du serveur dans le dossier local, sans jamais écraser
  // ce que l'appareil sait de plus. Les clés réservées (__pricing, __validated,
  // __pricingAt) sont de l'état d'appareil : le serveur ne les porte pas et ne
  // doit pas y toucher.
  function hydrateDossier(serveur, sid) {
    if (!sid || !serveur || typeof serveur !== 'object') return;
    var keys = Object.keys(serveur);
    if (!keys.length) return; // vide n'est pas une instruction d'effacement
    var d = dossierState();
    var pour = d[sid] || (d[sid] = {});
    var change = false;
    keys.forEach(function (k) {
      if (k.indexOf('__') === 0) return;      // état d'appareil, jamais du serveur
      if (pour[k] === serveur[k]) return;
      if (pour[k]) return;                    // l'appareil a déjà une réponse : elle gagne
      pour[k] = serveur[k]; change = true;
    });
    if (change) lsSave(LS_DOSSIER, d);
  }
  ...
  function renderDossier() {
    ...
    if (pcrit.length) {
      ...
      // Les mêmes sections que la feuille — une seule fonction les rend, donc
      // les deux surfaces ne peuvent pas poser les questions dans deux ordres.
      renderCriteriaSections(pbox, svc, function (c) {
        var prow = buildCriterionRow(c, pans[c.id], function (val) {
          dossierSetPricing(svc.id, c.id, val);
          updateDossierPrice(svc);
          if (prow.critEffet) prow.critEffet.paint(val);
  ...
  // Free notary signup — the door that does NOT depend on Stripe.
  // /notaries/connect (below) starts a payout account and answers 503 whenever
  // billing is unconfigured, which is the state of production; it was the only
  // caller here, so a new notary could not register at all. /notaries/signup
  // files the application (`status: 'en_attente'`), mails the notary a « we are
  // checking » notice and mails the operator the admin link. Console access is
  // the OPERATOR's approval (`approuveLe`), never Stripe's — so nothing about
  // payouts belongs on this path.
  async function ncFreeSignup(email, onError) {
    email = (email || '').trim();
    if (!email) { onError && onError('Un courriel est requis.'); return false; }
    // Referral attribution (ADR 0011), same rule as everywhere: the typed field
    // wins, the captured ?ref link is the fallback, an invalid code is dropped
    ...
    if (parrain) body.parrain = parrain;
    try {
      var r = await fetch(API_BASE + '/notaries/signup', {
        method: 'POST', headers: { 'content-type': 'application/json' },
        body: JSON.stringify(body),
      });
      var j = {}; try { j = await r.json(); } catch (e) {}
      if (r.ok) { ncShowPendingStep(email); return true; }
      onError && onError((j.errors && j.errors[0] && j.errors[0].message) || 'Inscription indisponible pour le moment.');
    } catch (err) { onError && onError('Hors ligne. Réessayez une fois en ligne.'); }
    return false;
  }
  ...
  function ncShowGateStep(which) {
    if (which !== 'email') {
      var access = $('notary-calendar-access');
    ...
    var pending = $('notary-gate-step-pending'); if (pending) pending.hidden = which !== 'pending';
  }

  // "Your application is filed" — the honest end of the free signup. It does
  // NOT open the console: /notaries/signup files the notary as `en_attente`,
  // and only the operator's approval (`approuveLe`) opens the door. Promising
  // an immediate console here would be the old lie moved one screen along.
  function ncShowPendingStep(email) {
    ncSetErrors([]);
    var who = $('notary-pending-email'); if (who) who.textContent = email;
    ncShowGateStep('pending');
    var box = $('notary-gate-step-pending');
  ...
  var LS_NC_FILTERS = 'nota.notary.filters.v1';
  ...
  // card is on screen.
  var nc = { token: null, feedToken: null, email: null, open: [], filter: { service: 'all', readyOnly: false, day: null }, openDetails: {}, retainedDetails: {}, rating: null, profil: { lienCNQ: null }, tarif: null, cote: null, conditions: null, fenetre: null, deepAct: null, manquantsServeur: null, aiAccess: null };

  // --- AI preparation access -------------------------------------------------
  // The dossier checklist is always available. This separate product card is
  // the only place that reveals the paid assistant, and the API remains the
  // authority if a browser is stale or a notary opens an old tab.
  function ncRenderAIProduct() {
    var card = $('notary-ai-product'), actions = $('notary-ai-product-actions'), lead = $('notary-ai-product-lead');
    if (!card || !nc.token) { if (card) card.hidden = true; return; }
    var a = nc.aiAccess;
    if (!a || a.reason === 'legacy_open') { card.hidden = true; return; }
    card.hidden = false; clear(actions);
    var beta = a.beta || {}, sub = a.subscription || {};
    var remaining = Number(beta.remaining) || 0;
    var paidRemaining = Number(sub.remaining) || 0;
    if (beta.enrolled && (remaining || paidRemaining || a.paidUses > 0)) {
      lead.textContent = T('Votre accès est actif. Chaque nouvelle analyse consomme une unité; une analyse réutilisée et votre révision restent gratuites.');
      var quota = el('span', 'nc-ai-quota', remaining ? T(remaining + ' essai' + (remaining > 1 ? 's' : '') + ' bêta restant' + (remaining > 1 ? 's' : ''))
        : (paidRemaining ? T(paidRemaining + ' unité' + (paidRemaining > 1 ? 's' : '') + ' restante' + (paidRemaining > 1 ? 's' : '')) : T(a.paidUses + ' unité' + (a.paidUses > 1 ? 's' : '') + ' achetée' + (a.paidUses > 1 ? 's' : ''))));
      actions.appendChild(quota);
    } else {
      lead.textContent = T(beta.enrolled ? 'Vos 5 essais bêta sont terminés. Passez à une formule mensuelle ou achetez seulement les unités dont vous avez besoin.' : 'Inscrivez-vous à la bêta : 5 préparations gratuites, une seule fois par notaire. Le notaire vérifie chaque proposition avant tout envoi.');
      if (!beta.enrolled) {
        var enroll = el('button', 'btn btn-sm btn-primary', T('Activer mes 5 essais')); enroll.type = 'button';
        enroll.addEventListener('click', ncEnrollAIBeta); actions.appendChild(enroll);
      }
      var plans = a.plans || [];
      if (plans.length) {
        var select = el('select', 'nc-ai-plan'); select.setAttribute('aria-label', T('Formule IA'));
        plans.forEach(function (plan) {
          var planName = window.NotaI18N && window.NotaI18N.lang() === 'en' ? (plan.nomEn || plan.nom) : plan.nom;
          var amount = window.NotaI18N && window.NotaI18N.lang() === 'en' && D.moneyEn ? D.moneyEn(plan.monthlyCents / 100) : D.money(plan.monthlyCents / 100);
          var option = el('option', null, planName + ' · ' + amount + T('/mois · ') + plan.includedUses + ' ' + T('dossiers'));
          option.value = plan.id; select.appendChild(option);
        });
        actions.appendChild(select);
        var subscribe = el('button', 'btn btn-sm', T('Choisir cette formule')); subscribe.type = 'button';
        subscribe.addEventListener('click', function () { ncAICheckout('subscription', select.value, 1); }); actions.appendChild(subscribe);
        var quantity = el('input', 'nc-ai-quantity'); quantity.type = 'number'; quantity.min = '1'; quantity.max = '100'; quantity.value = '1'; quantity.setAttribute('aria-label', T('Nombre d’unités à acheter'));
        var piece = el('button', 'btn btn-sm btn-ghost', T('Acheter à la pièce')); piece.type = 'button';
        piece.addEventListener('click', function () { ncAICheckout('usage', select.value, Number(quantity.value)); });
        actions.appendChild(quantity); actions.appendChild(piece);
      }
    }
  }
  async function ncLoadAIAccess() {
    if (!nc.token) return;
    try {
      var r = await fetch(API_BASE + '/notary/ai-access', { headers: { authorization: 'Bearer ' + nc.token } });
      if (r.status === 401) { ncExpire('Session expirée. Reconnectez-vous.'); return; }
      if (!r.ok) { nc.aiAccess = { enabled: true, reason: 'legacy_open' }; ncRenderAIProduct(); return; }
      nc.aiAccess = await r.json();
      ncRenderAIProduct();
    } catch (e) {
      // A transient access-read failure must not block the normal dossier. The
      // AI endpoint still enforces access server-side before any provider call.
      nc.aiAccess = { enabled: true, reason: 'legacy_open' }; ncRenderAIProduct();
    }
  }
  async function ncEnrollAIBeta() {
    if (!nc.token) return;
    var status = $('notary-ai-product-status');
    try {
      var r = await fetch(API_BASE + '/notary/ai-beta/enroll', { method: 'POST', headers: { authorization: 'Bearer ' + nc.token } });
      var j = {}; try { j = await r.json(); } catch (e) {}
      if (!r.ok) { if (status) status.textContent = T('Inscription à la bêta impossible pour le moment.'); return; }
      nc.aiAccess = Object.assign({}, j.access, { plans: (nc.aiAccess && nc.aiAccess.plans) || [] }); ncRenderAIProduct(); ncRenderRetained();
      if (status) status.textContent = T('Bêta activée. Vos 5 essais sont prêts.');
    } catch (e) { if (status) status.textContent = T('Hors ligne. Réessayez une fois en ligne.'); }
  }
  async function ncAICheckout(kind, planId, quantity) {
    if (!nc.token || !planId) return;
    var status = $('notary-ai-product-status');
    try {
      ...
      var r = await fetch(API_BASE + '/notary/ai/checkout', { method: 'POST', headers: { 'content-type': 'application/json', authorization: 'Bearer ' + nc.token }, body: JSON.stringify({ kind, planId, quantity, requestId }) });
      var j = {}; try { j = await r.json(); } catch (e) {}
      if (r.ok && j.url) { window.location.href = j.url; return; }
      if (status) status.textContent = (j.errors && j.errors[0] && j.errors[0].message) || T('Paiement momentanément indisponible.');
    } catch (e) { if (status) status.textContent = T('Hors ligne. Réessayez une fois en ligne.'); }
  }
  ...
  // The manual calendar URL is a secondary path for Outlook/Apple users. Keep
  // it explicit and compact: opening the panel is a choice, while the URL and
  // copy action stay in one predictable row once revealed.
  function wireCalendarSubscriptionDetails() {
    ['sub', 'notary'].forEach(function (prefix) {
      var toggle = $(prefix + '-subscription-toggle');
      var panel = $(prefix + '-subscription-panel');
  ...
  function setCalendarSubscription(prefix, http, name) {
    ...
    var toggle = $(prefix + '-subscription-toggle'), panel = $(prefix + '-subscription-panel');
  ...
  function ncExpire(msg) {
    ...
    nc.aiAccess = null;
    nc.pendingRetain = null;
    var offerNotice = $('nc-offer-notice'); if (offerNotice) offerNotice.hidden = true;
  ...
  async function ncSignIn(email) {
    email = (email || '').trim();
    // A mailbox link normally opens another tab. Keep only the public offer
    // ID and intended account, so that tab can resume the review after login.
    lsSave('nota.notary.offerIntent', nc.deepAct || nc.pendingRetain
      ? { id: nc.deepAct || nc.pendingRetain, email: email.toLowerCase(), at: Date.now() } : null);
    ...
    // 429 — le plafond anti-abus, PAS un refus. La porte est volontairement
    // muette sur l'existence du compte (`{ ok: true, throttled: true }`, sans
    // tableau d'erreurs), donc le repli générique disait « Connexion refusée. »
    // à un notaire qui a simplement redemandé son lien trop souvent : « vous
    // n'avez pas le droit d'entrer » au lieu de « attendez un quart d'heure ».
    if (r.status === 429) {
      ncSetErrors([throttledMessage(r)]);
      return { ok: false };
    }
    if (r.status !== 200) {
  ...
  async function ncApplySession(j, emailHint) {
    ...
    var intent = lsLoad('nota.notary.offerIntent');
    lsSave('nota.notary.offerIntent', null);
    if (!nc.deepAct && intent && typeof intent.id === 'string' && intent.email === String(email || '').toLowerCase()
      && Date.now() - intent.at >= 0 && Date.now() - intent.at < 30 * 60 * 1000) nc.deepAct = intent.id;
    ...
    if (loaded) {
      ...
      if (!nc.pendingRetain && !ncSheetBid) accountTour('notary');
    }
    returnToSigningBeta();
    return { ok: true };
  }
  ...
  async function ncLoadBids() {
    ...
    await ncLoadAIAccess();
    if (!nc.token) return false;
    nc.manquantsServeur = null;           // a fresh profile supersedes an old 403
    nc.profilCharge = true;               // le profil vient du SERVEUR, plus du défaut vide
    ...
    // 4 mois vers l'avant) jusqu'au premier clic — « 0 $ » sur un navigateur
    // neuf. Au mieux : un échec laisse simplement les tuiles au cache.
    ncLoadActs();
  ...
  // Keep a compact six-slot footprint for a sparse month so the landing stays
  // balanced beside the sign-in rail. Once the inventory really grows, the grid
  // adds rows up to the twelve-card cap. Empty slots are inert, clearly
  // labelled, and never fake offers. At zero there is no inventory to reserve
  // room for: the footprint drops to three, one large empty-state panel covers
  // it, and the next-step block under the grid carries the page instead.
  var NC_LIVE_MAX = 12;
  ...
  // ADR 0052 §A — la troisième promesse est un CHIFFRE, pas un adjectif : les
  // honoraires du même acte à délai normal, puis au premier palier que le
  // domaine marque « élevé » (celui où la date est le problème). Les deux
  // sortent de tierAmount(), c'est-à-dire de l'échelle TIERS et des offres du
  // mois — la phrase suit le marché, et changer l'échelle la change.
  // Le service est celui par défaut du domaine, jamais le filtre du carnet :
  // la page des notaires ne doit pas dépendre de ce qu'un client a cliqué.
  function renderNotaryPromesse() {
    var out = $('nc-promesse-urgence'); if (!out) return;
    var svc = D.serviceById(D.DEFAULT_SERVICE_ID) || D.SERVICES[0];
    var presse = null;
    for (var i = 0; i < D.TIERS.length && !presse; i++) if (D.TIERS[i].eleve) presse = D.TIERS[i];
    var calmeAmt = tierAmount('standard', svc);
    var presseAmt = presse ? tierAmount(presse.id, svc) : 0;
    // Une échelle plate ne donne pas une demi-phrase : elle n'en donne aucune.
    if (!svc || !presse || !calmeAmt || presseAmt <= calmeAmt) { out.textContent = ''; return; }
    out.textContent = T('Le même acte : ' + D.money(calmeAmt) + ' d’honoraires à délai normal, '
      + D.money(presseAmt) + ' à ' + presse.maxJours + ' jours d’avis.');
  }

  function renderNotaryLive() {
    renderNotaryPromesse();
    // La vignette de l'agenda vit et meurt avec cette page : son `enabled` la
    // referme dès que la console s'ouvre, et son onScreen() park la boucle dès
    // que l'onglet change.
  ...
  function makeWeekVignette(cfg) {
    var v = { timers: [], raf: null, offset: 0, cycling: false, waits: 0 };
    // Combien de fois la boucle attend que sa vignette apparaisse avant
    // d'abandonner. Elle est armée au moment où la page se rend, ce qui peut
    // précéder l'instant où elle est VISIBLE : sur un lien direct vers
    // #t=notaires, le film d'accueil couvre encore l'application et le tableau
    // restait vide pour toujours (constaté au navigateur, 2026-09-12). Borné :
    // une vignette qu'on ne verra jamais ne doit pas sonder indéfiniment.
    var WAIT_MS = 400, MAX_WAITS = 20;
    ...
    function cycle() {
      if (!onScreen()) {
        // Pas encore à l'écran : on repasse, quelques fois. Une fois la patience
        // épuisée, on se gare pour de bon — c'est ce qui park la boucle quand
        // l'onglet change ou que la fenêtre passe en arrière-plan.
        if (v.waits < MAX_WAITS) { v.waits++; later(cycle, WAIT_MS); return; }
        stop(); return;
      }
      v.waits = 0;
      var batch = batchNow();
  ...
  // Même réveil pour les deux vignettes des pages d'acquisition. Sans lui, un
  // visiteur qui change d'onglet une dizaine de secondes revient devant un
  // tableau mort : la boucle s'était garée, et plus rien ne la rarmait.
  document.addEventListener('visibilitychange', function () {
    if (document.hidden) return;
    if (state.tab === 'notaires') renderNotaryLive();
  });
  ...
  var NC_PREFS_SAVED = '✓ Préférences enregistrées.';
  var NC_PROFIL_SAVED = '✓ Profil enregistré.';
  var NC_LENDERS_SAVED = '✓ Prêteurs enregistrés sur cet appareil.';
  // A confirmation that comes and goes must not move the panel it confirms.
  // The line is RESERVED (CSS min-height) and only its sentence arrives and
  // leaves — which is also exactly what makes a role="status" announce.
  function ncSavedNote(id, text, timer) {
    var saved = $(id);
    if (!saved) return timer;
    saved.textContent = text;
    clearTimeout(timer);
    return setTimeout(function () { saved.textContent = ''; }, 2200);
  }
  function ncPrefsSavedNote() { ncPrefsSavedT = ncSavedNote('notary-prefs-saved', NC_PREFS_SAVED, ncPrefsSavedT); }
  ...
  function ncLendersSavedNote() { ncLendersSavedT = ncSavedNote('notary-lenders-saved', NC_LENDERS_SAVED, ncLendersSavedT); }
  ...
  function ncRenderPrefs() {
    ...
    if (lendBlock) {
      ...
      if (!$('notary-lenders-saved')) {
        var note = el('p', 'help nc-prefs-saved');
        note.id = 'notary-lenders-saved';
  ...
  function ncRenderAuthState() {
    ...
    var authed = !!nc.token;
    var pageTitle = $('notary-page-title');
    if (pageTitle) pageTitle.textContent = T(authed ? 'Espace notaire' : 'Ajoutez Nota à votre agenda.');
    ...
    var calendarAccess = $('notary-calendar-access');
    if (calendarAccess) calendarAccess.hidden = authed;
  ...
  function ncFiltersExpanded() { return flagGet(LS_NC_FILTERS) === 'open'; }
  function ncRenderFilterDisclosure() {
    var expanded = ncFiltersExpanded();
    var toggle = $('notary-filters-toggle');
    if (toggle) {
      toggle.hidden = !nc.open.length;
      toggle.setAttribute('aria-expanded', expanded ? 'true' : 'false');
    }
    var count = Number(nc.filter.service !== 'all') + Number(nc.filter.readyOnly) + Number(!!nc.filter.day);
    var badge = $('notary-filters-count');
    if (badge) { badge.textContent = String(count); badge.hidden = !count; }
    var filters = $('notary-open-filter');
    if (filters) filters.hidden = !expanded || !nc.open.length;
    var days = $('notary-open-days');
  ...
  function ncOpenCard(b) {
    ...
    // Keep the signing location in the scan. The lender and full sector
    // context remain available in the expanded offer and the review sheet.
    var location = ncDeplacementPill(b.deplacement);
    if (location) {
      var locationRow = el('div', 'nc-card-location');
      locationRow.appendChild(location);
      if (b.distanceKm != null) locationRow.appendChild(el('span', 'nc-distance', '≈ ' + b.distanceKm + ' km'));
    ...
    var offerDetails = Array.isArray(b.details) ? b.details.filter(function (d) { return d && typeof d.label === 'string'; }) : [];
    if (offerDetails.length) {
      var parameters = el('dl', 'nc-card-parameters');
      var parameterLabels = [];
      offerDetails.forEach(function (d) {
        if (!d.label.trim() || parameterLabels.indexOf(d.label) !== -1) return;
        parameterLabels.push(d.label);
        var row = el('div');
        row.appendChild(el('dt', null, T(d.label)));
        row.appendChild(el('dd', null, d.value == null ? T('Non précisé') : T(String(d.value))));
        parameters.appendChild(row);
      });
      body.appendChild(parameters);
    } else if (ncOfferLabels(b.complexity && b.complexity.factors).length) {
      ...
      var factors = ncOfferLabels(b.complexity.factors).filter(function (f) {
    ...
    var dec = el('button', 'btn btn-sm btn-ghost nc-decline', 'Décliner'); dec.type = 'button';
    more.appendChild(dec);
  ...
  function ncProfilGate(j, bid) {
    // Keep the selected offer while the notary fills the required contact fields.
    if (bid || ncSheetBid) nc.pendingRetain = (bid || ncSheetBid).id;
  ...
  function ncRenderProfilBanner() {
    ...
    // PREMIÈRE CONNEXION. Le film d'accueil est inatteignable une fois connecté
    // (`maybeShowOnboarding` sort dès que le rôle n'est plus « anon »), et le
    // panneau de profil est replié, en 8e position. Un notaire neuf arrivait
    // donc devant un fil quasi vide — `rayonKm` vaut 0 par défaut, donc
    // `notaryCanServe` ne lui montre que les clients qui se déplacent — sans
    // rien pour relier l'un à l'autre. On ouvre le panneau UNE fois par
    // session : c'est le geste qui débloque tout le reste.
    if (missing.length && nc.profilCharge && !nc.profilOuvertAuto) {
      nc.profilOuvertAuto = true;
      var panel = $('notary-profil');
  ...
  // Labels supplied by the API are text, never markup. Older offers can omit
  // these optional arrays; ignore blank/malformed entries and repeated labels.
  function ncOfferLabels(value) {
    return Array.isArray(value) ? value.filter(function (v, i, all) {
      return typeof v === 'string' && v.trim() && all.indexOf(v) === i;
    }) : [];
  }
  function ncOpenRetainSheet(b) {
    ...
    ncSheetBid = b;
    nc.pendingRetain = null;
    var scroll = dlg.querySelector('.nc-retenir'); if (scroll) scroll.scrollTop = 0;
    var sheetError = $('nc-retenir-error'); if (sheetError) sheetError.hidden = true;
    var svc = D.serviceById(b.serviceId);
    ncSetText('nc-retenir-svc', svc ? svc.nom : b.serviceId);
    var fullDate = new Intl.DateTimeFormat(LOCALE, { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric', timeZone: 'UTC' });
    ncSetText('nc-retenir-date', fullDate.format(new Date(b.dateISO + 'T00:00:00Z')) + ' · ' + relativeDay(b.dateISO));
    ncSetText('nc-retenir-fee', D.money(b.montant));
    ncSetText('nc-retenir-go-amt', D.money(b.montant));
    var signals = ncSetText('nc-retenir-signals', null);
    if (signals) {
      signals.appendChild(ncTierPill(b.tier));
      if (b.complexity) signals.appendChild(ncComplexityPill(b.complexity));
    }
    ...
    // Separate the permitted travel band from the measured distance: they
    // answer different questions and must never read as competing estimates.
    ...
    ncSetText('nc-retenir-sector', b.prefixe || T('Non précisé'));
    var hasDistance = b.distanceKm != null && Number.isFinite(Number(b.distanceKm)) && Number(b.distanceKm) >= 0;
    ncSetText('nc-retenir-distance', hasDistance ? '≈ ' + b.distanceKm + ' km' : T('Non précisée'));
    ...
    if (lender) {
      ...
      // The service definition owns whether a lender question applies.
      var hasLender = !!lp || !!(svc && svc.pricing && svc.pricing.criteria.some(function (c) { return c.id === D.LENDER_CRITERION_ID; }));
      lender.hidden = !hasLender;
      $('nc-retenir-lender-label').hidden = !hasLender;
    }
    var expiry = ncSetText('nc-retenir-expiry', null);
    var validExpiry = /^\d{4}-\d{2}-\d{2}$/.test(b.expiresOn || '') && Number.isFinite(Date.parse(b.expiresOn + 'T00:00:00Z'));
    if (expiry) {
      expiry.hidden = !validExpiry;
      $('nc-retenir-expiry-label').hidden = !validExpiry;
      if (validExpiry) expiry.textContent = fullDate.format(new Date(b.expiresOn + 'T00:00:00Z'));
    }
    var dossier = ncSetText('nc-retenir-dossier', null);
    if (dossier) {
      dossier.appendChild(ncReadyBadge(b.ready));
      dossier.appendChild(el('p', 'nc-retenir-p nc-retenir-readiness', 'L’état du dossier indique si les renseignements requis et le consentement sont reçus. Les documents restent à vérifier.'));
      var missing = ncOfferLabels(b.missing);
      if (missing.length) {
        var miss = el('div', 'nc-retenir-missing');
        var heading = el('h5', null, 'Éléments à compléter');
        heading.appendChild(document.createTextNode(' '));
        heading.appendChild(el('span', 'nc-retenir-count', String(missing.length)));
        miss.appendChild(heading);
        var list = el('ul', 'nc-retenir-checklist');
        missing.forEach(function (m) { list.appendChild(el('li', null, m)); });
        miss.appendChild(list);
        dossier.appendChild(miss);
      } else {
        dossier.appendChild(el('p', 'nc-retenir-p', Array.isArray(b.missing) ? 'Aucun élément manquant signalé.' : 'La liste des éléments manquants n’est pas disponible.'));
      }
    }
    var factors = ncSetText('nc-retenir-factors', null);
    if (factors) {
      var detailRows = Array.isArray(b.details) ? b.details.filter(function (d) { return d && typeof d.label === 'string'; }) : [];
      var labels = ncOfferLabels(b.complexity && b.complexity.factors);
      factors.appendChild(el('h5', null, 'Renseignements sur la demande'));
      if (detailRows.length) {
        var detailList = el('dl', 'nc-retenir-details');
        detailRows.forEach(function (d) {
          var row = el('div', 'nc-retenir-detail');
          row.appendChild(el('dt', null, T(d.label)));
          row.appendChild(el('dd', null, d.value == null ? T('Non précisé') : T(String(d.value))));
          detailList.appendChild(row);
        });
        factors.appendChild(detailList);
      } else if (labels.length) {
        var factorList = el('ul', 'nc-retenir-factor-list');
        labels.forEach(function (label) { factorList.appendChild(el('li', null, T(label))); });
        factors.appendChild(factorList);
      } else factors.appendChild(el('p', 'nc-retenir-p', 'Aucun renseignement supplémentaire fourni.'));
    }
    var status = ncSetText('nc-retenir-status', null);
    if (status) {
      var statusRow = ncStatusRow(b);
      if (statusRow) {
        status.appendChild(el('h5', null, 'Vos démarches sur cette offre'));
        status.appendChild(statusRow);
        var requested = ncOfferLabels(b.demande && b.demande.documents);
        if (requested.length) {
          var requestedList = el('ul', 'nc-retenir-checklist');
          requested.forEach(function (id) {
            var item = D.requestableItems(b.serviceId).find(function (it) { return it.id === id; });
            requestedList.appendChild(el('li', null, item ? item.nom || item.label : id));
          });
          status.appendChild(requestedList);
        }
      }
    }
    ...
    if (cautionDd) {
      ...
      if (jours > 0 && b.caution && b.caution.etat !== 'posee') {
        var rule = el('div', 'nc-retenir-more');
        rule.appendChild(el('div', 'nc-retenir-sub', T(ncCautionRegle(jours))));
        cautionDd.appendChild(rule);
      }
    }
  ...
  function ncRevealRetained(id) {
    var card = document.querySelector('#notary-retained-list .nc-card[data-id="' + id + '"]');
    if (!card) return;
    ncSetRetainedExpanded(card, true);
  ...
  async function ncConfirmRetainSheet() {
    ...
    var go = $('nc-retenir-go'); if (go && go.disabled) return;
    if (go) go.disabled = true;
    var err = $('nc-retenir-error'); if (err) err.hidden = true;
    var ok = false;
    try { ok = (await ncAccept(b.id, b.dateISO, b)) === true; }
    finally { if (go) go.disabled = false; }
    // A temporary failure leaves the review in place so the notary can retry.
    // Expired sessions, unavailable offers and profile refusals still leave it.
    if (ok || !nc.token || !ncFindOpen(b.id) || ncProfilIncomplet()) ncCloseRetainSheet();
    else if (err) err.hidden = false;
    // The act moved to « Dossiers retenus »: the focus follows it (audit P2-2).
    if (ok) {
      // Keep the accessible focus on the section heading, but bring the actual
      // dossier into view so the notary can continue without hunting below the
      // open-demand list. The flash also confirms which card accepted.
      ncRevealRetained(b.id);
  ...
  function ncRenderFilter() {
    ...
    D.SERVICES.forEach(function (s) {
      var n = nc.open.filter(function (b) { return b.serviceId === s.id && ncLenderAccepted(b); }).length;
      if (!n) return;
  ...
  function ncRenderOpen() {
    ...
    ncRenderFilter();
    var all = nc.open;
    var filtered = ncFilteredOpen();
    var days = D.agendaByDate(filtered);
    if (nc.filter.day && !days.some(function (d) { return d.dateISO === nc.filter.day; })) nc.filter.day = null;
    ncRenderFilterDisclosure();
    var shown = filtered.filter(function (b) { return !nc.filter.day || b.dateISO === nc.filter.day; });
    var reset = $('notary-open-reset');
    if (reset) reset.hidden = nc.filter.service === 'all' && !nc.filter.readyOnly && !nc.filter.day;
    var lendersHideAll = all.length > 0 && !all.some(ncLenderAccepted);
    var prefsDoor = $('notary-open-prefs'); if (prefsDoor) prefsDoor.hidden = !lendersHideAll;
    ...
    if (head) {
      ...
      head.appendChild(el('span', 'nc-h-n', String(shown.length)));
      if (shown.length) {
        var inPlay = shown.reduce(function (s, b) { return s + (Math.round(Number(b.montant)) || 0); }, 0);
        head.appendChild(document.createTextNode(' · '));
    ...
    if (!all.length) {
      if (empty) {
        // Un fil vide a DEUX causes très différentes, et les confondre coûte un
        // notaire : ou bien le marché est calme, ou bien c'est SON périmètre
        // qui le rend aveugle. `rayonKm` vaut 0 par défaut (« je ne me déplace
        // pas »), donc `notaryCanServe` ne montre alors que les clients qui
        // viennent à l'étude. Sans cette phrase, un profil neuf se lit comme
        // « Nota n'a pas de clients ».
        var p = nc.profil || {};
        var rayon = Number(p.rayonKm) || 0;
        empty.textContent = rayon === 0
          ? 'Aucune demande ouverte pour l’instant. Vous n’acceptez aucun déplacement : seuls les clients qui viennent à votre étude vous sont proposés. Augmentez votre rayon dans « Votre profil public » pour en voir davantage.'
    ...
    if (!days.length) {
      if (empty) {
        empty.textContent = lendersHideAll ? 'Vos préférences de prêteurs masquent ces demandes.' : 'Aucune demande ne correspond à ce filtre.';
        empty.hidden = false;
      }
      return;
    }
  ...
  function ncResetFilters() {
    nc.filter.service = 'all'; nc.filter.readyOnly = false; nc.filter.day = null;
    ncRenderOpen();
    var head = $('notary-open-h');
  ...
  function ncDayStrip(days) {
    ...
    strip.id = 'notary-open-days';
    strip.hidden = !ncFiltersExpanded();
  ...
  function ncWorkflowBlock(packet) {
    var workflow = packet && packet.workflow;
    if (!workflow) return null;
    var wrap = el('section', 'nc-workflow');
    wrap.appendChild(el('h4', 'nc-dossier-h', T('Prochaine action')));
    wrap.appendChild(el('p', 'help', T('Nota rassemble le contexte client et les propositions IA pour réduire la préparation. Le notaire conserve chaque contrôle critique et chaque décision professionnelle.')));
    var aiLabels = {
      not_started: 'Aucune analyse IA lancée.',
      abstained: 'L’IA n’a pas produit de proposition exploitable.',
      awaiting_review: 'Des propositions IA attendent votre révision.',
      reviewed: 'Les propositions IA ont été révisées.',
    };
    var ai = el('p', 'nc-workflow-ai');
    ai.appendChild(el('strong', null, T('État de l’IA')));
    ai.appendChild(document.createTextNode(' : '));
    ai.appendChild(el('span', null, T(aiLabels[workflow.ai && workflow.ai.status] || 'État de l’IA non précisé.')));
    if (workflow.ai && workflow.ai.proposalCount > 1) {
      ai.appendChild(document.createTextNode(' · ' + workflow.ai.proposalCount + ' ' + T('propositions')));
    }
    wrap.appendChild(ai);
    var list = el('ol', 'nc-workflow-actions');
    var labels = {
      request_missing_items: 'Demander les renseignements et pièces manquants',
      analyze_documents: 'Analyser les documents autorisés avec l’IA',
      review_ai_proposals: 'Réviser les propositions de l’IA',
      resolve_exceptions: 'Résoudre les différences et dates signalées',
      complete_critical_controls: 'Compléter les contrôles critiques et joindre les preuves',
      confirm_signing_conditions: 'Confirmer les conditions de signature et de clôture',
    };
    (workflow.nextActions || []).forEach(function (action) {
      var item = el('li', 'nc-workflow-action');
      var label = el('strong', null, T(labels[action.id] || 'Action à préciser'));
      item.appendChild(label);
      var meta = el('span', 'nc-workflow-action-meta');
      meta.appendChild(el('span', null, T(action.owner === 'client' ? 'À faire par le client' : 'À faire par le notaire')));
      meta.appendChild(document.createTextNode(' · '));
      meta.appendChild(el('span', null, T(action.priority === 'now' ? 'À faire maintenant' : 'Ensuite')));
      if (action.count > 1) meta.appendChild(document.createTextNode(' · ' + action.count + ' ' + T('éléments')));
  ...
  function ncFinancingWorkPacket(packet, active) {
    ...
    var workflow = ncWorkflowBlock(packet); if (workflow) wrap.appendChild(workflow);
    ...
    if ((packet.controls || []).length) {
      var controls = section('Plan des contrôles et intégrations');
      note(controls, 'Chaque contrôle reste en attente d’une décision et d’une preuve dans le dossier du notaire.');
      packet.controls.forEach(function (control) {
        var block = el('div', 'nc-work-packet-control');
        line(block, control.label, T('Préparation automatique seulement'), control.critical ? 'Contrôle critique' : 'Contrôle à confirmer');
        line(block, 'Type d’intégration', T(control.integrationLabel || control.integrationType));
        line(block, 'Système candidat', T(control.integrationCandidate));
        controls.appendChild(block);
      });
    }
    if (packet.parameterCoverage) {
      var coverage = section('Couverture des paramètres');
      note(coverage, 'Nota relie les réponses du client, les pièces, les propositions IA et les décisions qui restent au notaire.');
      var parameterCounts = packet.parameterCoverage;
      line(coverage, 'Paramètres tarifaires', (parameterCounts.pricing || []).length);
      line(coverage, 'Champs de collecte', (parameterCounts.intake || []).length);
      line(coverage, 'Documents du parcours', (parameterCounts.documents || []).length);
      line(coverage, 'Champs préparables par l’IA', (parameterCounts.ai && parameterCounts.ai.fields || []).length);
      line(coverage, 'Contrôles humains critiques', (parameterCounts.humanControls || []).length);
      if (parameterCounts.caseCoverage) {
        line(coverage, 'Branches de cas connues', (parameterCounts.caseCoverage.cases || []).length);
        note(coverage, 'Les cas inconnus sont orientés vers le notaire avant toute automatisation.');
      }
      if (parameterCounts.ai && (parameterCounts.ai.unmappedIntake || []).length) {
        note(coverage, 'Certains champs de collecte sont préparés par le dossier et le notaire sans extraction automatique : ' + parameterCounts.ai.unmappedIntake.join(', ') + '.');
      }
      (parameterCounts.integrations || []).forEach(function (integration) {
        line(coverage, integration.label, integration.candidate);
      });
    }
  ...
  function ncFinancingAIBlock(entry) {
    // The normal dossier remains visible when a notary has not opted into the
    // beta, has exhausted its trial, or has an unpaid subscription.
    if (nc.aiAccess && !nc.aiAccess.enabled && nc.aiAccess.reason !== 'legacy_open') return null;
    var actAI = !['financement', 'refinancement'].includes(entry.serviceId);
    var aiFields = actAI ? (D.actAIFields(entry.serviceId) || []) : D.FINANCING_AI_FIELDS;
    var aiRoute = actAI ? '/notary/acts' : '/notary/financing';
    ...
    function fieldLabel(id) {
      var field = aiFields.find(function (item) { return item.id === id; });
    ...
    function failure(node, status, fallback, body) {
      ...
      var copy = status === 503 && [actAI ? 'act_ai_disabled' : 'financing_ai_disabled', actAI ? 'act_ai_unavailable' : 'financing_ai_unavailable'].includes(code)
        ? 'Analyse IA indisponible : le fournisseur est désactivé ou indisponible. Aucune nouvelle analyse n’a été produite.'
        : code === (actAI ? 'act_ai_invalid_output' : 'financing_ai_invalid_output')
    ...
    consentLabel.appendChild(el('span', null, T('Je confirme être autorisé à transmettre ce document au fournisseur d’IA configuré, sans utilisation pour l’entraînement du texte du dossier; seules des décisions notariales minimisées peuvent contribuer au programme contrôlé d’amélioration.')));
    ...
    function render(analysis) {
      ...
      var uncertaintyQuestions = D.notaryAIUncertaintyQuestions
        ? D.notaryAIUncertaintyQuestions(entry.serviceId, preparation)
        : [];
      if (uncertaintyQuestions.length) {
        results.appendChild(el('div', 'nc-dossier-h', T('Questions de clarification')));
        results.appendChild(el('p', 'help', T('Un point incertain a été repéré. Confirmez-le avec les sources et utilisez votre décision comme feedback professionnel pour améliorer Nota.')));
        var questions = el('ul', 'nc-ai-questions');
        uncertaintyQuestions.forEach(function (question) {
          var item = el('li');
          var prefix = question.kind === 'conflicting_evidence'
            ? 'Confirmer la contradiction et choisir la source autorisée : '
            : 'Confirmer l’absence ou rechercher une pièce source : ';
          item.appendChild(el('strong', null, T(prefix)));
          item.appendChild(fieldLabel(question.fieldId));
          var feedbackForm = el('form', 'nc-ai-question-feedback');
          var decisionLabel = el('label', 'field');
          decisionLabel.appendChild(el('span', null, T('Réponse à la question')));
          var decision = el('select');
          decision.name = 'decision';
          [['', 'Choisir une réponse'], ['confirmed', 'Point confirmé'], ['resolved', 'Point résolu'],
            ['not_applicable', 'Non applicable'], ['escalated', 'À examiner par le notaire']].forEach(function (option) {
            var choice = el('option', null, T(option[1])); choice.value = option[0]; decision.appendChild(choice);
          });
          decision.required = true;
          decisionLabel.appendChild(decision);
          feedbackForm.appendChild(decisionLabel);
          var note = el('textarea'); note.name = 'note'; note.rows = 2; note.maxLength = 500;
          var noteLabel = el('label', 'field');
          noteLabel.appendChild(el('span', null, T('Note facultative'))); noteLabel.appendChild(note);
          feedbackForm.appendChild(noteLabel);
          var saveFeedback = el('button', 'btn btn-sm btn-secondary', 'Enregistrer la réponse');
          saveFeedback.type = 'submit'; feedbackForm.appendChild(saveFeedback);
          var feedbackStatus = el('p', 'help'); feedbackStatus.setAttribute('role', 'status'); feedbackForm.appendChild(feedbackStatus);
          feedbackForm.addEventListener('submit', async function (event) {
            event.preventDefault();
            if (!active() || busy || saveFeedback.disabled || !feedbackForm.reportValidity()) return;
            saveFeedback.disabled = true; decision.disabled = true; note.disabled = true;
            message(feedbackStatus, 'Enregistrement de la réponse…');
            try {
              var response = await ncPost('/notary/ai-feedback', {
                id: entry.id, dateISO: entry.dateISO, questionId: question.id,
                decision: decision.value, note: note.value.trim(),
              });
              if (!active()) return;
              if (!response || response.status < 200 || response.status >= 300 || !response.json.ok) {
                saveFeedback.disabled = false; decision.disabled = false; note.disabled = false;
                message(feedbackStatus, 'Impossible d’enregistrer la réponse. Réessayez.', true); return;
              }
              message(feedbackStatus, 'Réponse enregistrée.');
            } catch {
              saveFeedback.disabled = false; decision.disabled = false; note.disabled = false;
              if (active()) message(feedbackStatus, 'Impossible d’enregistrer la réponse. Réessayez.', true);
            }
          });
          item.appendChild(feedbackForm);
      ...
      form.addEventListener('submit', async function (event) {
        ...
        try {
          var response = await ncPost(aiRoute + '/review', {
    ...
    wrap.addEventListener('toggle', async function (event) {
      ...
      try {
        var response = await fetch(API_BASE + aiRoute + '/preparation?id=' + encodeURIComponent(entry.id) + '&dateISO=' + encodeURIComponent(entry.dateISO), {
    ...
    sourceForm.addEventListener('submit', async function (event) {
      ...
      try {
        var response = await ncPost(aiRoute + '/preparation', {
  ...
  function ncDossierBlock(entry) {
    ...
    var isFinancing = ['financement', 'refinancement'].includes(entry.serviceId);
    var preparation = isFinancing
      ? D.financingPreparation(entry.serviceId, d, entry.pricing)
      : D.actPreparation(entry.serviceId, d, entry.pricing);
    if (preparation) {
      var brief = el('details', 'nc-preparation');
      brief.appendChild(el('summary', null, isFinancing ? 'Préparation du financement' : 'Préparation de l’acte'));
      ...
      brief.appendChild(el('div', 'nc-dossier-h', isFinancing ? 'Vérifications du notaire et du prêteur' : 'Vérifications du notaire'));
      var checks = el('ul');
      preparation.checks.forEach(function (check) { checks.appendChild(el('li', null, check.texte || check.label)); });
      brief.appendChild(checks);
      if ((preparation.controls || []).length) {
        brief.appendChild(el('div', 'nc-dossier-h', 'Plan des contrôles et intégrations'));
        brief.appendChild(el('p', 'help', 'La préparation peut être automatisée; chaque contrôle reste à décider et à documenter par le notaire.'));
        var controls = el('ul');
        preparation.controls.forEach(function (control) {
          var item = el('li');
          item.appendChild(el('strong', null, T(control.label)));
          item.appendChild(el('p', 'help', T(control.integrationLabel || control.integrationType) + ' · ' + T(control.integrationCandidate)));
          controls.appendChild(item);
        });
        brief.appendChild(controls);
      }
      var aiBlock = ncFinancingAIBlock(entry);
      if (aiBlock) brief.appendChild(aiBlock);
  ...
  // ADR 0041 — l'acte que le client a annulé après la rétention, tant que
  // l'indemnité est à décider : le notaire réclame (montant sous le plafond,
  // justification) ou renonce. Rien du client n'y figure : la relation est
  // finie (art. 37). La carte disparaît à la décision, ou à l'échéance.
  function ncIndemniteCard(entry) {
    var svc = D.serviceById(entry.serviceId);
    var a = entry.annulation || {};
    var plafond = Number(a.plafond) || 0;
    var card = el('div', 'nc-card is-retained is-annulee'); card.dataset.id = entry.id; card.dataset.date = entry.dateISO || '';
    var head = el('div', 'nc-card-head');
    ...
    meta.appendChild(el('span', 'pill pill-annulee', 'Annulée par le client'));
    card.appendChild(meta);
    var facts = ncFactsRow({ dateISO: entry.dateISO, showDate: true });
    if (facts) card.appendChild(facts);

    var box = el('div', 'nc-indemnite');
    var p = el('p', 'help');
    p.appendChild(el('span', null, 'Le client a annulé après votre rétention. Vous pouvez réclamer vos frais réels et la valeur du travail accompli, sur justification, jusqu’à'));
    p.appendChild(document.createTextNode(' '));
    p.appendChild(el('strong', 'nc-forecast-amt', D.money(plafond)));
    p.appendChild(document.createTextNode(' (' + ncPct(a.taux) + ')'));
    if (a.echeanceISO) {
      p.appendChild(document.createTextNode(' · '));
      p.appendChild(el('span', null, 'd’ici le'));
      p.appendChild(document.createTextNode(' '));
      p.appendChild(el('span', null, dayShort(a.echeanceISO)));
    }
    p.appendChild(document.createTextNode('.'));
    box.appendChild(p);
    box.appendChild(el('p', 'help', 'Sans réclamation de votre part, rien n’est prélevé au client. L’indemnité vous est versée en entier.'));

    var form = el('div', 'nc-indemnite-form');
    var amtField = el('div', 'field');
    amtField.appendChild(el('label', null, 'Montant réclamé (en dollars)'));
    var amt = el('input', 'input nc-indemnite-amt');
    amt.type = 'number'; amt.min = '0'; amt.max = String(plafond); amt.step = '1'; amt.value = String(plafond);
    amt.setAttribute('aria-label', 'Montant réclamé, en dollars');
    amt.setAttribute('data-i18n-skip', '');
    amtField.appendChild(amt);
    form.appendChild(amtField);
    var just = el('textarea', 'nc-indemnite-just');
    just.rows = 3; just.maxLength = D.INDEMNITE_JUSTIFICATION_MAX || 600;
    just.placeholder = 'Justification : frais engagés, travail accompli (au moins 20 caractères)';
    just.setAttribute('aria-label', 'Justification de l’indemnité');
    form.appendChild(just);
    var err = el('p', 'help nc-indemnite-err'); err.hidden = true;
    form.appendChild(err);
    var actions = el('div', 'nc-release-actions');
    var go = el('button', 'btn btn-sm btn-primary nc-indemnite-go', 'Réclamer cette indemnité'); go.type = 'button';
    var waive = el('button', 'btn btn-sm nc-indemnite-waive', 'Renoncer à toute indemnité'); waive.type = 'button';
    actions.appendChild(go); actions.appendChild(waive);
    form.appendChild(actions);
    go.addEventListener('click', function () { ncIndemniteSend(entry, Number(amt.value), just.value, err, [go, waive]); });
    waive.addEventListener('click', function () { ncIndemniteSend(entry, 0, '', err, [go, waive]); });
    box.appendChild(form);
    card.appendChild(box);
    return card;
  }
  async function ncIndemniteSend(entry, montant, justification, err, btns) {
    btns.forEach(function (b) { b.disabled = true; });
    err.hidden = true; err.textContent = '';
    var r, j = {};
    try {
      r = await fetch(API_BASE + '/notary/bids/indemnite', {
        method: 'POST',
        headers: { 'content-type': 'application/json', authorization: 'Bearer ' + nc.token },
        body: JSON.stringify({ id: entry.id, dateISO: entry.dateISO, montant: montant, justification: justification }),
      });
      try { j = await r.json(); } catch (e) {}
    } catch (e) {
      btns.forEach(function (b) { b.disabled = false; });
      toast('Réclamation impossible (hors ligne).');
      return;
    }
    if (!r.ok) {
      btns.forEach(function (b) { b.disabled = false; });
      var msg = (j.errors && j.errors[0] && j.errors[0].message) || 'Réclamation impossible.';
      err.hidden = false; err.textContent = msg;
      if (r.status === 409) { ncRetainedSave(nc.email, ncRetainedFor(nc.email).filter(function (e) { return e.id !== entry.id; })); ncRenderRetained(); }
      return;
    }
    var a = j.annulation || {};
    toast(a.statut === 'percue' ? 'Indemnité réclamée : ' + D.money(a.frais) + '.'
      : a.statut === 'refusee' ? 'La carte du client a refusé le prélèvement.'
        : 'Vous avez renoncé à toute indemnité.');
    // Decided: the server no longer returns the act; drop it locally too.
  ...
  function ncRetainedCard(entry) {
    if (entry && entry.annulee) return ncIndemniteCard(entry);
    var svc = D.serviceById(entry.serviceId);
    var card = el('div', 'nc-card is-retained'); card.dataset.id = entry.id; card.dataset.date = entry.dateISO || '';
    var summary = el('div', 'nc-retained-summary');
    var overview = el('div', 'nc-retained-overview');
    var head = el('div', 'nc-card-head');
    head.appendChild(el('div', 'nc-card-title', svc ? svc.nom : entry.serviceId));
    ...
    if (!entry.completed) {
      ...
      if (cautionLine) {
        ...
        overview.appendChild(cautionRow);
      }
    }
    ...
    if (status) overview.appendChild(status);
    summary.appendChild(overview);
    summary.appendChild(ncClientBlock(entry));
    card.appendChild(summary);
    // A quick overview stays readable in the list. The entire working file
    // remains mounted behind one disclosure so collapsing never loses a draft.
    var start = el('div', 'nc-first-contact nc-retained-actions');
    var firstContact = !entry.completed && !(entry.messages || []).length;
    if (firstContact) start.appendChild(el('p', null, 'Confirmez l’heure du rendez-vous et les documents à préparer.'));
    var buttons = el('div', 'nc-retained-buttons');
    var write = el('button', 'btn btn-primary ' + (firstContact ? 'nc-contact-start' : 'nc-contact-open'), firstContact ? 'Écrire au client' : 'Ouvrir la conversation');
    write.type = 'button';
    buttons.appendChild(write);
    var toggle = el('button', 'btn nc-retained-toggle', 'Détails du dossier');
    toggle.type = 'button';
    toggle.setAttribute('aria-controls', 'nc-retained-body-' + entry.id);
    toggle.setAttribute('aria-expanded', 'false');
    buttons.appendChild(toggle);
    start.appendChild(buttons);
    card.appendChild(start);
    var workspace = el('div', 'nc-retained-workspace');
    workspace.id = 'nc-retained-body-' + entry.id;
    workspace.hidden = true;
    workspace.appendChild(ncChatBlock(entry));
    var file = el('div', 'nc-retained-file');
    file.appendChild(ncDossierBlock(entry));
    ...
    file.appendChild(ncCompleteBlock(entry));
    ...
    if (!entry.completed) {
      ...
      if (forecast) file.appendChild(forecast);
      file.appendChild(ncReleaseBlock(entry));
    }
    workspace.appendChild(file);
    card.appendChild(workspace);
    ncSetRetainedExpanded(card, !!(nc.retainedDetails && nc.retainedDetails[entry.id]));
    return card;
  }
  function ncSetRetainedExpanded(card, expanded) {
    var workspace = card && card.querySelector('.nc-retained-workspace');
    if (!workspace) return;
    if (!nc.retainedDetails) nc.retainedDetails = {};
    nc.retainedDetails[card.dataset.id] = !!expanded;
    workspace.hidden = !expanded;
    card.classList.toggle('is-expanded', !!expanded);
    var toggle = card.querySelector('.nc-retained-toggle');
    if (toggle) {
      toggle.setAttribute('aria-expanded', expanded ? 'true' : 'false');
      toggle.textContent = T(expanded ? 'Vue simple' : 'Détails du dossier');
    }
  }
  ...
  function ncForecastLine(entry) {
    ...
    if (!a || !(Number(a.plafond) > 0)) return null;
    // ADR 0041 — un PLAFOND, réclamable sur justification : jamais une somme
    // promise. Le notaire lit ce qu'il pourrait réclamer, pas ce qu'il reçoit.
    var p = el('p', 'help nc-forecast');
    p.appendChild(el('span', null, 'Si le client annule aujourd’hui :'));
    p.appendChild(document.createTextNode(' '));
    p.appendChild(el('span', null, 'vous pourriez réclamer, sur justification, jusqu’à'));
    p.appendChild(document.createTextNode(' '));
    p.appendChild(el('strong', 'nc-forecast-amt', D.money(a.plafond)));
    p.appendChild(document.createTextNode(' ('));
  ...
  function ncReleaseBlock(entry) {
    ...
    var open = el('button', 'btn btn-sm btn-ghost nc-release-open', 'Un détail rend ce dossier impossible ? Me désister');
  ...
  function ncConsumeDeepAct(loaded) {
    // Wait for the authenticated response before deciding whether a link is
    // still available. Cached cards and filters are not authoritative.
    if (!loaded || !nc.token) return;
    var id = nc.deepAct; if (!id) return;
    var bid = ncFindOpen(id);
    var card = Array.prototype.find.call(document.querySelectorAll('#notary-retained-list .nc-card, #notary-open-list .nc-card'), function (node) { return node.dataset.id === id; });
    nc.deepAct = null;
    var notice = $('nc-offer-notice'); if (notice) notice.hidden = true;
    if (!card && !bid) {
      if (ncProfilIncomplet()) {
        nc.pendingRetain = id;
        ncProfilGate(null);
      } else if (notice) {
        ncSetText('nc-offer-notice-text', 'Cette demande n’est plus disponible pour votre profil. Choisissez une autre demande.');
        notice.hidden = false;
        var action = $('nc-offer-notice-dismiss');
    ...
    if (card) {
      if (card.classList.contains('is-retained')) ncSetRetainedExpanded(card, true);
      if (card.scrollIntoView) { try { card.scrollIntoView({ block: 'center', behavior: 'smooth' }); } catch (e) {} }
      retrigger(card, 'is-flash');
      card.addEventListener('animationend', function () { card.classList.remove('is-flash'); }, { once: true });
    }
    // Calendar/email links are already an explicit choice of this offer.
    // Open its review; only the confirmation button may actually accept it.
    if (bid && nc.token) {
      if (ncProfilIncomplet()) ncProfilGate(null, bid);
      else ncOpenRetainSheet(bid);
    }
  }
  ...
  function ncRenderEarnings() {
    ...
    if (own) {
      ...
      rrow.appendChild(el('span', 'help', ' Votre moyenne, lisible par vous seul. Aucun client ne la voit.'));
    ...
    // Le RELEVÉ fait foi quand il a répondu : il vient du registre ACT# et
    // couvre tout l'historique, alors que `e` se calcule sur le cache local,
    // semé par la fenêtre de 4 mois VERS L'AVANT du fil. Sur un navigateur
    // neuf, ce cache est vide et affichait « 0 $ » à un notaire chevronné.
    var t = nc.actsTotals;
    if (t) {
      e = {
        done: Number(t.actes) || 0,
        honoraires: Number(t.honoraires) || 0,
        prixNota: Number(t.prixNota) || 0,
        // « À compléter » reste une notion du FIL (des actes à venir), pas du
        // relevé (qui ne contient que du réglé) : le cache local garde ce rôle.
        pending: e.pending, pendingVal: e.pendingVal,
      };
    } else if (nc.actsError && !e.done) {
      // Une panne n'est pas « vous n'avez rien gagné ». Sans relevé ET sans
      // cache, on le dit — inventer un zéro serait le bogue qu'on corrige.
      box.appendChild(el('p', 'help', 'Vos honoraires sont momentanément indisponibles. Réessayez dans un instant — rien n’est perdu.'));
      if (e.pending) box.appendChild(el('p', 'help', e.pending + ' dossier' + (e.pending > 1 ? 's' : '') + ' à compléter · valeur estimée ' + D.money(e.pendingVal) + '.'));
      return;
    }
    ...
    if (e.pending) {
      var pending = el('p', 'help', e.pending + ' dossier' + (e.pending > 1 ? 's' : '') + ' à compléter · valeur estimée ' + D.money(e.pendingVal) + '. ');
      pending.appendChild(el('span', null, 'Vos honoraires vous sont virés à la signature, en entier.'));
  ...
  async function ncLoadActs() {
    ...
    try {
      ...
      nc.actsError = true; ncRenderEarnings();
      return;
    }
    if (r.status === 401) { ncExpire('Session expirée. Reconnectez-vous.'); return; }
    if (r.status === 404) {
      ...
      ncActsFor = nc.email;
      // Porte absente (déploiement plus ancien) : ce n'est pas une panne — les
      // tuiles retombent sur le cache local, qui vaut mieux que rien.
      nc.actsTotals = null; nc.actsError = false;
      clear(box);
    ...
    ncActsFor = nc.email;
    // Les totaux du RELEVÉ font foi : ils viennent du registre ACT#, écrit une
    // fois pour toutes, et couvrent tout l'historique. Le cache local, lui,
    // n'est semé que par la fenêtre de 4 mois VERS L'AVANT du fil — d'où un
    // « 0 $ » sur un navigateur neuf. On garde donc la réponse et on repeint
    // « Vos revenus » avec elle.
    nc.actsTotals = (j && j.totaux) || null;
    nc.actsError = false;
    ncRenderEarnings();
    ncRenderActs(j);
  }
  ...
  async function ncSaveProfil() {
    ...
    if (saved) saved.textContent = NC_PROFIL_SAVED;
    toast('Profil enregistré.');
    if (nc.pendingRetain && !ncProfilIncomplet()) {
      var selectedId = nc.pendingRetain;
      // Refresh availability and the matching rules after changing the profile.
      if (!(await ncLoadBids())) return;
      var selected = ncFindOpen(selectedId);
      if (selected && !ncProfilIncomplet()) ncOpenRetainSheet(selected);
      else if (!selected) {
        nc.pendingRetain = null;
        toast('Cette demande n’est plus disponible pour votre profil.');
      }
    }
  }
  ...
  function syncNavTabs(tab) {
    var tabs = Array.prototype.filter.call(document.querySelectorAll('.nav-tab'), function (t) { return !t.hidden; });
  ...
  function setTab(tab, opts) {
    opts = opts || {};
    // A door that needs an account does not open on a deep link either: the
    // visitor lands on the carnet with the sign-in sheet in front of them,
    // rather than on a pane that has nothing to show them. `gate: false` is
    // the one way through — it belongs to the sign-in sheet's own fallback,
    // which walks an engine without <dialog> to the identity form.
    if (opts.gate !== false && paneNeedsAccount(tab)) {
      openAuthModal('client', 'signin');
      tab = 'carnet';
    }
    var prev = state.tab;
    if (tab !== prev) {
      var prevIndex = PANES.indexOf(prev), nextIndex = PANES.indexOf(tab);
      document.documentElement.setAttribute('data-nav-direction', nextIndex >= prevIndex ? 'forward' : 'back');
    }
    state.tab = tab;
    if (tab !== 'carnet' && clientWalk) clientWalkStop();
    syncNavTabs(tab);
    ...
    // Le circuit du code ne joue que devant quelqu'un : il démarre à
    // l'ouverture de la porte et se gare de lui-même quand elle se referme.
    if (tab === 'partenaires') partnerFlow.restart(); else partnerFlow.stop();
  ...
  function renderCatalogueLd() {
    ...
    var offers = D.SERVICES.map(function (svc) {
      return {
        '@type': 'Offer', priceCurrency: 'CAD', price: String(prixAnnonceDollars(svc.id)),
        availability: 'https://schema.org/InStock',
        // LPC art. 224 c) : le prix annoncé est le total, honoraires de départ
        // du notaire et service de Nota compris. Taxes et débours en sus.
        description: 'Prix de départ pour cet acte dans la Ville de Québec, honoraires de départ du notaire et prix du service de Nota, publié d’avance, compris. Taxes et débours en sus. Le notaire reçoit 100 % de votre offre.',
  ...
  function salleActeClient() {
    ...
    return myOffers()
      .filter(function (o) {
        return o && o.id && o.dateISO && o.clientToken && o.retained && !o.cancelled && !o.completed
  ...
  function salleActeNotaire() {
    ...
    return ncRetainedFor(nc.email)
      .filter(function (e) { return e && e.id && e.dateISO && !e.completed && !e.cancelled && D.daysBetween(today, e.dateISO) >= 0; })
      .sort(function (a, b) { return String(a.dateISO).localeCompare(String(b.dateISO)); })[0] || null;
  }
  ...
  function wire() {
    ...
    if (tablist) tablist.addEventListener('keydown', function (e) {
      // A door closed to this visitor is not in the arrow order either: a
      // hidden tab cannot be focused, so landing on it would strand the walk.
      var tabs = Array.prototype.slice.call(tablist.querySelectorAll('.nav-tab')).filter(function (t) { return !t.hidden; });
    ...
    var pEmailEdit = $('partner-email-edit');
    if (pEmailEdit) pEmailEdit.addEventListener('click', function () {
      var email = $('partner-courriel');
    ...
    if (pMail) pMail.addEventListener('blur', function () {
      pMail.setAttribute('aria-invalid', pMail.value.trim() && !D.isEmail(pMail.value.trim()) ? 'true' : 'false');
    });
    ...
    // a click on the info control only exposes that surface for touch users.
    var betaNote = $('notary-ai-beta-note');
    ...
    $('acct-close').addEventListener('click', function () {
      toggleNotifPanel(false);
      var bell = $('notif-bell'); if (bell) { try { bell.focus(); } catch (e) {} }
    });
    ...
    var authSwitch = $('auth-switch-btn');
    if (authSwitch) authSwitch.addEventListener('click', authToggleMode);
    ...
    window.addEventListener('popstate', function () {
      var navState = history.state;
      if (navState && navState.nota && Number.isInteger(navState.notaIndex)) {
        navHistoryIndex = Math.max(0, Math.min(navHistory.length - 1, navState.notaIndex));
      } else {
        // A caller may have used history.pushState directly (shared-link tests,
        // integrations, or a browser extension). Keep the rail honest by
        // resolving the current URL against the entries we know.
        var currentUrl = location.href;
        var found = navHistory.findIndex(function (entry) { return entry.url === currentUrl; });
        if (found >= 0) navHistoryIndex = found;
      }
      var h = new URLSearchParams(location.hash.replace(/^#/, ''));
    ...
    $('o-service-chips').addEventListener('click', function (e) {
      var b = e.target.closest('.chip'); if (!b) return;
      // Un acte annoncé n'est pas une réponse : il ne prend pas la sélection.
    ...
    if (dayPick) {
      ...
      dayPick.addEventListener('change', async function () {
        var iso = this.value;
        if (!D.isISODate(iso) || iso < todayISO()) { this.value = state.selectedDate || ''; return; }
        if (iso === state.selectedDate) return;
        await openDay(iso, { preview: $('day-dialog').dataset.interaction === 'preview' });
        if ($('day-date-edit')) $('day-date-edit').open = false;
        // Later form screens have no visible service chip to focus. Keep the
        // keyboard at the date control instead of sending it into hidden content.
        if ($('day-dialog').dataset.interaction === 'offer' && bookAt > 1) {
          var dateSummary = document.querySelector('#day-date-edit > summary');
          if (dateSummary) dateSummary.focus();
        } else focusBookingStart();
      });
    }

    // Day booking dialog
    // Les quatre écrans : la barre d'action et le rail (ADR 0040).
    var bnext = $('book-next'); if (bnext) bnext.addEventListener('click', bookForward);
    var bback = $('book-back'); if (bback) bback.addEventListener('click', function () { bookGoTo(bookAt - 1); });
    var brail = $('book-rail');
    if (brail) brail.addEventListener('click', function (e) {
      var b = e.target && e.target.closest ? e.target.closest('.book-rail-b') : null;
      if (b && !b.disabled) bookGoTo(Number(b.dataset.goto));
    });
    bookPaint();
    ...
    $('contact-dialog').addEventListener('close', function () { track('contact_ferme'); });
    $('ct-message').addEventListener('input', function () {
      contactGrow();
      if (!contactMessageStarted && $('ct-message').value.trim()) {
        contactMessageStarted = true;
        track('contact_message_commence');
      }
    });
    ...
    $('ct-sujet').addEventListener('change', function () {
      if (!contactSubjectSyncing) track('contact_sujet_choisi');
    });
    ...
    // A new notary's signup CTA files their application — no Stripe on this
    // path (see ncFreeSignup). Payouts are connected later, from the console.
    ...
    if (ncSignup) ncSignup.addEventListener('click', function () {
      ...
      try { if (email) lsSave(LS_NC_EMAIL, email); } catch (e) {} // so the gate reopens on their address
      ncSignup.disabled = true; ncSignup.textContent = 'Envoi de votre inscription…';
      ncFreeSignup(email, function (msg) {
    ...
    var ncPendingBack = $('notary-pending-back');
    if (ncPendingBack) ncPendingBack.addEventListener('click', ncShowEmailStep);
    ...
    // « J'ai perdu mon code » : le rappel repart par courriel. La réponse de l'API
    // est volontairement la même pour une adresse partenaire et pour une autre,
    // donc l'écran ne peut rien affirmer de plus que « si vous êtes partenaire ».
    var partnerRappel = $('partner-rappel');
    if (partnerRappel) partnerRappel.addEventListener('click', async function () {
      var mail = $('partner-courriel');
      var val = mail ? mail.value.trim() : '';
      if (!val || !D.isEmail(val)) {
        toast('Entrez votre courriel ci-dessus, puis redemandez votre code.');
        if (mail) { try { mail.focus(); } catch (e) {} }
        return;
      }
      partnerRappel.disabled = true;
      try {
        var r = await fetch(API_BASE + '/partenaires/rappel', {
          method: 'POST', headers: { 'content-type': 'application/json' },
          body: JSON.stringify({ courriel: val }),
        });
        if (r.status === 429) toast(throttledMessage(r));
        else if (!r.ok) toast('Impossible d’envoyer le rappel pour l’instant.');
        else toast('Si ce courriel a un code partenaire, il vient de partir vers ' + val + '.');
      } catch (e) {
        toast('Hors ligne. Réessayez une fois en ligne.');
      }
      partnerRappel.disabled = false;
    });
    ...
    if ($('notary-filters-toggle')) $('notary-filters-toggle').addEventListener('click', function () {
      flagSet(LS_NC_FILTERS, ncFiltersExpanded() ? 'closed' : 'open');
      ncRenderFilterDisclosure();
    });
    if ($('notary-open-reset')) $('notary-open-reset').addEventListener('click', ncResetFilters);
    if ($('notary-open-prefs')) $('notary-open-prefs').addEventListener('click', function () {
      var panel = $('notary-prefs'); if (panel) panel.open = true;
      var first = document.querySelector('#pref-lenders button'); if (first) first.focus();
    });
    if ($('nc-offer-notice-dismiss')) $('nc-offer-notice-dismiss').addEventListener('click', function () {
      $('nc-offer-notice').hidden = true;
      ncResetFilters();
    });
    if (ncFilter) ncFilter.addEventListener('click', function (e) {
      var c = e.target.closest('.chip'); if (!c) return;
      var isReady = c.classList.contains('nc-chip-ready');
      if (isReady) nc.filter.readyOnly = !nc.filter.readyOnly;
      else nc.filter.service = c.dataset.svc || 'all';
      ncRenderOpen();
      var selected = isReady ? ncFilter.querySelector('.nc-chip-ready') : Array.from(ncFilter.querySelectorAll('[data-svc]')).find(function (x) { return x.dataset.svc === nc.filter.service; });
      if (selected && !ncFilter.hidden) selected.focus({ preventScroll: true });
    });
    ...
    if (ncOpenList) {
      ncOpenList.addEventListener('click', function (e) {
        ...
        if (tile) {
          ...
          ncRenderOpen();
          var selectedDay = Array.from(ncOpenList.querySelectorAll('.nc-daytile')).find(function (x) { return x.dataset.date === tile.dataset.date; });
          if (selectedDay && ncFiltersExpanded()) selectedDay.focus({ preventScroll: true });
          return;
        }
        ...
        if (e.target.closest('.nc-accept')) {
          if (ncProfilIncomplet()) ncProfilGate(null, b); else ncOpenRetainSheet(b);
        }
    ...
    if (ncRetList) {
      ncRetList.addEventListener('click', function (e) {
        ...
        if (e.target.closest('.nc-retained-toggle')) {
          ncSetRetainedExpanded(card, !card.classList.contains('is-expanded'));
          return;
        }
        if (e.target.closest('.nc-contact-start, .nc-contact-open')) {
          ncSetRetainedExpanded(card, true);
          var composer = card.querySelector('.chat-input');
          if (composer) {
            // Prepare an editable draft; the normal Send button is the only
            // action that transmits it. Never replace the notary's own draft.
            if (e.target.closest('.nc-contact-start') && !composer.value.trim()) {
              composer.value = T('Bonjour, j’ai retenu votre demande dans Nota. Quelle heure vous conviendrait pour la signature ? Nous pouvons aussi préciser les documents à préparer ici.');
    ...
    // Ordinary booking keeps its existing direct entry. The optional tour
    // starts only at the end of the product introduction or on replay.
  ...
  // The arrival door takes partners straight to the next real action. A
  // preference never creates a claim: only the existing verified flow does.
  function partnerStart() {
    roleSet('partner');
    onbMarkSeen();
    closeOpenDialogs();
    setTab('partenaires', { scroll: false });
    renderPartnerPane();
    var confirmed = partnerGet();
    var details = $('partner-details');
    if (details) details.open = !confirmed;
    var region = confirmed ? $('partner-success') : details;
    var target = confirmed ? $('partner-copy') : (!partnerState.type
      ? document.querySelector('#partner-type .chip') : $('partner-courriel'));
    if (region && region.scrollIntoView) region.scrollIntoView({ block: 'start', behavior: 'instant' });
  ...
  function driftBuild(host, id, variant, opts) {
    if (!host || $(id)) return;
    // Under prefers-reduced-motion the stylesheet freezes the dice; not
    // building the cubes at all also spares the DOM and the paint.
    if (reducedMotion()) return;
    // opts lets one caller (the intro gate) run a denser, more visible field
    // than the site-wide layer without touching that layer's own tuning —
    // defaults below reproduce the original intro field; the site-wide layer
    // asks for a denser but much quieter field below.
    opts = opts || {};
    var cols = opts.cols || 5, rows = opts.rows || 4;
    var oMin = opts.oMin != null ? opts.oMin : 0.04, oRange = opts.oRange != null ? opts.oRange : 0.05;
    var sMin = opts.sMin != null ? opts.sMin : 22, sRange = opts.sRange != null ? opts.sRange : 58;
    var bg = document.createElement('div');
    bg.id = id;
    bg.className = 'mark-drift' + (variant ? ' ' + variant : '');
    bg.setAttribute('aria-hidden', 'true');
    // A jittered grid spreads the marks evenly — no clumps, no bare corners —
    // while the jitter keeps the lattice invisible.
    var cellW = 100 / cols, cellH = 100 / rows;
    for (var i = 0; i < cols * rows; i++) {
      var m = document.createElement('i');
      var col = i % cols, row = (i / cols) | 0;
      m.style.setProperty('--x', (col * cellW + 2 + Math.random() * (cellW - 4)).toFixed(1) + '%');
      m.style.setProperty('--y', (row * cellH + 2 + Math.random() * (cellH - 4)).toFixed(1) + '%');
      ...
      var t = Math.random();
      var amp = 40 + t * 120;
      m.style.setProperty('--s', (sMin + t * sRange).toFixed(0) + 'px');
      m.style.setProperty('--o', (oMin + t * oRange).toFixed(3));
  ...
  // --- Le circuit du code (propriétaire, 2026-09-12 soir) --------------------
  // « Une animation qui explique clairement comment la section partenaire
  // fonctionne. » Un code, deux voies, une seule fin :
  //   • la voie du LIEN — le client clique, le champ « Code de référence » de
  //     la feuille de réservation est DÉJÀ rempli (refPrefillOfferField) ;
  //   • la voie du CODE DIT — le client tape le même code lui-même, à l'écran 4
  //     de cette feuille. Le champ montré ici est exactement celui-là, pour
  //     qu'on le reconnaisse une fois devant.
  // Les deux voies se rejoignent sur la même chaîne (publiée → retenue →
  // signée) et la même récompense — jamais « en quelques secondes » : le
  // versement suit la signature (vignettes-montrees.test.mjs tient la porte).
  // Rien n'est écrit en dur : le code est celui du visiteur, la récompense
  // vient du domaine, et le « 0 $ » du client est D.money(0).
  function makePartnerFlow() {
    var v = { timers: [], raf: null, running: false, waits: 0 };
    // Même patience bornée que la vignette de la semaine : la boucle est armée
    // au rendu, qui peut précéder l'instant où la figure est VISIBLE (film
    // d'accueil, lien direct). Une figure qu'on ne verra jamais ne sonde pas
    // indéfiniment.
    var WAIT_MS = 400, MAX_WAITS = 20;
    var LIT = ['pr-fl-seed', 'pr-fl-step-lien', 'pr-fl-link', 'pr-fl-card-lien',
      'pr-fl-step-code', 'pr-fl-said', 'pr-fl-card-code',
      'pr-fl-beat-publiee', 'pr-fl-beat-retenue', 'pr-fl-beat-signe',
      'pr-fl-end-vous', 'pr-fl-end-client'];
    ...
    function lightUp(id, on) { var n = $(id); if (n) n.classList.toggle('is-on', on !== false); }

    // Le code montré est celui que le visiteur a déjà sous les yeux : son code
    // confirmé, sinon celui qu'il est en train de taper, sinon l'exemple du
    // champ. Jamais un code inventé à côté du sien.
    function flowCode() {
      var rec = partnerGet();
      if (rec && rec.code) return rec.code;
      var inp = $('partner-code');
      var typed = inp ? inp.value.trim() : '';
      if (D.isReferralCode(typed)) return D.normalizeReferralCode(typed);
      return (inp && inp.placeholder) || 'EVEROY';
    }

    // Ce qui ne bouge pas d'un tour à l'autre : le code, le lien, la somme du
    // client. Rendu aussi en mouvement réduit — la figure n'est jamais vide.
    function paint(code) {
      var seed = $('pr-fl-seed-code'); if (seed) seed.textContent = code;
      var link = $('pr-fl-link');
      // Le lien sans son protocole : c'est ce qu'on lit dans un message, et
      // la moitié « https:// » ne dit rien de plus au partenaire.
      if (link) link.textContent = partnerShareLink(code).replace(/^https?:\/\//, '');
      var said = $('pr-fl-said'); if (said) said.textContent = '« ' + code + ' »';
      var cli = $('pr-fl-client-amount'); if (cli) cli.textContent = D.money(0);
    }
    function setField(id, text) { var n = $(id); if (n) n.textContent = text; }
    function reset() {
      LIT.forEach(function (id) { lightUp(id, false); });
      setField('pr-fl-field-lien', '');
      setField('pr-fl-field-code', '');
      // La récompense reste VIDE jusqu'à la signature : un « 0 $ » en attente,
      // même éteint, se lit comme une promesse à zéro. Sa place est tenue par
      // la hauteur minimale du chiffre (.pr-fl-end-amt).
      setField('pr-fl-amount', '');
    }

    // La frappe du client, lettre par lettre — la seule différence visible
    // entre les deux voies, et toute la raison d'en montrer deux.
    function typeCode(id, code, step) {
      var i = 0;
      (function next() {
        if (!onScreen()) { stop(); return; }
        i++;
        setField(id, code.slice(0, i));
        if (i < code.length) later(next, step);
      })();
    }

    // La récompense monte jusqu'au chiffre du domaine ; un minuteur garantit le
    // chiffre FINAL là où rAF est suspendu (onglet en arrière-plan).
    function tween(to) {
      var out = $('pr-fl-amount'); if (!out) return;
      if (v.raf) cancelAnimationFrame(v.raf);
      var t0 = performance.now(), dur = 700;
      (function frame(t) {
        var k = Math.min(1, ((t || performance.now()) - t0) / dur);
        k = 1 - Math.pow(1 - k, 3);
        out.textContent = D.money(Math.round(to * k));
        v.raf = k < 1 ? requestAnimationFrame(frame) : null;
      })(t0);
      later(function () { out.textContent = D.money(to); }, dur + 120);
    }
    ...
    function cycle() {
      ...
      var code = flowCode();
      reset(); paint(code);
      // Le minutage de la scène, une fois pour toutes : ce que la lecture
      // demande, pas ce que le code rend facile.
      lightUp('pr-fl-seed');
      later(function () { lightUp('pr-fl-step-lien'); }, 500);
      later(function () { lightUp('pr-fl-link'); }, 850);
      later(function () { lightUp('pr-fl-card-lien'); setField('pr-fl-field-lien', code); }, 1250);
      later(function () { lightUp('pr-fl-step-code'); }, 1650);
      later(function () { lightUp('pr-fl-said'); }, 1950);
      later(function () { lightUp('pr-fl-card-code'); typeCode('pr-fl-field-code', code, 110); }, 2350);
      later(function () { lightUp('pr-fl-beat-publiee'); }, 3300);
      later(function () { lightUp('pr-fl-beat-retenue'); }, 3900);
      later(function () { lightUp('pr-fl-beat-signe'); }, 4500);
      later(function () { lightUp('pr-fl-end-vous'); tween(D.REFERRAL.client); }, 5100);
      later(function () { lightUp('pr-fl-end-client'); }, 5500);
      // Le temps de lire la fin avant de recommencer.
      later(cycle, 9500);
    }

    function render() {
      var box = $('pr-flow'); if (!box) return;
      var code = flowCode();
      // Mouvement réduit : l'état FINAL d'un coup, jamais une bande vide.
      if (reducedMotion()) {
        stop(); reset(); paint(code);
        LIT.forEach(function (id) { lightUp(id); });
        setField('pr-fl-field-lien', code);
        setField('pr-fl-field-code', code);
        var amt = $('pr-fl-amount'); if (amt) amt.textContent = D.money(D.REFERRAL.client);
        return;
      }
      if (v.running) return;
      v.running = true; v.waits = 0;
      cycle();
    }

    var api = { render: render, restart: function () { stop(); render(); }, stop: stop };
    // Une porte ouverte dans un onglet d'arrière-plan : la boucle épuise sa
    // patience et se gare, et le visiteur qui revient trouverait une scène à
    // moitié éteinte. Le retour à l'écran la relance depuis le début.
    document.addEventListener('visibilitychange', function () {
      if (!document.hidden && state.tab === 'partenaires') api.restart();
    });
    return api;
  }
  var partnerFlow = makePartnerFlow();
  ...
  function renderPartnerPane() {
    ...
    if (rec) {
      ...
      var email = $('partner-courriel');
      if (email && !email.value && D.isEmail(profileGet().courriel)) {
        email.value = profileGet().courriel;
    ...
    // Le circuit : montré ici, sous la promesse, une fois le code connu.
    partnerFlow.render();
  }
  ...
  function partnerNextStepRender() {
    var hint = $('partner-next-step'), submit = $('partner-submit');
    if (!hint || !submit) return;
    var mail = $('partner-courriel'), code = $('partner-code');
    var message;
    if (submit.getAttribute('aria-busy')) message = 'Envoi du lien d’activation…';
    else if (submit.dataset.state === 'sent') message = 'Ouvrez le lien reçu par courriel pour activer votre code.';
    else if (submit.dataset.state === 'claimed') message = 'Votre lien est prêt à partager.';
    else if (!partnerState.type) message = 'Choisissez votre métier pour commencer.';
    else if (!mail || !mail.value.trim()) message = 'Entrez votre courriel professionnel. Votre code sera suggéré automatiquement.';
    else if (!D.isEmail(mail.value.trim())) message = 'Vérifiez votre adresse courriel pour recevoir le lien d’activation.';
    else if (!code || !D.isReferralCode(code.value.trim())) message = 'Choisissez un code valide pour créer votre lien Nota.';
    else message = 'Tout est prêt. Un lien par courriel vous permettra d’activer votre code.';
    // Avoid reannouncing the same instruction on every keystroke, including
    // after the DOM has been translated to English.
    if (hint.dataset.message !== message) {
      hint.dataset.message = message;
      hint.textContent = message;
    }
    hint.dataset.ready = !submit.disabled && !submit.dataset.state ? 'true' : 'false';
  }
  ...
  function partnerValidateUI() {
    ...
    if (mailInp && mailOk) mailInp.removeAttribute('aria-invalid');
    ...
    if (!submit.getAttribute('aria-busy') && submit.dataset.state) {
      ...
      submit.textContent = 'Recevoir mon lien d’activation →';
      var succ = $('partner-success'); if (succ) succ.hidden = true;
      var pend0 = $('partner-pending'); if (pend0) pend0.hidden = true;
    }
    submit.disabled = !!submit.getAttribute('aria-busy') || !(partnerState.type && mailOk && okCode);
    partnerNextStepRender();
  }
  ...
  async function onPartnerSubmit(e) {
    ...
    submit.disabled = true; submit.setAttribute('aria-busy', 'true'); submit.textContent = 'Envoi du lien d’activation…';
    partnerNextStepRender();
    var r = null;
    try {
    ...
    var destination = $('partner-pending-email'); if (destination) destination.textContent = courriel;
    partnerNextStepRender();
  ...
  function partnerClaimConfirmed(saved, hint) {
    ...
    var activeCode = $('partner-active-code'); if (activeCode) activeCode.textContent = code;
    var details = $('partner-details'); if (details) details.open = false;
    var title = $('partner-form-title'); if (title) title.textContent = 'Votre code partenaire';
    var pane = $('pane-partenaires');
    if (pane) {
      pane.dataset.partnerState = 'confirmed';
      var heading = pane.querySelector('.pr-hero h1'); if (heading) heading.textContent = 'Votre lien est prêt à partager.';
      var intro = pane.querySelector('.pr-hero-copy > p'); if (intro) intro.textContent = 'Retrouvez votre code et un message à copier.';
    }
    ...
    if (state.tab === 'profil') renderProfil();
    // Le circuit rejoue avec le VRAI code, pas l'exemple du champ.
    partnerFlow.restart();
  }
  ...
  function partnerFail(msgs) {
    var details = $('partner-details'); if (details) details.open = true;
    var errs = $('partner-errors');
    if (errs) {
    ...
    var pend = $('partner-pending'); if (pend) pend.hidden = true;
    var submit = $('partner-submit');
    if (submit) { submit.removeAttribute('aria-busy'); delete submit.dataset.state; submit.textContent = 'Recevoir mon lien d’activation →'; }
    partnerValidateUI();
  }
  ...
  var IG_MAX_PLAYS = 2;
  var IG_SCENE_MS = 3500;
  var IG_FILM_MS = 14400;
  var igTimer = null;
  ...
  var BACKDROP_SELECTORS = ['.skip', '.site-header', '#main', '.site-footer', '#chat-wrap', '.guide-fab'];
  ...
  function igPlay(film) {
    if (film === 'client') {
      var dates = $('ig-client-dates');
      if (dates) {
        clear(dates);
        var chosen = firstStandardDate();
        var first = chosen.slice(0, 8) + '01';
        var firstDate = new Date(first + 'T00:00:00Z');
        var start = D.addDays(first, -((firstDate.getUTCDay() + 6) % 7));
        var end = new Date(Date.UTC(firstDate.getUTCFullYear(), firstDate.getUTCMonth() + 1, 0)).toISOString().slice(0, 10);
        var count = Math.ceil((D.daysBetween(start, end) + 1) / 7) * 7;
        $('ig-client-month').textContent = monthTitle(chosen);
        for (var day = 0; day < 7; day++) dates.appendChild(el('span', 'ig-client-weekday', weekdayShort(D.addDays(start, day))));
        for (var offset = 0; offset < count; offset++) {
          var iso = D.addDays(start, offset);
          var tile = el('span', 'ig-client-date' + (iso === chosen ? ' ig-client-picked' : '')
            + (monthKey(iso) !== monthKey(chosen) ? ' ig-client-outside' : ''));
          tile.appendChild(el('strong', '', String(Number(iso.slice(8, 10)))));
          dates.appendChild(tile);
        }
      }
    }
    ...
    stage.style.setProperty('--ig-seek-offset', '0ms');
    var seek = stage.querySelector('.ig-progress-seek');
    if (seek) {
      seek.value = '0';
    ...
    igRemaining = IG_FILM_MS; // matches the CSS film timeline: 4 beats x 3.5s + a short grace on the last one
    igResumeTimer();
    // Focus follows the film to its direct route into the product.
    try { $('ig-skip').focus(); } catch (e) {}
  }
  function igSeekScene(index) {
    if (!igActiveStage) return;
    index = Math.max(0, Math.min(3, Number(index) || 0));
    var stage = igActiveStage;
    var wasPaused = stage.classList.contains('is-paused');
    var offset = index * IG_SCENE_MS;
    clearTimeout(igTimer);
    stage.style.setProperty('--ig-seek-offset', offset + 'ms');
    var seek = stage.querySelector('.ig-progress-seek');
    if (seek) {
      seek.value = String(index);
      seek.setAttribute('aria-valuenow', String(index));
    }
    // Reflow restarts the CSS timeline with the requested scene at time zero.
    // Keeping the paused class means a paused film stays paused after seeking.
    stage.classList.remove('run');
    void stage.offsetWidth;
    stage.classList.add('run');
    igRemaining = Math.max(500, IG_FILM_MS - offset);
    if (wasPaused) stage.classList.add('is-paused');
    else igResumeTimer();
  }
  ...
  function igDismiss(tab, explicit) {
    clearTimeout(igTimer);
    var gate = $('intro-gate');
    if (!gate || gate.hidden || gate.classList.contains('ig-out')) return;
    if (tab === 'carnet' || tab === 'notaires') roleSet(tab === 'notaires' ? 'notary' : 'client');
    ...
    var guideClient = tab === 'carnet' && igActiveStage && igActiveStage.id === 'ig-stage-client';
    setTimeout(function () {
      gate.hidden = true; gate.classList.remove('ig-out');
      if (guideClient && !$('day-dialog').open) clientWalkStart(false);
    }, 320);
    document.body.classList.remove('ig-open');
    document.documentElement.classList.remove('ig-open');
  ...
  // L'ARRIVÉE MONTRE LE CARNET (propriétaire, 2026-09-12) — « je suis certain
  // que l'on peut faire beaucoup mieux ». La porte ne promet plus, elle
  // démontre : une date par palier d'avis du domaine, au prix TOTAL que la
  // case du calendrier affiche pour le même jour (tierFromLabel, une seule
  // formule, ADR 0042). L'échelle se lit de gauche à droite — signer demain
  // coûte ce que coûte demain, et la date calme est marquée comme telle.
  // Rien n'est écrit à la main : le jour où l'échelle du domaine change, la
  // vitrine change avec elle.
  function igCarnetDates() {
    var today = todayISO();
    var seen = {};
    var out = [];
    // Le premier jour où chaque palier s'applique, lu sur la même échelle que
    // le calendrier — jamais un compte de jours en dur.
    for (var n = 0; n <= 400 && out.length < D.TIERS.length; n++) {
      var id = D.tierForDays(n);
      if (seen[id]) continue;
      seen[id] = true;
      out.push({ iso: D.addDays(today, n), tier: D.tierById(id) });
    }
    return out.filter(function (d) { return d.tier; });
  }
  function igRenderCarnet() {
    var row = $('ig-carnet-row'); if (!row) return;
    clear(row);
    var svc = carnetService();
    var anchor = firstStandardDate();
    igCarnetDates().forEach(function (d) {
      var cell = el('button', 'ig-cal-cell');
      cell.type = 'button';
      cell.dataset.date = d.iso;
      cell.dataset.tier = d.tier.id;
      if (d.iso === anchor) cell.dataset.anchor = 'true';
      var when = el('span', 'ig-cal-when');
      when.appendChild(el('span', 'ig-cal-dow', weekdayShort(d.iso)));
      when.appendChild(el('span', 'ig-cal-day', String(Number(d.iso.slice(8, 10)))));
      when.appendChild(el('span', 'ig-cal-month', monthShort(d.iso)));
      cell.appendChild(when);
      cell.appendChild(el('span', 'ig-cal-price', tierFromLabel(d.tier.id, svc)));
      cell.appendChild(el('span', 'ig-cal-tier', T(tierName(d.tier))));
      cell.setAttribute('aria-label', dayTitle(d.iso) + ' — ' + tierFromLabel(d.tier.id, svc)
        + ' en ' + T(svc.nom).toLowerCase() + '.');
      // Une tuile mène à SA date : le carnet s'ouvre sur ce jour, ce qui met
      // la première offre à trois gestes de l'arrivée.
      cell.addEventListener('click', function () {
        igDismiss('carnet', true);
        openDay(d.iso);
      });
      row.appendChild(cell);
    });
    // Les chiffres de la bande sortent de state.monthBids : si le mois est
    // retombé sur les fixtures, elle le dit comme toutes les autres régions.
    renderDemoState();
  }
  ...
  function igMaybeShow() {
    ...
    $('ig-door-partner').addEventListener('click', function () {
      if (gate.hidden || gate.classList.contains('ig-out')) return;
      igDismiss('partenaires', true);
      partnerStart();
    });
    $('ig-partner-action').textContent = partnerGet() ? 'Partager mon code' : 'Créer et partager mon code';
    ...
    $('ig-enter').addEventListener('click', function () { igDismiss('carnet', true); });
    ...
    gate.querySelectorAll('.ig-progress-seek').forEach(function (input) {
      input.addEventListener('input', function () {
        if (input.closest('.ig-stage') === igActiveStage) igSeekScene(input.value);
      });
    });
    document.addEventListener('keydown', function (e) {
      if (gate.hidden || gate.classList.contains('ig-out')) return;
      if (e.key === 'Escape') {
        e.preventDefault();
        igDismiss(igActiveStage ? $('ig-skip').dataset.tab : null, true);
      } else if (e.key === 'Tab') {
        // Only the current scene and controls participate. The CSS timeline
        // hides future/past scenes with visibility, not the hidden attribute.
        var buttons = Array.from(gate.querySelectorAll('button:not([disabled]), .ig-progress-seek')).filter(function (button) {
          return !button.closest('[hidden]') && getComputedStyle(button).visibility !== 'hidden' && getComputedStyle(button).display !== 'none';
        });
        // The seek bar lives in the stage for layout, but follows Pause in the
        // keyboard sequence so the playback controls read as one cluster.
        var seekControl = buttons.find(function (button) { return button.classList.contains('ig-progress-seek'); });
        var pauseControl = buttons.find(function (button) { return button.id === 'ig-pause'; });
        if (seekControl && pauseControl) {
          buttons.splice(buttons.indexOf(seekControl), 1);
          buttons.splice(buttons.indexOf(pauseControl) + 1, 0, seekControl);
        }
        var first = buttons[0], last = buttons[buttons.length - 1];
        if (!first) return;
        if (!buttons.includes(document.activeElement) || (e.shiftKey ? document.activeElement === first : document.activeElement === last)) {
          e.preventDefault();
          (e.shiftKey ? last : first).focus();
        }
      }
    });
    // A visible logo field gives the arrival screen the same identity as the
    // marketplace. Six columns across three rows add three more marks than the
    // earlier field without turning the chooser into wallpaper. Each face is
    // the shared Nota N mark (never a placeholder X); the panel and its
    // decisions remain the foreground.
    igRenderCarnet();
    gate.hidden = false;
    driftBuild(gate, 'ig-logo-field', 'mark-drift--intro', {
      cols: 6, rows: 3, oMin: 0.038, oRange: 0.05, sMin: 30, sRange: 46
    });
    document.body.classList.add('ig-open');
    document.documentElement.classList.add('ig-open');
    setBackdropInert(true);
    // A known partner can return to their code without repeating the choice.
    // Everyone else begins at the client door, inside the modal.
    try { $(roleGet() === 'partner' ? 'ig-door-partner' : 'ig-door-client').focus(); } catch (e) {}
    return true;
  }
  ...
  // polled every 3 s so the operator's reply lands live; closed, a thread keeps
  // a slow 30 s watch for a day so the FAB can light its unread dot. The
  // operator answers in the admin inbox or through the optional signed link
  // (#reponse=<token>); the visitor's optional emailed copy links to
  ...
  var chatMessageStarted = false;
  // ADR 0046 — l'ancienne ligne (« on vous répond en général en quelques
  // minutes ») promettait un délai que RIEN dans le code ne tient : c'est
  // l'affirmation que l'audit a marquée invérifiable, et elle a disparu. Le
  // vide qu'elle occupait est maintenant rempli par les questions d'amorce,
  // qui disent ce que l'assistant sait vraiment répondre.
  var CHAT_TYPING_TEXT = 'L’assistant écrit…';
  var CHAT_ESCALADE_TEXT = 'Une personne prendra le relais ici. Vous pouvez revenir dans ce navigateur pour lire sa réponse.';
  ...
  // Qui a parlé. L'assistant porte SON nom : la voix est celle de la maison,
  // mais un visiteur ne doit jamais croire qu'une personne lui a écrit — et
  // quand une personne écrit vraiment, la différence doit se voir.
  function chatLabelFor(de, me) {
    if (de === D.SUPPORT_FROM.ASSISTANT) return 'Assistant Nota';
  ...
  function chatRenderMessages(log, messages, me) {
    ...
    if (!messages.length) {
      // Un fil vide n'affiche plus de phrase : ce sont les questions d'amorce
      // qui occupent la place, et elles, elles mènent quelque part. Le journal
      // doit être vide LUI AUSSI : un sondage qui revient sans message (une
      // réponse partielle, un fil purgé) ne doit pas faire réapparaître les
      // amorces au-dessus d'une conversation déjà commencée.
      if (log.id === 'chat-log' && !log.querySelector('.sup-msg')) chatSuggestShow(true);
      return;
    }
    if (empty) empty.parentNode.removeChild(empty);
    if (log.id === 'chat-log') chatSuggestShow(false);
    var known = {};
    ...
    if (added) chatGroupBubbles(log);
    if (added && atBottom) log.scrollTop = log.scrollHeight;
  }
  // Deux messages de suite du même côté forment un bloc : le nom ne se répète
  // pas, seule l'heure du dernier reste. Un fil de six lignes cesse d'être six
  // fois « Vous · 15:33 » et redevient une conversation.
  function chatGroupBubbles(log) {
    var rows = log.querySelectorAll('.sup-msg');
    for (var i = 0; i < rows.length; i++) {
      var prev = i > 0 ? rows[i - 1] : null;
      var next = i + 1 < rows.length ? rows[i + 1] : null;
      rows[i].classList.toggle('is-cont', !!prev && prev.dataset.de === rows[i].dataset.de);
      rows[i].classList.toggle('is-mid', !!next && next.dataset.de === rows[i].dataset.de);
    }
  }

  // --- Les questions d'amorce, et le temps d'attente (ADR 0046) --------------
  // Le panneau s'ouvrait sur un vide et une promesse de délai. Il s'ouvre
  // maintenant sur quatre questions que l'assistant sait traiter — bâties
  ...
  // « L'assistant écrit… » — une vraie ligne dans le fil, du côté de
  // l'assistant, retirée dès que la réponse arrive. Sans elle, l'envoi
  // paraissait tomber dans le vide pendant les secondes du modèle.
  function chatTyping(on) {
    var log = $('chat-log');
    if (!log) return;
    var row = log.querySelector('.sup-typing');
    if (!on) { if (row) row.parentNode.removeChild(row); return; }
    if (row) return;
    row = el('div', 'sup-msg sup-typing');
    row.dataset.de = D.SUPPORT_FROM.ASSISTANT;
    var b = el('div', 'sup-bubble sup-bubble-typing');
    b.setAttribute('aria-label', CHAT_TYPING_TEXT);
    for (var i = 0; i < 3; i++) b.appendChild(el('span', 'sup-dot'));
    row.appendChild(b);
    log.appendChild(row);
    log.scrollTop = log.scrollHeight;
  }
  // L'escalade, dite en clair : le visiteur doit savoir qu'une PERSONNE a
  // désormais la question, et pourquoi son courriel compte à cet instant.
  function chatEscalade(on) {
    var note = $('chat-escalade');
  ...
  async function chatRefresh() {
    ...
    try {
      ...
      if (messages.some(function (m) {
        return (m.de === D.SUPPORT_FROM.NOTA || m.de === D.SUPPORT_FROM.ASSISTANT) && chatNewer(m.createdAt, current.seenAt);
      })) track('messagerie_reponse_recue');
  ...
  function chatToggle(open, opts) {
    ...
    if (show && panel.hidden) {
      chatMessageStarted = false;
      track('messagerie_ouverte');
    }
    if (!show && !panel.hidden) track('messagerie_fermee');
    panel.hidden = !show;
  ...
  // Email is an explicit optional copy, opened only by the visitor.
  function chatMailOpen(open, opts) {
    var row = $('chat-courriel-row'), btn = $('chat-courriel-open');
    if (!row || !btn) return;
    var wasHidden = row.hidden;
    var show = open != null ? open : row.hidden;
    row.hidden = !show;
    btn.setAttribute('aria-expanded', show ? 'true' : 'false');
    if (show && wasHidden) track('messagerie_courriel_ouvert');
    if (show && !(opts && opts.noFocus)) {
  ...
  async function chatSaveEmail() {
    ...
    if (res && res.status === 200) track('messagerie_courriel_enregistre');
  ...
  async function chatSend() {
    ...
    if (texte && !chatMessageStarted) {
      chatMessageStarted = true;
      track('messagerie_message_commence');
    }
    ...
    // elle s'affiche sans attendre le prochain sondage, et c'est ce qui fait
    // la différence entre une messagerie vivante et une boîte à lettres.
    var r = res.json.reponse || null;
    if (r) track('messagerie_reponse_recue');
    ...
    if (log) {
      ...
      if (m) arrivee.push(m);
      if (r) arrivee.push(r);
      if (arrivee.length) chatRenderMessages(log, arrivee, D.SUPPORT_FROM.VISITEUR);
    }
    // Handoffs continue in the chat; email remains an explicit optional copy.
    chatMailControls();
  ...
  function supportBoot() {
    ...
    var human = $('chat-human');
    if (human) human.addEventListener('click', function () {
      var input = $('chat-text');
      if (chatSending || !input) return;
      if (input.value.trim()) {
        var error = $('chat-error'); error.textContent = T('Envoyez ou effacez votre brouillon avant de demander une personne.'); error.hidden = false; input.focus(); return;
      }
      var topic = D.SUPPORT_TOPICS.find(function (item) { return item.id === 'humain'; });
      input.value = LOCALE.indexOf('en') === 0 ? topic.en : topic.fr;
      chatSend();
    });
    ...
    if (text) {
      ...
      text.addEventListener('input', function () {
        chatAutoGrow(text); chatCount();
        if (!chatMessageStarted && text.value.trim()) {
          chatMessageStarted = true;
          track('messagerie_message_commence');
        }
      });
  ...
  async function boot() {
    ...
    // More, smaller marks make the Nota identity read as atmosphere rather
    // than as a foreground illustration. Keep the logo faces legible only on
    // a close look: the content remains the visual priority.
    driftBuild(document.body, 'site-bg', 'mark-drift--site', {
      cols: 6, rows: 4, oMin: 0.012, oRange: 0.018
    });
    ...
    wire();
    wireCalendarSubscriptionDetails();
    wireCarnetSubscribe();
    enhanceSelects();
    supportBoot();
    ...
    await refreshMonthData();
    renderActiveView();
    // Le mois réel vient d'arriver : la vitrine de la porte se recalibre
    // dessus, exactement comme le calendrier derrière elle.
    if (igShown) igRenderCarnet();
  ...
  window.Nota = {
    ...
    contact: { open: openContactDialog, openEmail: openEmailContactDialog },

...
(function (root) {
  ...
  var C = B.colors, bid = B.demoOffer, duration = 18;
  var W=1120,H=630;
  ...
  function render(time,lang) {
    ...
    var step=s<4?0:s<10.5?1:s<15.5?2:3;
    var titles=['Le notaire ajoute Nota à son agenda.','Choisissez une offre dans votre agenda.','Vérifiez et confirmez dans Nota.','Le client a maintenant son notaire.'];
    var notes=['Google Agenda · Outlook · Apple Calendrier','Mises à jour selon la synchronisation de votre agenda.','Le lien ouvre directement cette demande après connexion.','La demande est retenue. Les échanges peuvent commencer.'];
    var out='<svg xmlns="http://www.w3.org/2000/svg" width="'+W+'" height="'+H+'" viewBox="0 0 '+W+' '+H+'">';
    // The surrounding page already carries Nota's identity. Keep this frame
    // for the experience, with one short instruction above the actual screens.
    out+=rect(0,0,W,H,C.lightBg,12);
    out+=text(24,38,T(titles[step]),25,C.lightInk,700,true);
    out+=text(24,64,T(notes[step]),13,C.lightMuted);
    out+=rect(24,76,1072,1,C.lightBorder);
    out+='<g>';

    if(step===0) {
      var sub=B.captures[lang].subscribe,subScale=Math.min(530/sub.width,410/sub.height),subX=24;
      out+=shot('subscribe',lang,subX,115,subScale,sub.height*subScale);
      ...
      out+=rect(586,101,510,488,C.lightSurface,12,C.lightBorder);
      out+=text(608,140,T('Septembre 2026'),20,C.lightInk,700,true);
      var miniDays=lang==='en'?['M','T','W','T','F','S','S']:['L','M','M','J','V','S','D'];
      for(var md=0;md<7;md++) out+=text(623+md*67,170,miniDays[md],12,C.lightMuted,600);
      for(var mi=0;mi<35;mi++) {
        var mx=607+(mi%7)*67,my=183+Math.floor(mi/7)*76,mdate=new Date(Date.UTC(2026,7,31+mi));
        out+=rect(mx,my,67,76,C.lightSurface,0,C.lightBorder);
      ...
      if(action&&s>1.2) out+=pointer(mix(subX+sub.width*subScale+45,subX+action.x*subScale,progress),mix(430,115+action.y*subScale,progress),s>2.7&&s<3.3?(s-2.7)/.6:0);
      if(s>3.2) out+=group(text(24,604,T('Confirmez l’ajout dans votre calendrier.'),14,C.lightMuted,600),ease((s-3.2)/.3));
    }

    if(step===1) {
      var gx=24,gy=160,cw=153.142857,rh=87;
      out+=text(24,120,T('Septembre 2026'),20,C.lightInk,700,true);
      out+=rect(856,99,236,28,C.lightSurface,8,C.lightBorder)+'<path d="M867 113l4 4 7-8" fill="none" stroke="'+C.brand+'" stroke-width="2" stroke-linecap="round"/>'+text(886,118,T('Demandes Nota'),13,C.lightMuted,600);
      var days=lang==='en'?['MON','TUE','WED','THU','FRI','SAT','SUN']:['LUN.','MAR.','MER.','JEU.','VEN.','SAM.','DIM.'];
      days.forEach(function(day,i) { out+=text(gx+i*cw+61,148,day,11,C.lightMuted,600); });
      out+='<defs><clipPath id="month"><rect x="'+gx+'" y="'+gy+'" width="1072" height="435" rx="12"/></clipPath></defs><g clip-path="url(#month)">';
      var selected;
      for(var i=0;i<35;i++) {
      ...
      out+='</g>'+rect(gx,gy,1072,435,'none',12,C.lightBorder);
      if(s>5.5&&s<7.7) out+=group(rect(727,99,115,28,C.lightSurface,8,C.lightBorder)+text(739,118,T('Nouvelle offre'),12,C.brand,600),ease((s-5.5)/.3));
    ...
    if(step===2) {
      var confirm=B.captures[lang].confirm,confirmScale=1072/confirm.width,cx=24,cy=95,reviewHeight=512;
      if(confirm.footer) {
        ...
        var footerH=confirm.footer.height*confirmScale,readingH=reviewHeight-footerH;
        ...
        confirmScale=Math.min(1072/confirm.width,reviewHeight/confirm.height);cx=(W-confirm.width*confirmScale)/2;
        out+=shot('confirm',lang,cx,cy,confirmScale);
      }
      if(s>13.3&&confirm.action) {
        var p4=ease((s-13.3)/.9),tx=cx+confirm.action.x*confirmScale,ty=confirm.footer?cy+reviewHeight-(confirm.height-confirm.action.y)*confirmScale:cy+confirm.action.y*confirmScale;
        out+=pointer(mix(tx+160,tx,p4),mix(ty-75,ty,p4),s>14.7?(s-14.7)/.6:0);
      }
    }

    if(step===3) {
      // The real retained file proves the outcome of the previous click.
      // Its crop starts below the application header, which is already visible
      // around the embedded video on the page.
      var retained=B.captures[lang].retained,retainedScale=1072/retained.width;
      out+=shot('retained',lang,24,95,retainedScale,Math.min(512,retained.height*retainedScale));
    }

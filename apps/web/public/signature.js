/* Nota signing beta. No third-party scripts; no legal signature.
...
(function () {
  ...
  var EN = {
    ...
    'Chiffré de bout en bout':'Encrypted end to end','Audio et vidéo chiffrés de bout en bout : Nota ne peut pas les lire, et un relais fourni par Nota peut les acheminer lorsque la connexion directe est impossible. Aucun enregistrement de la séance par Nota.':'Audio and video encrypted end to end: Nota cannot read them, and a relay provided by Nota may carry them when a direct connection is impossible. No recording of the session by Nota.','Entre vous deux':'Between the two of you','Audio et vidéo chiffrés entre les navigateurs avec WebRTC.':'Audio and video encrypted between browsers with WebRTC.','Audio et vidéo chiffrés entre les navigateurs avec WebRTC. Aucun enregistrement de la séance par Nota.':'Audio and video are encrypted between browsers with WebRTC. Nota does not record the session.','Aucun enregistrement':'No recording','Le droit de faire pause':'The right to pause','Une question, une interruption ou un doute? La signature de test s’arrête. Votre notaire décide de la reprise.':'A question, an interruption or a concern? Test signing stops. Your notary decides when to resume.','Des limites explicites':'Clear boundaries','La bêta sert aux essais. Elle ne produit pas d’acte authentique et ne représente pas une approbation de la Chambre des notaires.':'This beta is for testing. It does not produce an authentic deed or represent approval by the Chambre des notaires.',
    'VOTRE RENDEZ-VOUS NOTA':'YOUR NOTA APPOINTMENT','Un moment important. Toute votre attention.':'An important moment. Your full attention.','Copier l’invitation':'Copy invitation','Plein écran':'Full screen','Quitter le plein écran':'Exit full screen','BÊTA · ESSAI':'BETA · REHEARSAL','Cette séance est une répétition. Document fictif uniquement. La signature notariale officielle est à venir.':'This session is a rehearsal. Fictional document only. Official notarial signing is coming soon.','Préparation':'Preparation','Vérification':'Verification','Lecture ensemble':'Read together','Signature de test':'Test signing','Reçu':'Receipt',
  ...
  function applyLanguage() {
    ...
    if (languageToggle) {
      languageToggle.setAttribute('aria-label', lang === 'en' ? 'Language' : 'Langue');
      languageToggle.querySelectorAll('[data-set-lang]').forEach(function (button) {
        var buttonLang = button.getAttribute('data-set-lang') === 'en' ? 'en' : 'fr';
        button.setAttribute('aria-pressed', buttonLang === lang ? 'true' : 'false');
        button.setAttribute('aria-label', buttonLang === 'en' ? 'English' : 'Français');
        button.setAttribute('lang', buttonLang === 'en' ? 'en-CA' : 'fr-CA');
      });
    }
  ...
  $('language-toggle').querySelectorAll('[data-set-lang]').forEach(function (button) {
    button.addEventListener('click', function () {
      var next = button.getAttribute('data-set-lang') === 'en' ? 'en' : 'fr';
      if (next === lang) return;
      lang = next; applyLanguage(); renderEvents();
    });
  });

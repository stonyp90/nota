...
test('widget, local admin and both email participants keep one transcript through retries and reopening', async () => {
  ...
  assert.equal(reopened.thread.messages.length, 4);
  assert.equal(reopened.thread.messages[3].texte, 'Comment préparer mes documents?');
  ...
  assert.deepEqual(widget.messages.map(message => message.de), ['visiteur', 'assistant', 'nota', 'visiteur', 'nota']);
  assert.equal(widget.messages[4].texte, 'Le notaire vous indiquera les documents requis.');

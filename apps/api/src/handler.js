...
function createApp(repo, opts = {}) {
  ...
  async function handle(request) {
    ...
    if (route === '/stripe/webhook' && method === 'POST') {
      ...
      // Fire the matching account lifecycle email (activation welcome/win-back
      // + operator alert). Best-effort; skipped on a redelivered event.
      const n = notifier();
      if (n && result.event && !result.duplicate) {
        Promise.resolve(n.onAccountEvent(result.event, result.notary)).catch(() => {});
      }

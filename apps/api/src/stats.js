...
function statsDeltasForFunnel(id, dayISO, context, acquisition) {
  if (!domain.isFunnelEvent(id)) return [];
  const day = dayOf(dayISO);
  if (!day) return [];
  // Deux lectures INDÉPENDANTES du même événement, jamais jointes : la
  // provenance (`acq_<source>_<étape>`, #4) et les segments bornés
  // (`segment__<dimension>__<valeur>__<id>`). Aucune ne relie deux événements
  // au même visiteur.
  const adds = { [FUNNEL_COUNTER_PREFIX + id]: 1, ...(acquisition ? counter(acquisition, id) : {}) };

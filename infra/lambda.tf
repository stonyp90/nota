...
resource "aws_lambda_function" "api" {
  ...
  environment {
    variables = {
      ...
      NOTA_ASSISTANT_KEY_PARAM  = var.use_secrets_manager ? "" : var.assistant_key_param
  ...
  lifecycle {
    ...
    precondition {
      condition = !var.use_secrets_manager || (
        var.domain_name != "" && var.base_url == "https://${var.domain_name}" &&
        trimspace(var.sender_address) != "" && var.from_email != "" && var.reply_to_email != ""
      )
      error_message = "Production Secrets Manager rollout requires the canonical HTTPS origin, sender, monitored Reply-To, and business mailing address."
    }
  }
}
